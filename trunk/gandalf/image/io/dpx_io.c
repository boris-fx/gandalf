 ...
 * Last edited:   $Date: 2007/03/13 14:00:43 $
 * Author:        $Author: pm $
                         ...
                         Gan_Bool (*abortRequested)(void*), void* abortObj)
{
   ...
   switch(eFormat)
   {
      ...
      case GAN_YUV422:
        for(uiRow = 0; uiRow<ui32LinesPerImageEle; uiRow++)
        {
           Gan_YUV422Pixel_ui8 *pui8yuvImPix, *pui8yuvBufPix;
           ...
           if(bSingleField)
           {
              ...
              if((bUpper && (uiRow % 2) == 0) || (!bUpper && (uiRow % 2) == 1))
              {
                 ...
                 for(iCol=(int)ui32PixelsPerLine-1,
                     pui8yuvImPix = gan_image_get_pixptr_yuv422_ui8(pgiImage, bWholeImage ? (bFlip ? (ui32LinesPerImageEle-uiRow-1) : uiRow) : (bFlip ? (uiInternalHeight-uiRow/2-1) : uiRow/2), iCol),
                     pui8yuvBufPix = ((Gan_YUV422Pixel_ui8*)acAlignedBuffer) + iCol; iCol >= 0; iCol--)
                    *pui8yuvImPix-- = *pui8yuvBufPix--;
              }
              /* otherwise ignore this scanline */
              else if(fseek(pfInFile, uiRowSizeInBytes, SEEK_CUR) != 0)
              {
           ...
           else
           {
              ...
              for(iCol=(int)ui32PixelsPerLine-1,
                  pui8yuvImPix = gan_image_get_pixptr_yuv422_ui8(pgiImage, bFlip ? (ui32LinesPerImageEle-uiRow-1) : uiRow, iCol),
                  pui8yuvBufPix = ((Gan_YUV422Pixel_ui8*)acAlignedBuffer) + iCol; iCol >= 0; iCol--)
                 *pui8yuvImPix-- = *pui8yuvBufPix--;
           }
                          ...
                          Gan_Bool (*abortRequested)(void*), void* abortObj)
{
   ...
   switch(eFormat)
   {
      ...
      case GAN_YUV422:
        for(uiRow = 0; uiRow<ui32LinesPerImageEle; uiRow++)
        {
           Gan_YUV422Pixel_ui16* pui16yuvPix;
           gan_uint32* pui32Pix;
           unsigned int uiCol;
           ...
           if(bSingleField)
           {
              ...
              if((bUpper && (uiRow % 2) == 0) || (!bUpper && (uiRow % 2) == 1))
              {
                 ...
                 for(uiCol=0,
                     pui16yuvPix = gan_image_get_pixptr_yuv422_ui16(pgiImage, bWholeImage ? (bFlip ? (ui32LinesPerImageEle-uiRow-1) : uiRow) : (bFlip ? (uiInternalHeight-uiRow/2-1) : uiRow/2), 0),
                     pui32Pix = (gan_uint32*)acAlignedBuffer; uiCol < ui32PixelsPerLine; uiCol++, pui16yuvPix++)
                 {
                    switch(uiCol % 3)
                    {
                       case 0:
                         ...
                         pui16yuvPix->UV = (gan_uint16)(((*pui32Pix) & 0xffc00000) >> 16);
                         pui16yuvPix->Y  = (gan_uint16)(((*pui32Pix) &   0x3ff000) >>  6);
                         break;

                       case 1:
                         pui16yuvPix->UV = (gan_uint16)(((*pui32Pix) &      0xffc) <<  4);
                         pui32Pix++;
                         if(bReversedEndianness)
                            vReverseEndianness32(pui32Pix);
                       ...
                       case 2:
                         pui16yuvPix->UV = (gan_uint16)(((*pui32Pix) &   0x3ff000) >>  6);
                         pui16yuvPix->Y  = (gan_uint16)(((*pui32Pix) &      0xffc) <<  4);
                         pui32Pix++;
                    }
                 }
              }
           ...
           else
           {
              ...
              for(uiCol=0,
                  pui16yuvPix = gan_image_get_pixptr_yuv422_ui16(pgiImage, bFlip ? (ui32LinesPerImageEle-uiRow-1) : uiRow, 0),
                  pui32Pix = (gan_uint32*)acAlignedBuffer; uiCol < ui32PixelsPerLine; uiCol++, pui16yuvPix++)
              {
                 switch(uiCol % 3)
                 {
                          ...
                          Gan_Bool (*abortRequested)(void*), void* abortObj)
{
   ...
   switch(eFormat)
   {
      ...
      case GAN_YUV422:
        for(uiRow = 0; uiRow<ui32LinesPerImageEle; uiRow++)
        {
           Gan_YUV422Pixel_ui16 *pui16yuvImPix, *pui16yuvBufPix;
           ...
           if(bSingleField)
           {
              ...
              if((bUpper && (uiRow % 2) == 0) || (!bUpper && (uiRow % 2) == 1))
              {
                 ...
                 {
                    for(iCol=(int)ui32PixelsPerLine-1,
                        pui16yuvImPix = gan_image_get_pixptr_yuv422_ui16(pgiImage, bWholeImage ? (bFlip ? (ui32LinesPerImageEle-uiRow-1) : uiRow) : (bFlip ? (uiInternalHeight-uiRow/2-1) : uiRow/2), iCol),
                        pui16yuvBufPix = ((Gan_YUV422Pixel_ui16*)acAlignedBuffer) + iCol; iCol >= 0; iCol--, pui16yuvImPix--, pui16yuvBufPix--)
                    {
                       *pui16yuvImPix = *pui16yuvBufPix;
                       vReverseEndianness16(&pui16yuvImPix->UV);
                       vReverseEndianness16(&pui16yuvImPix->Y);
                    }
                 }
                 else
                    for(iCol=(int)ui32PixelsPerLine-1,
                        pui16yuvImPix = gan_image_get_pixptr_yuv422_ui16(pgiImage, bWholeImage ? (bFlip ? (ui32LinesPerImageEle-uiRow-1) : uiRow) : (bFlip ? (uiInternalHeight-uiRow/2-1) : uiRow/2), iCol),
                        pui16yuvBufPix = ((Gan_YUV422Pixel_ui16*)acAlignedBuffer) + iCol; iCol >= 0; iCol--)
                       *pui16yuvImPix-- = *pui16yuvBufPix--;
              }
              /* otherwise ignore this scanline */
              else if(fseek(pfInFile, uiRowSizeInBytes, SEEK_CUR) != 0)
           ...
           else
           {
              ...
              {
                 for(iCol=(int)ui32PixelsPerLine-1,
                     pui16yuvImPix = gan_image_get_pixptr_yuv422_ui16(pgiImage, bFlip ? (ui32LinesPerImageEle-uiRow-1) : uiRow, iCol),
                     pui16yuvBufPix = ((Gan_YUV422Pixel_ui16*)acAlignedBuffer) + iCol; iCol >= 0;
                     iCol--, pui16yuvImPix--, pui16yuvBufPix--)
                 {
                    *pui16yuvImPix = *pui16yuvBufPix;
                    vReverseEndianness16(&pui16yuvImPix->UV);
                    vReverseEndianness16(&pui16yuvImPix->Y);
 ...
 */
Gan_Image *
 gan_read_dpx_image(const Gan_UnicodeChar *filename, Gan_Image *image, const struct Gan_ImageReadControlStruct *ictrlstr, struct Gan_ImageHeaderStruct *header,
                    Gan_Bool (*abortRequested)(void*), void* abortObj)
{
   FILE *infile;
   Gan_Image *result;
   ...
   {
      gan_err_flush_trace();
      gan_err_register_unicode( "gan_read_dpx_image", GAN_ERROR_OPENING_FILE, filename );
      return NULL;
   }
   ...
   for(uiRow = 0; uiRow<uiHeight; uiRow++)
   {
      ...
      else
      {
         ...
         switch(pgiImage->format)
         {
            ...
            case GAN_YUV422:
            {
               Gan_YUV422Pixel_ui8 *pui8yuv422ImPix, *pui8yuv422BufPix;

               memset((void*)acAlignedBuffer, 0, uiRowSizeInBytes+uieolPadding);
               for(iCol=(int)pgiImage->width-1,
                   pui8yuv422ImPix = gan_image_get_pixptr_yuv422_ui8(pgiImage, uiImRow, iCol),
                   pui8yuv422BufPix = ((Gan_YUV422Pixel_ui8*)acAlignedBuffer) + iCol; iCol >= 0; iCol--)
                  *pui8yuv422BufPix-- = *pui8yuv422ImPix--;
                                                         ...
                                                         const Gan_ImageWriteControlStruct *octrlstr )
{
   ...
   *((gan_float32*)(acAlignedHeader + OFFSET_REFHIGHQUANTITY0)) = f32RefHighQuantity;
   ...
   switch(image->format)
   {
      ...
      case GAN_YUV422:
        *((gan_uint8*)(acAlignedHeader + OFFSET_DESCRIPTOR0)) = 100; /* yuv422 */
        break;
 ...
 */
Gan_Bool
 gan_write_dpx_image ( const Gan_UnicodeChar *filename, const Gan_Image *image, const struct Gan_ImageWriteControlStruct *octrlstr )
{
   if(filename == NULL)
   ...
   else
   {
      ...
      if(octrlstr != NULL && octrlstr->single_field)
      {
         ...
         if(outfile != NULL)
         {
            ...
            {
               fclose(outfile);
               gan_err_register_unicode ( "gan_write_dpx_image", GAN_ERROR_FAILURE, filename );
               return GAN_FALSE;
            }
      ...
      {
         gan_err_flush_trace();
         gan_err_register_unicode ( "gan_write_dpx_image", GAN_ERROR_OPENING_FILE, filename );
         return GAN_FALSE;
      }
