 ...
 */
Gan_Image *
 gan_read_jpeg_image(const Gan_Char *filename, Gan_Image *image,
                     const struct Gan_ImageReadControlStruct *ictrlstr, struct Gan_ImageHeaderStruct *header,
                     Gan_Bool (*abortRequested)(void*), void* abortObj)
{
   FILE *infile;
 ...
 */
Gan_Bool
 gan_write_jpeg_image ( const Gan_Char *filename, const Gan_Image *image, const struct Gan_ImageWriteControlStruct *octrlstr )
{
   if(filename == NULL)
