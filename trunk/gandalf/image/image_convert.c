 ...
 * Last edited:   $Date: 2007/02/13 17:32:12 $
 * Author:        $Author: pm $
...
{
   if(dest == NULL)
      dest = gan_image_alloc(source->format, source->type, source->height, source->width);
   else if(dest != source)
      dest = gan_image_set_format_type_dims(dest, source->format, source->type, source->height, source->width);
