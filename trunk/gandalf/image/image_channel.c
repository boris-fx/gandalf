 ...
 * Last edited:   $Date: 2007/06/04 08:53:39 $
 * Author:        $Author: pm $
        ...
        for ( i = (int)height-1; i >= 0; i-- )
           for ( j = (int)width-1; j >= 0; j-- )
              gan_image_set_pix_gl_uc ( dest, i, j,
                                        gan_image_get_pixptr_gla_uc(source, r0+i, c0+j)->I );
        break;
...
static Gan_Bool insert_gl_into_gl ( const Gan_Image *source,
                 ...
                 case GAN_ALPHA_CHANNEL:
                   for(r=(int)source->height-1; r>=0; r--)
                      for(c=(int)source->width-1, spix = gan_image_get_pixptr_gl_uc(source,r,0),
                          dpix = gan_image_get_pixptr_gl_uc(dest,r,0); c>=0; c--, dpix++)
                         *dpix = *spix++;
                   break;
                 ...
                 case GAN_ALPHA_CHANNEL:
                   for(r=(int)source->height-1; r>=0; r--)
                      for(c=(int)source->width-1, spix = gan_image_get_pixptr_gl_us(source,r,0),
                          dpix = gan_image_get_pixptr_gl_us(dest,r,0); c>=0; c--, dpix++)
                         *dpix = *spix++;
                   break;
                 ...
                 case GAN_ALPHA_CHANNEL:
                   for(r=(int)source->height-1; r>=0; r--)
                      for(c=(int)source->width-1, spix = gan_image_get_pixptr_gl_f(source,r,0),
                          dpix = gan_image_get_pixptr_gl_f(dest,r,0); c>=0; c--, dpix++)
                         *dpix = *spix++;
                   break;
                                    ...
                                    Gan_ImageChannelType dchannel )
{
   ...
   switch(source->format)
   {
      case GAN_GREY_LEVEL_IMAGE:
        ...
        {
           case GAN_GREY_LEVEL_IMAGE:
             result = insert_gl_into_gl(source, schannel, dest, dchannel);
             break;
