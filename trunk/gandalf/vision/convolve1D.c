 ...
 * Last edited:   $Date: 2007/04/18 16:44:59 $
 * Author:        $Author: pm $
                    ...
                    Gan_Mask1D *mask, int dsize )
{
   ...
   gan_err_test_bool ( mask->type == GAN_FLOAT, "gan_convolve1D_f", GAN_ERROR_INCOMPATIBLE, "" );
   switch ( mask->format )
   {
      case GAN_MASK1D_SYMMETRIC:
      {
         ...
         switch ( behaviour )
         {
            case GAN_EDGE_BEHAVIOUR_CLIP:
            {
               ...
               for ( i = dsize-1; i >= 0; i--, dest += dstride, source += sstride )
               {
                  total = mask->data.f[0]*source[0];
                  for ( j = half_size; j > 0; j-- )
                     total += mask->data.f[j]*(source[j*sstride] + source[-j*sstride]);
            ...
            case GAN_EDGE_BEHAVIOUR_REPEAT:
            {
               for ( i = dsize-1; i >= dsize-half_size; i-- )
               {
                  ...
                  for ( j = half_size; j > 0; j-- )
                     total += mask->data.f[j]*(source[SHIFT_DOWN_REPEAT(i+j,dsize)*sstride] + source[(i-j)*sstride]);
               ...
               for ( ; i >= half_size; i-- )
               {
                  total = mask->data.f[0]*source[i*sstride];
                  for ( j = half_size; j > 0; j-- )
                     total += mask->data.f[j]*(source[(i+j)*sstride] + source[(i-j)*sstride]);
               ...
               for ( ; i >= 0; i-- )
               {
                  total = mask->data.f[0]*source[i*sstride];
                  for ( j = half_size; j > 0; j-- )
                     total += mask->data.f[j]*(source[(i+j)*sstride] + source[SHIFT_UP_REPEAT((i-j),dsize)*sstride]);
            ...
            case GAN_EDGE_BEHAVIOUR_CIRCULAR:
            {
               for ( i = dsize-1; i >= 0; i-- )
               {
                  total = mask->data.f[0]*source[i*sstride];
                  for ( j = half_size; j > 0; j-- )
                     total += mask->data.f[j]*(source[SHIFT_DOWN_CIRCULAR(i+j,dsize)*sstride]+source[SHIFT_UP_CIRCULAR(i-j,dsize)*sstride]);
         ...
         switch ( behaviour )
         {
            case GAN_EDGE_BEHAVIOUR_CLIP:
            {
                     ...
                     i--, dest += dstride, source += sstride )
               {
                  total = 0.0;
                  for ( j = half_size; j > 0; j-- )
                     total += mask->data.f[j-1]*(source[j*sstride] - source[-j*sstride]);
         ...
         switch ( behaviour )
         {
            case GAN_EDGE_BEHAVIOUR_CLIP:
            {
                     ...
                     i--, dest += dstride, source += sstride )
               {
                  ...
                  for ( j = half_size; j > 0; j-- )
                     total += mask->data.d[j]*(source[j*sstride] + source[-j*sstride]);
            ...
            case GAN_EDGE_BEHAVIOUR_REPEAT:
            {
               for ( i = dsize-1; i >= dsize-half_size; i-- )
               {
                  ...
                  for ( j = half_size; j > 0; j-- )
                     total += mask->data.d[j]*(source[SHIFT_DOWN_REPEAT(i+j,dsize)*sstride] + source[(i-j)*sstride]);
               ...
               for ( ; i >= half_size; i-- )
               {
                  total = mask->data.d[0]*source[i*sstride];
                  for ( j = half_size; j > 0; j-- )
                     total += mask->data.d[j]*(source[(i+j)*sstride] + source[(i-j)*sstride]);
               ...
               for ( ; i >= 0; i-- )
               {
                  total = mask->data.d[0]*source[i*sstride];
                  for ( j = half_size; j > 0; j-- )
                     total += mask->data.d[j]*(source[(i+j)*sstride] + source[SHIFT_UP_REPEAT((i-j),dsize)*sstride]);
            ...
            case GAN_EDGE_BEHAVIOUR_CIRCULAR:
            {
               for ( i = dsize-1; i >= 0; i-- )
               {
                  total = mask->data.d[0]*source[i*sstride];
                  for ( j = half_size; j > 0; j-- )
                     total += mask->data.d[j]*(source[SHIFT_DOWN_CIRCULAR(i+j,dsize)*sstride]+source[SHIFT_UP_CIRCULAR(i-j,dsize)*sstride]);
         ...
         switch ( behaviour )
         {
            case GAN_EDGE_BEHAVIOUR_CLIP:
            {
                     ...
                     i--, dest += dstride, source += sstride )
               {
                  total = 0.0;
                  for ( j = half_size; j > 0; j-- )
                     total += mask->data.d[j-1]*(source[j*sstride] - source[-j*sstride]);
         ...
         switch ( behaviour )
         {
            case GAN_EDGE_BEHAVIOUR_CLIP:
            {
                     ...
                     i--, dest += dstride, source += sstride )
               {
                  ...
                  for ( j = half_size; j > 0; j-- )
                     total += mask->data.i[j]*(source[j*sstride] + source[-j*sstride]);
            ...
            case GAN_EDGE_BEHAVIOUR_REPEAT:
            {
               for ( i = dsize-1; i >= dsize-half_size; i-- )
               {
                  ...
                  for ( j = half_size; j > 0; j-- )
                     total += mask->data.i[j]
                     *(source[SHIFT_DOWN_REPEAT(i+j,dsize)*sstride] + source[(i-j)*sstride]);
               ...
               for ( ; i >= half_size; i-- )
               {
                  total = mask->data.i[0]*source[i*sstride];
                  for ( j = half_size; j > 0; j-- )
                     total += mask->data.i[j]*(source[(i+j)*sstride] + source[(i-j)*sstride]);
               ...
               for ( ; i >= 0; i-- )
               {
                  total = mask->data.i[0]*source[i*sstride];
                  for ( j = half_size; j > 0; j-- )
                     total += mask->data.i[j]*(source[(i+j)*sstride] + source[SHIFT_UP_REPEAT((i-j),dsize)*sstride]);
            ...
            case GAN_EDGE_BEHAVIOUR_CIRCULAR:
            {
               for ( i = dsize-1; i >= 0; i-- )
               {
                  total = mask->data.i[0]*source[i*sstride];
                  for ( j = half_size; j > 0; j-- )
                     total += mask->data.i[j]*(source[SHIFT_DOWN_CIRCULAR(i+j,dsize)*sstride] + source[SHIFT_UP_CIRCULAR(i-j,dsize)*sstride]);
         ...
         switch ( behaviour )
         {
            case GAN_EDGE_BEHAVIOUR_CLIP:
            {
                     ...
                     i--, dest += dstride, source += sstride )
               {
                  total = 0;
                  for ( j = half_size; j > 0; j-- )
                     total += mask->data.i[j-1]*(source[j*sstride] - source[-j*sstride]);
         ...
         switch ( behaviour )
         {
            case GAN_EDGE_BEHAVIOUR_CLIP:
            {
                     ...
                     i--, dest += dstride, source += sstride )
               {
                  ...
                  for ( j = (int)half_size; j > 0; j-- )
                     total += mask->data.f[j]*((float)source[j*sstride] + (float)source[-j*sstride]);
            ...
            case GAN_EDGE_BEHAVIOUR_REPEAT:
            {
               for ( i = dsize-1; i >= dsize-half_size; i-- )
               {
                  ...
                  for ( j = half_size; j > 0; j-- )
                     total += mask->data.f[j]*((float)source[SHIFT_DOWN_REPEAT(i+j,dsize)*sstride] + (float)source[(i-j)*sstride]);
               ...
               for ( ; i >= half_size; i-- )
               {
                  total = mask->data.f[0]*(float)source[i*sstride];
                  for ( j = half_size; j > 0; j-- )
                     total += mask->data.f[j]*((float)source[(i+j)*sstride] + (float)source[(i-j)*sstride]);
               ...
               for ( ; i >= 0; i-- )
               {
                  total = mask->data.f[0]*(float)source[i*sstride];
                  for ( j = half_size; j > 0; j-- )
                     total += mask->data.f[j]*((float)source[(i+j)*sstride] + (float)source[SHIFT_UP_REPEAT((i-j),dsize)*sstride]);
            ...
            case GAN_EDGE_BEHAVIOUR_CIRCULAR:
            {
               for ( i = dsize-1; i >= 0; i-- )
               {
                  total = mask->data.f[0]*(float)source[i*sstride];
                  for ( j = half_size; j > 0; j-- )
                     total += mask->data.f[j]*((float)source[SHIFT_DOWN_CIRCULAR(i+j,dsize)*sstride] + (float)source[SHIFT_UP_CIRCULAR(i-j,dsize)*sstride]);
            ...
            case GAN_EDGE_BEHAVIOUR_EXPAND:
            {
               ...
               for ( ; i >= 2*half_size; i-- )
               {
                  ...
                  for ( j = half_size; j > 0; j-- )
                     total += mask->data.f[j]*(source[(i-half_size+j)*sstride] + source[(i-half_size-j)*sstride]);
         ...
         switch ( behaviour )
         {
            case GAN_EDGE_BEHAVIOUR_CLIP:
            {
                     ...
                     i--, dest += dstride, source += sstride )
               {
                  total = 0.0F;
                  for ( j = half_size; j > 0; j-- )
                     total += mask->data.f[j-1]*((float)source[j*sstride] - (float)source[-j*sstride]);
   ...
   switch ( mask->format )
   {
      case GAN_MASK1D_SYMMETRIC:
      {
               ...
               i--, dest += dstride, source += sstride )
         {
            ...
            for ( j = (int)half_size; j > 0; j-- )
               total += (double)mask->data.f[j]*((double)source[j*sstride] + (double)source[-j*sstride]);
      ...
      case GAN_MASK1D_ANTISYMMETRIC:
      {
               ...
               i--, dest += dstride, source += sstride )
         {
            total = 0.0;
            for ( j = (int)half_size; j > 0; j-- )
               total += (double)mask->data.f[j-1]*((double)source[j*sstride] - (double)source[-j*sstride]);
                           ...
                           Gan_Mask1D *mask, Gan_Image *dest )
{
   ...
   switch ( image->type )
   {
      ...
      case GAN_UCHAR:
        for ( i = (int)dest->height-1; i >= 0; i-- )
           if ( !gan_convolve1D_uc ( gan_image_get_pixptr_gl_uc(image,i,0), 1, eEdgeBehaviour, GAN_FALSE,
                                     gan_image_get_pixptr_gl_uc(dest,i,0),  1, mask, (int)dwidth ) )
           {
              gan_err_register ( "image_convolve1Dx_gl_gl", GAN_ERROR_FAILURE, "" );
              return NULL;
           }
      ...
      case GAN_USHORT:
        for ( i = (int)dest->height-1; i >= 0; i-- )
           if ( !gan_convolve1D_us ( gan_image_get_pixptr_gl_us(image,i,0), 1, eEdgeBehaviour, GAN_FALSE,
                                     gan_image_get_pixptr_gl_us(dest,i,0),  1, mask, (int)dwidth ) )
           {
              gan_err_register ( "image_convolve1Dx_gl_gl", GAN_ERROR_FAILURE, "" );
              return NULL;
           }
...
Gan_Image *
 gan_image_convolve1Dx_q ( const Gan_Image *image, Gan_ImageChannelType channel,
                           ...
                           Gan_Mask1D *mask, Gan_Image *dest )
{
   Gan_Mask1D* newMask=NULL;

   /* restrict size of filter if necessary */
   if(mask->size >= image->width)
   {
      newMask = gan_mask1D_alloc(mask->format, mask->type, mask->size);
      if(newMask == NULL)
      {
      ...
      gan_mask1D_copy_q(mask, newMask);
      newMask->size = 2*((dest->width-1)/2)+1;

      /* limited support right now... */
      if ( mask->format != GAN_MASK1D_SYMMETRIC || mask->type != GAN_FLOAT )
      {
         gan_err_flush_trace();
         gan_err_register ( "gan_image_convolve1Dy_q", GAN_ERROR_ILLEGAL_IMAGE_CHANNEL, "" );
         return NULL;
      }
   ...
   switch ( image->type )
   {
      ...
      case GAN_USHORT:
        gan_err_test_ptr ( (image->stride % sizeof(unsigned short)) == 0 &&
                           ...
                           "" );
        for ( j = (int)dest->width-1; j >= 0; j-- )
           if ( !gan_convolve1D_us ( gan_image_get_pixptr_gl_us(image,0,j), image->stride/sizeof(unsigned short),
                                     eEdgeBehaviour, GAN_FALSE,
                           ...
                           Gan_Mask1D *mask, Gan_Image *dest )
{
   ...
   if(mask->size >= image->height)
   {
      ...
      gan_mask1D_copy_q(mask, newMask);
      newMask->size = 2*((dest->height-1)/2)+1;
