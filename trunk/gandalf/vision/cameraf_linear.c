/**
 * File:          $RCSfile: cameraf_linear.c,v $
                 ...
                 int *error_code )
{
   ...
   (void)gan_vec3f_fill_q ( p, CX0 + CFX*XZ, CY0 + CFY*YZ, CZH );
                     ...
                     int *error_code )
{
   ...
   /* consistency check */
   gan_err_test_bool ( camera->type == GAN_LINEAR_CAMERA, "point_backproject",
                       GAN_ERROR_INCOMPATIBLE, "" );
 ...
 * \sa gan_camera_build_linear().
 */
Gan_Bool
 gan_cameraf_build_linear ( Gan_Camera_f *camera,
                            float zh, float fx, float fy,
                            float x0, float y0 )
{
   if ( !gan_cameraf_set_common_fields ( camera, GAN_LINEAR_CAMERA,
                                         zh, fx, fy, x0, y0 ) )
   {
      gan_err_register ( "gan_cameraf_build_linear", GAN_ERROR_FAILURE, "" );
      return GAN_FALSE;
   }
