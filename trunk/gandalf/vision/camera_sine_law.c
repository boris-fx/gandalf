/**
 * File:          $RCSfile: camera_sine_law.c,v $
                 ...
                 int *error_code )
{
   ...
   /* consistency check */
   gan_err_test_bool ( camera->type == GAN_SINE_LAW_CAMERA,
                        ...
                        int *error_code )
{
   ...
   /* compute sine law image coordinates */
...
/**
 * \brief Builds a structure representing a sine law camera.
 ...
 *
 * Constructs a structure representing a camera, under the sine law
 ...
 * \sa gan_cameraf_build_sine_law().
 */
Gan_Bool
 gan_camera_build_sine_law ( Gan_Camera *camera,
                             double zh, double fx, double fy,
                             double x0, double y0 )
{
   if ( !gan_camera_set_common_fields ( camera, GAN_SINE_LAW_CAMERA,
                                        zh, fx, fy, x0, y0 ) )
   {
      gan_err_register ( "gan_camera_build_sine_law", GAN_ERROR_FAILURE,
                         "" );
      return GAN_FALSE;
   }
