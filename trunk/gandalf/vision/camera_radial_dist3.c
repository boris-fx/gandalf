/**
 * File:          $RCSfile: camera_radial_dist3.c,v $
                 ...
                 int *error_code )
{
   ...
   if ( R2 > CTR2 )
   {
      ...
      if ( HC != NULL )
      {
         ...
         HC[0].nonlinear.radial3.K1 = 0.0;
         HC[0].nonlinear.radial3.K2 = 0.0;
         HC[0].nonlinear.radial3.K3 = 0.0;
         HC[1].zh = 0.0;
         HC[1].fx = 0.0;
         HC[1].fy = 0.0;
         HC[1].x0 = 0.0;
         ...
         HC[1].nonlinear.radial3.K1 = 0.0;
         HC[1].nonlinear.radial3.K2 = 0.0;
         HC[1].nonlinear.radial3.K3 = 0.0;
      }
   ...
   if ( HX != NULL )
      (void)gan_mat22_fill_q ( HX,
                    CFX*(1.0 + CK1*(3.0*XZ2 + YZ2) + R2*(CK2*(5.0*XZ2 + YZ2) + R2*CK3*(7.0*XZ2 + YZ2))),
                    2.0*CFX*XZ*YZ*(CK1 + R2*(2.0*CK2 + 3.0*R2*CK3)),
                    2.0*CFY*XZ*YZ*(CK1 + R2*(2.0*CK2 + 3.0*R2*CK3)),
                    CFY*(1.0 + CK1*(3.0*YZ2 + XZ2) + R2*(CK2*(5.0*YZ2 + XZ2) + R2*CK3*(7.0*YZ2 + XZ2))) );
...
static Gan_Bool
 compute_distortion ( double K1, double K2, double K3,
                      ...
                      double *dp, int *error_code )
{
   ...
   if ( (K1 == 0.0 && K2 == 0.0 && K3 == 0.0) || dR < SMALL_DR_THRES )
   {
      *dp = 1.0;
      return GAN_TRUE;
   }
   ...
   {
      R2 = R*R;
      deriv = 1.0 + R2*(3.0*K1 + R2*(5.0*K2 + 7.0*R2*K3));
      if ( deriv == 0.0 )
      {
         if ( error_code == NULL )
         {
      ...
      Rdiff = (R*(1.0 + R2*(K1 + R2*(K2 + R2*K3))) - dR)/deriv;
      R -= Rdiff;
 ...
 * \sa gan_cameraf_build_radial_distortion_3().
 */
Gan_Bool
 gan_camera_build_radial_distortion_3 ( Gan_Camera *camera,
                                        ...
                                        double K1, double K2, double K3 )
{
   ...
   if ( !gan_camera_set_common_fields ( camera, GAN_RADIAL_DISTORTION_3,
                                        zh, fx, fy, x0, y0 ) )
   {
      gan_err_register ( "gan_camera_build_radial_distortion_3",
                         GAN_ERROR_FAILURE, "" );
      return GAN_FALSE;
   }
   ...
   if ( minR2 > 0.0 )
   {
      ...
      /* compute threshold on squared radial distance R^2 and distorted
         radial distance d*R */
      CTR2 = MODEL_FRACTION*MODEL_FRACTION*minR2;
      R = sqrt(CTR2);
      CTDR = (1.0 + CTR2*(K1 + CTR2*(K2 + CTR2*K3)))*R;

      /* compute parameters of outer linear model */
      COA = 1.0 + CTR2*(3.0*K1 + CTR2*(5.0*K2 + CTR2*7.0*K3));
      COB = CTDR - COA*R;
   }
   else
