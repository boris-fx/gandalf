...
Gan_Bool
 gan_mat_realloc_gen ( Gan_Matrix *A, unsigned long rows, unsigned long cols )
{
   /* re-allocate matrix data */
   gan_err_test_bool ( A->data_alloc, "gan_mat_realloc_gen",
   ...
   {
      gan_err_flush_trace();
      gan_err_register_with_number ( "gan_mat_realloc_gen", GAN_ERROR_MALLOC_FAILED, "", A->data_size*sizeof(double) );
      return GAN_FALSE;
   }
 ...
 */
Gan_Matrix *
 gan_mat_set_dims ( Gan_Matrix *A, unsigned long rows, unsigned long cols )
{
   /* check whether matrix A is big enough; if it isn't, reallocate it */
   if ( A->data_size < rows*cols )
   {
      /* re-allocate matrix data */
      if ( !gan_mat_realloc ( A, rows, cols ) )
      {
         gan_err_register ( "gan_mat_set_dims", GAN_ERROR_FAILURE, "" );
         return NULL;
      }
   }
   else
 ...
 */
double
 gan_mat_sumsqr ( const Gan_Matrix *A )
{
   long i;
   double total = 0.0;
 ...
 */
double
 gan_mat_Fnorm ( const Gan_Matrix *A )
{
   long i;
   double total = 0.0;
 ...
 */
Gan_Matrix *
 gan_mat_invert_q ( const Gan_Matrix *A, Gan_Matrix *B )
{
   long *ipiv;
   double *work;

   gan_err_test_ptr ( A->rows == A->cols, "gan_mat_invert_q",
                      GAN_ERROR_ILLEGAL_ARGUMENT,
                      "cannot invert non-square matrix" );

   /* allocate/set output matrix */
   if ( B == NULL )
      B = gan_mat_alloc ( A->rows, A->cols );
   else
      B = gan_mat_set_dims ( B, A->rows, A->cols );

   if ( B == NULL )
   {
      gan_err_register ( "gan_mat_invert_q", GAN_ERROR_FAILURE, "" );
      return NULL;
   }
   ...
   work = gan_malloc_array ( double, A->rows );
   if ( ipiv == NULL || work == NULL )
   {
      gan_err_flush_trace();
      gan_err_register_with_number ( "gan_mat_invert_q", GAN_ERROR_MALLOC_FAILED, "", A->rows*sizeof(double) );
      return NULL;
   }

   /* invert using LU decomposition */
   if ( !gan_clapack_getrf ( B, ipiv, NULL ) )
   {
   ...
   if ( !gan_clapack_getri ( B, ipiv, work, A->rows ) )
   {
      gan_free_va ( work, ipiv, NULL );
      gan_err_register ( "gan_mat_invert_q", GAN_ERROR_FAILURE, "" );
      return NULL;
 ...
 */
Gan_Matrix *
 gan_mat_fill_va ( Gan_Matrix *A, unsigned long rows, unsigned long cols, ... )
{
   unsigned int i, j;
   va_list ap;
   ...
   if ( A == NULL )
   {
      gan_err_register ( "gan_mat_fill_va", GAN_ERROR_FAILURE, "" );
      return NULL;
   }
   ...
   for ( i = 0; i < rows; i++ )
      for ( j = 0; j < cols; j++ )
         A->data[j*rows+i] = va_arg ( ap, double );
 ...
 * \return Pointer to the filled matrix, or \c NULL on failure.
 *
 * Fill matrix \a A with values from variable argument list \a aptr, and
 * set the dimensions of \a A to \a rows by \a cols.
 */
Gan_Matrix *
 gan_mat_fill_vap ( Gan_Matrix *A, unsigned long rows, unsigned long cols,
                    va_list *aptr )
{
   unsigned int i, j;
   ...
   if ( A == NULL )
   {
      gan_err_register ( "gan_mat_fill_vap", GAN_ERROR_FAILURE, "" );
      return NULL;
   }
   ...
   for ( i = 0; i < rows; i++ )
      for ( j = 0; j < cols; j++ )
         A->data[j*rows+i] = va_arg ( *aptr, double );
 ...
 *
 * \sa gan_mat_fill_const_s().
 */
Gan_Matrix *
 gan_mat_fill_const_q ( Gan_Matrix *A, unsigned long rows, unsigned long cols,
                        double value )
{
   int i;
   ...
   if ( A == NULL )
   {
      gan_err_register ( "gan_mat_fill_const_q", GAN_ERROR_FAILURE, "" );
      return NULL;
   }
 ...
 */
Gan_Bool
 gan_mat_read_va ( const Gan_Matrix *A, unsigned long rows, unsigned long cols, ... )
{
   unsigned int i, j;
   va_list ap;

   /* check that dimensions match */
   gan_err_test_bool ( rows == A->rows && cols == A->cols, "gan_mat_read_va",
   ...
   for ( i = 0; i < rows; i++ )
      for ( j = 0; j < cols; j++ )
         *(va_arg ( ap, double * )) = A->data[j*A->rows+i];
 ...
 */
Gan_Matrix *
 gan_mat_copy_q ( const Gan_Matrix *A, Gan_Matrix *B )
{
   ...
   if ( B == NULL )
   {
      gan_err_register ( "gan_mat_copy_q", GAN_ERROR_FAILURE, "" );
      return NULL;
   }
   ...
   {
      gan_err_flush_trace();
      gan_err_register ( "gan_mat_copy_q", GAN_ERROR_CBLAS_FAILED, "" );
      return NULL;
   }
 ...
 */
Gan_Matrix *
 gan_mat_scale_q ( Gan_Matrix *A, double a, Gan_Matrix *B )
{
   ...
   if ( B == NULL )
   {
      gan_err_register ( "gan_mat_scale_q", GAN_ERROR_FAILURE, "" );
      return NULL;
   }
   ...
   if ( !gan_dcopy ( nel, A->data, 1, B->data, 1 ) ||
        !gan_dscal ( nel, a, B->data, 1 ) )
   {
      gan_err_flush_trace();
      gan_err_register ( "gan_mat_scale_q", GAN_ERROR_CBLAS_FAILED, "" );
      return NULL;
   }
#endif /* #ifdef HAVE_LAPACK */
 ...
 */
Gan_Matrix *
 gan_mat_divide_q ( Gan_Matrix *A, double a, Gan_Matrix *B )
{
   long i;
   ...
   {
      gan_err_flush_trace();
      gan_err_register ( "gan_mat_divide_q", GAN_ERROR_DIVISION_BY_ZERO, "" );
      return NULL;
   }
 ...
 gan_mat_tpose_q ( Gan_Matrix *A, Gan_Matrix *B )
{
   ...
   else
   {
      if ( A == B )
      {
         ...
         gan_err_test_ptr ( A->rows == A->cols, "gan_mat_tpose_q",
                            GAN_ERROR_FAILURE,
   ...
   if ( B == NULL )
   {
      gan_err_register ( "gan_mat_tpose_q", GAN_ERROR_FAILURE, "" );
      return NULL;
   }
   ...
   {
      for ( i = (long)B->cols-1; i >= 0; i-- )
         if ( !gan_dcopy ( B->rows, &A->data[i], A->rows,
                           &B->data[i*B->rows], 1 ) )
         {
            gan_err_register ( "gan_mat_tpose_q", GAN_ERROR_CBLAS_FAILED, "" );
            return NULL;
         }
   }
   else
   {
      for ( i = (long)B->rows-1; i >= 0; i-- )
         if ( !gan_dcopy ( B->cols, &A->data[i*A->rows], 1,
                           &B->data[i], B->rows ) )
         {
            gan_err_register ( "gan_mat_tpose_q", GAN_ERROR_CBLAS_FAILED, "" );
            return NULL;
 ...
 */
Gan_Matrix *
 gan_mat_add_gen ( Gan_Matrix *A, Gan_TposeFlag A_tr,
                   Gan_Matrix *B, Gan_TposeFlag B_tr, Gan_Matrix *C )
{
   ...
   gan_err_test_ptr ( B_tr ? (B->rows == Ccols && B->cols == Crows)
                      ...
                      "gan_mat_add_gen", GAN_ERROR_INCOMPATIBLE, "" );

   /* allocate result matrix C if necessary */
   if ( C == NULL )
      C = gan_mat_alloc ( Crows, Ccols );
   else
      C = gan_mat_set_dims ( C, Crows, Ccols );

   if ( C == NULL )
   {
      gan_err_register ( "gan_mat_add_gen", GAN_ERROR_FAILURE, "" );
      return NULL;
   }
...
#else
   {
      ...
      if ( C == A )
      {
         if ( !gan_daxpy ( Crows*Ccols, 1.0, B->data, 1, A->data, 1 ) )
         {
            gan_err_flush_trace();
            gan_err_register ( "gan_mat_add_gen", GAN_ERROR_CBLAS_FAILED, "" );
            return NULL;
         }
      }
      else if ( C == B )
      {
         /* in-place operation B += A */
         if ( !gan_daxpy ( Crows*Ccols, 1.0, A->data, 1, B->data, 1 ) )
         {
            gan_err_flush_trace();
            gan_err_register ( "gan_mat_add_gen", GAN_ERROR_CBLAS_FAILED, "" );
            return NULL;
      ...
      {
         /* C = A + B */
         if ( !gan_dcopy ( Crows*Ccols, A->data, 1, C->data, 1 ) ||
              !gan_daxpy ( Crows*Ccols, 1.0, B->data, 1, C->data, 1 ) )
         {
            gan_err_flush_trace();
            gan_err_register ( "gan_mat_add_gen", GAN_ERROR_CBLAS_FAILED, "" );
            return NULL;
   ...
   else if ( !A_tr )
   {
      ...
      gan_err_test_ptr ( B != C, "gan_mat_add_gen",
                         GAN_ERROR_INPLACE_TRANSPOSE, "" );

      if ( A == B )
      {
         gan_err_test_ptr ( A->rows == A->cols, "gan_mat_add_gen",
...
Gan_SquMatrix *
 gan_mat_add_sym_gen (
     ...
     const Gan_Matrix *B, Gan_TposeFlag B_tr, struct Gan_SquMatrix *C )
{
   /* result matrix dimensions */
   ...
   gan_err_test_ptr ( (A_tr == B_tr) ? (A->rows==B->rows && A->cols==B->cols)
                      ...
                      "gan_mat_add_sym_gen", GAN_ERROR_INCOMPATIBLE,
                      "input matrices" );

   /* allocate result matrix C if necessary */
   if ( C == NULL )
      C = gan_symmat_alloc ( Csize );
   else
      C = gan_symmat_set_size ( C, Csize );

   if ( C == NULL )
   {
      gan_err_register ( "gan_mat_add_sym_gen", GAN_ERROR_FAILURE, "" );
      return NULL;
   }
 ...
 */
Gan_Matrix *
 gan_mat_sub_gen ( Gan_Matrix *A, Gan_TposeFlag A_tr,
                   Gan_Matrix *B, Gan_TposeFlag B_tr, Gan_Matrix *C )
{
   /* result matrix dimensions */
   unsigned long Crows = (A_tr ? A->cols : A->rows),
   ...
   gan_err_test_ptr ( B_tr ? (B->rows == Ccols && B->cols == Crows)
                           : (B->rows == Crows && B->cols == Ccols),
                      "gan_mat_sub_gen", GAN_ERROR_INCOMPATIBLE, "" );
   ...
   if ( C == NULL )
   {
      gan_err_register ( "gan_mat_sub_gen", GAN_ERROR_FAILURE, "" );
      return NULL;
   }
...
#else
   {
      ...
      if ( C == A )
      {
         ...
         {
            gan_err_flush_trace();
            gan_err_register ( "gan_mat_sub_gen", GAN_ERROR_CBLAS_FAILED, "" );
            return NULL;
         }
      }
      else if ( C == B )
      {
         /* in-place operation B = A - B */
         if ( !gan_dscal ( Crows*Ccols, -1.0, B->data, 1 ) ||
              !gan_daxpy ( Crows*Ccols, 1.0, A->data, 1, B->data, 1 ) )
         {
            gan_err_flush_trace();
            gan_err_register ( "gan_mat_sub_gen", GAN_ERROR_CBLAS_FAILED, "" );
            return NULL;
   ...
   else if ( !A_tr )
   {
      ...
      gan_err_test_ptr ( B != C && A != B, "gan_mat_sub_gen",
                         GAN_ERROR_INPLACE_TRANSPOSE, "");
 ...
 */
Gan_Vector *
 gan_mat_multv_gen ( const Gan_Matrix *A, Gan_TposeFlag A_tr, const Gan_Vector *x,
                     Gan_Vector *y )
{
   ...
   gan_err_test_ptr ( (A_tr ? A->rows : A->cols) == x->rows,
                      "gan_mat_multv_gen", GAN_ERROR_INCOMPATIBLE, "" );

   /* allocate result vector y if necessary */
   if ( y == NULL )
      y = gan_vec_alloc ( yrows );
   else
   {
      /* can't do in-place matrix/vector multiply with generic matrix */
      gan_err_test_ptr ( y != x,  "gan_mat_multv_gen", GAN_ERROR_FAILURE,
                         "can't do in-place multiply" );
      y = gan_vec_set_size ( y, yrows );
   }
   
   if ( y == NULL || gan_vec_fill_zero_q ( y, y->rows ) == NULL )
   {
      gan_err_register ( "gan_mat_multv_gen", GAN_ERROR_FAILURE, "" );
      return NULL;
   }

   /* compute matrix/vector product */
   if ( gan_blas_gemv ( y, 1.0, A, A_tr, x, 0.0 ) == NULL )
   {
      gan_err_register ( "gan_mat_multv_gen", GAN_ERROR_FAILURE, "" );
      return NULL;
   }
 ...
 */
Gan_Matrix *
 gan_mat_rmult_gen ( const Gan_Matrix *A, Gan_TposeFlag A_tr,
                     const Gan_Matrix *B, Gan_TposeFlag B_tr, Gan_Matrix *C )
{
   /* result matrix dimensions */
   unsigned long Crows = (A_tr ? A->cols : A->rows),
   ...
   gan_err_test_ptr ( (B_tr ? B->cols : B->rows) == common,
                      "gan_mat_rmult_gen", GAN_ERROR_INCOMPATIBLE, "");
   ...
   if ( C == NULL || gan_mat_fill_zero_q ( C, C->rows, C->cols ) == NULL )
   {
      gan_err_register ( "gan_mat_rmult_gen", GAN_ERROR_FAILURE, "" );
      return NULL;
   }
   ...
   if ( !gan_dgemm ( A_tr, B_tr, Crows, Ccols, common, 1.0,
                     ...
                     B->data, (B->rows == 0) ? 1 : B->rows, 0.0,
                     C->data, (C->rows == 0) ? 1 : C->rows ) )
   {
      gan_err_flush_trace();
      gan_err_register ( "gan_mat_rmult_gen", GAN_ERROR_CBLAS_FAILED, "" );
      return NULL;
   }
#endif /* #ifdef HAVE_LAPACK */
 ...
 */
Gan_SquMatrix *
 gan_mat_rmult_sym_gen ( const Gan_Matrix *A, Gan_TposeFlag A_tr,
                         ...
                         struct Gan_SquMatrix *C )
{
   ...
   gan_err_test_ptr ( (B_tr ? B->cols : B->rows) == common &&
                      ...
                      "gan_mat_rmult_sym_gen", GAN_ERROR_INCOMPATIBLE, "" );
   ...
   if ( C == NULL )
   {
      gan_err_register ( "gan_mat_rmult_sym_gen", GAN_ERROR_FAILURE, "" );
      return NULL;
   }
 ...
 */
Gan_Matrix *
 gan_mat_insert_gen ( Gan_Matrix *A, unsigned long rA, unsigned long cA,
                      const Gan_Matrix *B, Gan_TposeFlag B_tr,
 ...
 */
Gan_Matrix *
 gan_mat_insertv_gen ( Gan_Matrix *A, unsigned long rA, unsigned long cA,
                       const Gan_Vector *x, Gan_TposeFlag x_tr, unsigned long rx,
                       unsigned long size )
{
   long i;
 ...
 */
Gan_Matrix *
 gan_mat_insertsym ( Gan_Matrix    *A, unsigned long rA, unsigned long cA,
                     const Gan_SquMatrix *B, unsigned long rB, unsigned long cB,
                     unsigned long rows , unsigned long cols )
{
   long i, j;
   ...
   if ( rB < cB )
   {
      ...
      gan_err_test_ptr ( rB + rows <= cB, "gan_mat_insertsym",
                         GAN_ERROR_INCOMPATIBLE, "input matrices" );
   ...
   else if ( rB > cB )
   {
      ...
      /* make sure region doesn't cross source matrix diagonal */
      gan_err_test_ptr ( cB + cols <= rB, "gan_mat_insertsym",
                         GAN_ERROR_INCOMPATIBLE, "input matrices" );
   ...
   else gan_assert ( 0,  "illegal region in gan_mat_insertsym()" );
...
/* not a user function */
Gan_Matrix *
 gan_mat_form_gen ( Gan_Matrix *A, unsigned long rows, unsigned long cols,
                    double *data, size_t data_size )
{
   if ( A == NULL )
   {
      /* dyamically allocate matrix */
      A = gan_malloc_object(Gan_Matrix);
      if ( A == NULL )
      {
         gan_err_flush_trace();
         gan_err_register_with_number ( "gan_mat_form_gen", GAN_ERROR_MALLOC_FAILED, "", sizeof(*A) );
         return NULL;
      }
   ...
   if ( data == NULL )
   {
      ...
      else
      {
         ...
         gan_err_test_ptr ( data_size >= rows*cols, "gan_mat_form_gen",
      ...
      else
      {
         ...
         {
            gan_err_flush_trace();
            gan_err_register_with_number ( "gan_mat_form_gen", GAN_ERROR_MALLOC_FAILED, "", A->data_size*sizeof(double) );
            return NULL;
         }
      }
 ...
 * \sa gan_mat_extract_s(), gan_mat_insert().
 */
Gan_Matrix *
 gan_mat_extract_q ( const Gan_Matrix *A,
                     unsigned long r0,   unsigned long c0,
                     unsigned long rows, unsigned long cols,
                     Gan_Matrix *B )
{
   gan_err_test_ptr ( A->rows >= r0+rows && A->cols >= c0+cols,
                      "gan_mat_extract_q", GAN_ERROR_INCOMPATIBLE,
                      "input matrix not big enough" );

   /* allocate/set output matrix */
   if ( B == NULL )
      B = gan_mat_alloc ( rows, cols );
   else
      B = gan_mat_set_dims ( B, rows, cols );

   if ( B == NULL )
   {
      gan_err_register ( "gan_mat_extract_q", GAN_ERROR_FAILURE, "" );
      return NULL;
   }

   if ( gan_mat_insert ( B, 0, 0, A, r0, c0, rows, cols ) == NULL )
   {
      gan_err_register ( "gan_mat_extract_q", GAN_ERROR_FAILURE, "" );
      return NULL;
   }
 ...
 */
Gan_Bool
 gan_mat_fprint ( FILE *fp, const Gan_Matrix *A, const char *prefix,
 ...
 */
Gan_Matrix *
 gan_mat_fscanf_q ( FILE *fp, Gan_Matrix *A, char *prefix, int prefix_len )
{
   int ch;
   ...
   {
      gan_err_flush_trace();
      gan_err_register ( "gan_mat_fscanf_q", GAN_ERROR_CORRUPTED_FILE, "" );
      return NULL;
   }
   ...
   gan_err_test_ptr ( ch == '(', "gan_mat_fscanf_q",
   ...
   if ( A == NULL )
   {
      gan_err_register ( "gan_mat_fscanf_q", GAN_ERROR_FAILURE, "" );
      return NULL;
   }
   ...
   if ( A->cols > 0 )
      ...
      {
         for ( j = 0; j < A->cols; j++ )
            if ( fscanf ( fp, "%lf", &A->data[j*A->rows+i] ) != 1 )
            {
               gan_err_flush_trace();
               gan_err_register ( "gan_mat_fscanf_q", GAN_ERROR_CORRUPTED_FILE,
                                  "" );
 ...
 */
Gan_Bool
 gan_mat_fwrite ( FILE *fp, const Gan_Matrix *A, gan_uint32 magic_number )
{
   if ( !gan_fwrite_lendian_ui32 ( fp, &magic_number, 1 ) ||
        !gan_fwrite_lendian_ul ( fp, &A->rows, 1 ) ||
        !gan_fwrite_lendian_ul ( fp, &A->cols, 1 ) ||
        !gan_fwrite_lendian_f64 ( fp, A->data, A->rows*A->cols ) )
   {
      gan_err_register ( "gan_mat_fwrite", GAN_ERROR_FAILURE, "" );
      return GAN_FALSE;
   }
 ...
 */
Gan_Matrix *
 gan_mat_fread_q ( FILE *fp, Gan_Matrix *A, gan_uint32 *magic_number )
{
   ...
   /* read size of matrix */
   if ( !gan_fread_lendian_l ( fp, &rows, 1 ) ||
        !gan_fread_lendian_l ( fp, &cols, 1 ) )
   {
      gan_err_register ( "gan_mat_fread_q", GAN_ERROR_CORRUPTED_FILE, "" );
      return NULL;
   }
   ...
   if ( A == NULL )
   {
      gan_err_register ( "gan_mat_fread_q", GAN_ERROR_FAILURE, "" );
      return NULL;
   }

   /* read matrix data */
   if ( !gan_fread_lendian_f64 ( fp, A->data, A->rows*A->cols ) )
   {
      gan_err_register ( "gan_mat_fread_q", GAN_ERROR_CORRUPTED_FILE, "" );
      return NULL;
   }
...
#ifndef NDEBUG
/* for use in a debugger: use gan_mat_print() or gan_mat_fprint in
 * your program!
 */
