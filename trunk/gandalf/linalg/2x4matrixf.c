/**
 * File:          $RCSfile: 2x4matrixf.c,v $
 * Module:        2x4 matrices (single precision)
 * Part of:       Gandalf Library
 *
 * Revision:      $Revision: 1.9 $
 * Last edited:   $Date: 2004/03/18 18:26:16 $
 * Author:        $Author: pm $
 ...
 */
Gan_Bool
 gan_mat24f_fprint ( FILE *fp, const Gan_Matrix24_f *A,
 ...
 */
Gan_Bool
 gan_mat24f_fscanf ( FILE *fp, Gan_Matrix24_f *A,
                     char *prefix, int prefix_len )
{
   ...
   {
      gan_err_flush_trace();
      gan_err_register ( "gan_mat24f_fscanf", GAN_ERROR_CORRUPTED_FILE, "" );
      return GAN_FALSE;
   }
   ...
   result = 0;
   result += fscanf ( fp, "\n%f%f%f%f\n", &A->xx, &A->xy, &A->xz, &A->xw );
   result += fscanf ( fp, "%f%f%f%f",     &A->yx, &A->yy, &A->yz, &A->yw );
   gan_err_test_bool ( result == 8, "gan_mat24f_fscanf",
                       GAN_ERROR_CORRUPTED_FILE, "" );
 ...
 */
Gan_Matrix24_f
 gan_mat24f_fill_s ( float XX, float XY, float XZ, float XW,
                     float YX, float YY, float YZ, float YW )
{
   Gan_Matrix24_f A;

   (void)gan_mat24f_fill_q ( &A, XX, XY, XZ, XW, YX, YY, YZ, YW );
   return A;
}
 ...
 */
Gan_Vector2_f
 gan_mat24f_multv3h_s ( const Gan_Matrix24_f *A, const Gan_Vector3_f *p, float h )
{
   Gan_Vector2_f q;

   (void)gan_mat24f_multv3h_q ( A, p, h, &q );
   return q;
}
...
{
   gan_mat24f_get_cols_q ( A, p, q, r, s );
}
 ...
 */
Gan_Matrix24_f *
 gan_mat24f_from_matf_q ( const Gan_Matrix_f *A, Gan_Matrix24_f *B )
{
   if ( !gan_matf_read_va ( A, 2, 4, &B->xx, &B->xy, &B->xz, &B->xw,
                                     &B->yx, &B->yy, &B->yz, &B->yw ) )
   {
      gan_err_register ( "gan_mat24f_from_matf_q", GAN_ERROR_FAILURE, "" );
      return NULL;
   }
