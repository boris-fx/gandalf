...
static Gan_Bool
 scaledI_matrix_set_el ( Gan_SquMatrix_f *A,
...
static float
 scaledI_matrix_get_el ( const Gan_SquMatrix_f *A, unsigned i, unsigned j )
{
   /* consistency check */
...
static Gan_Bool
 scaledI_matrix_inc_el ( Gan_SquMatrix_f *A,
                         unsigned i, unsigned j, float value )
{
...
static Gan_Bool
 scaledI_matrix_dec_el ( Gan_SquMatrix_f *A,
                         unsigned i, unsigned j, float value )
{
...
static Gan_Bool
 scaledI_matrix_fprint ( FILE *fp, const Gan_SquMatrix_f *A, const char *prefix,
...
static Gan_SquMatrix_f *
 scaledI_matrix_copy ( const Gan_SquMatrix_f *A, Gan_SquMatrix_f *B )
{
   /* consistency check */
   assert ( A->type == GAN_SCALED_IDENT_MATRIX );
   ...
   if ( B == NULL )
      B = gan_scalImatf_alloc ( A->size );
   else
      B = gan_scalImatf_set_size ( B, A->size );
...
static Gan_SquMatrix_f *
 scaledI_matrix_scale ( Gan_SquMatrix_f *A, float a, Gan_SquMatrix_f *B )
{
   /* consistency check */
   assert ( A->type == GAN_SCALED_IDENT_MATRIX );
 ...
 * returning C */
static Gan_SquMatrix_f *
 scaledI_matrix_add ( Gan_SquMatrix_f *A, Gan_TposeFlag A_tr,
                      Gan_SquMatrix_f *B, Gan_TposeFlag B_tr, Gan_SquMatrix_f *C )
{
   /* consistency check */
   assert ( A->type == GAN_SCALED_IDENT_MATRIX );
   ...
   /* check that the type and dimensions of input matrices A & B are
      compatible */
   gan_assert ( gan_squmatf_same_type_size(A,B),
                "incompatible matrices in gan_scalImatf_add()" );
 ...
 */
static Gan_SquMatrix_f *
 scaledI_matrix_sub ( Gan_SquMatrix_f *A, Gan_TposeFlag A_tr,
                      Gan_SquMatrix_f *B, Gan_TposeFlag B_tr, Gan_SquMatrix_f *C )
{
   /* consistency check */
   assert ( A->type == GAN_SCALED_IDENT_MATRIX );
   ...
   /* check that the type and dimensions of input matrices A & B are
      compatible */
   gan_assert ( gan_squmatf_same_size(A,B),
...
/* function to multipy multiple of identity A and generic matrix B: C = A*B(^T),
 * returning C
 */
static Gan_Matrix_f *
     ...
     Gan_Matrix_f    *B, Gan_TposeFlag B_tr, Gan_Matrix_f *C )
{
   ...
   if ( B_tr )
   {
      ...
      if ( A_inv )
      {
         ...
         gan_err_test_ptr ( Aval != 0.0F, "scaledI_matrix_rmult",
...
/* function to multipy multiple of identity A and generic matrix B: C = B(^T)*A,
 * returning C
 */
static Gan_Matrix_f *
     ...
     Gan_Matrix_f          *B, Gan_TposeFlag B_tr, Gan_Matrix_f *C )
{
   ...
   if ( B_tr )
   {
      ...
      {
         /* multiply matrix data C = A^-1*B */
         gan_err_test_ptr ( Aval != 0.0F, "scaledI_matrix_lmult",
                            GAN_ERROR_DIVISION_BY_ZERO, "" );
         for ( j = 0, Cptr = C->data; j < C->cols; j++ )
            for ( i = 0; i < C->rows; i++ )
               *Cptr++ = B->data[i*B->rows+j]/Aval;
     ...
     Gan_SquMatrix_f *C )
{
   ...
   if ( A == B && A_inv == B_inv )
   {
      ...
      {
         /* compute A^-T * A^-1 */
         gan_err_test_ptr ( C->data[0] != 0.0F, "scaledI_matrix_rmult_squ",
                            GAN_ERROR_DIVISION_BY_ZERO, "" );
         C->data[0] = 1.0F/gan_sqr(C->data[0]);
      }
      else
         /* compute A^T * A */
 ...
 */
static Gan_SquMatrix_f *
 scaledI_matrix_lrmult ( const Gan_SquMatrix_f *A,
                         ...
                         Gan_Matrix_f *C, Gan_SquMatrix_f *D )
{
   ...
   A->funcs->rmult ( A, GAN_NOTRANSPOSE, GAN_NOINVERT,
                     (Gan_Matrix_f *)B, (Gan_TposeFlag) !B_tr, C );
 ...
 */
static Gan_SquMatrix_f *
 scaledI_matrix_invert ( Gan_SquMatrix_f *A, Gan_SquMatrix_f *B )
{
   assert ( A->type == GAN_SCALED_IDENT_MATRIX );
 ...
 */
static Gan_SquMatrix_f *
 scaledI_matrix_cholesky ( Gan_SquMatrix_f *A, Gan_SquMatrix_f *B,
                           int *error_code )
{
   assert ( A->type == GAN_SCALED_IDENT_MATRIX );
 ...
 */
static float
 scaledI_matrix_sumsqr ( const Gan_SquMatrix_f *A )
{
   return ((float)A->size)*gan_sqr(A->data[0]);
}

/* Compute Frobenius norm of multiple of identity matrix A.
 */
static float
 scaledI_matrix_Fnorm ( const Gan_SquMatrix_f *A )
{
   return (float)sqrt(((double)A->size)*(double)gan_sqr(A->data[0]));
}

/* Convert multiple of identity to generic matrix */
static Gan_Matrix_f *
 scaledI_matrix_matrix ( const Gan_SquMatrix_f *A, struct Gan_Matrix_f *B )
{
   long i;
   float Aval = A->data[0];
 ...
 */
Gan_SquMatrix_f *
 gan_scalImatf_set_size ( Gan_SquMatrix_f *A, unsigned long size )
{
   /* set type of matrix */
   ...
   if ( A->data_size < 1 )
   {
      ...
      gan_assert ( A->data_alloc,
                   "cannot reallocate matrix in gan_scalImatf_set_size()" );
      A->data_size = 1;
      A->data = gan_realloc_array ( float, A->data, A->data_size );
      if ( A->data == NULL )
      {
         gan_err_flush_trace();
         gan_err_register_with_number ( "gan_scalImatf_set_size", GAN_ERROR_MALLOC_FAILED, "", A->data_size*sizeof(float) );
         return NULL;
      }
   }
 ...
 */
Gan_SquMatrix_f *
 gan_scalImatf_fill_va ( Gan_SquMatrix_f *A, unsigned long size, ... )
{
   va_list ap;

   if ( A == NULL )
      A = gan_scalImatf_alloc ( size );
   else
      A = gan_scalImatf_set_size ( A, size );

   if ( A == NULL )
   {
      gan_err_register ( "gan_scalImatf_fill_va", GAN_ERROR_FAILURE, "" );
      return NULL;
   }

   /* read variable argument list containing diagonal element */
   va_start ( ap, size );
   A->data[0] = (float)va_arg ( ap, double );
   va_end ( ap );
   return A;
}
 ...
 */
Gan_SquMatrix_f *
 gan_scalImatf_fill_const_q ( Gan_SquMatrix_f *A, unsigned long size,
                              float value )
{
   if ( A == NULL )
      A = gan_scalImatf_alloc ( size );
   ...
   if ( A == NULL )
   {
      gan_err_register ( "gan_scalImatf_fill_const_q", GAN_ERROR_FAILURE, "" );
      return NULL;
   }
...
/* not a user function */
Gan_SquMatrix_f *
 gan_scalImatf_form_gen ( Gan_SquMatrix_f *A, unsigned long size,
                          float *data, size_t data_size )
{
   ...
   if ( data == NULL )
   {
      ...
      else
      {
         ...
         {
            gan_err_flush_trace();
            gan_err_register_with_number ( "gan_scalImatf_form_gen", GAN_ERROR_MALLOC_FAILED, "", A->data_size*sizeof(float) );
            return NULL;
         }
      }
   ...
   else
   {
      ...
      gan_err_test_ptr ( data_size >= 1, "gan_scalImatf_form_gen",
                         GAN_ERROR_ILLEGAL_ARGUMENT, "" );
 ...
 */
Gan_SquMatrix_f *
 gan_scalImatf_fill_vap ( Gan_SquMatrix_f *A,
                          unsigned long size, va_list *aptr )
{
   A = gan_scalImatf_set_size ( A, size );
   if ( A == NULL )
   {
      gan_err_register ( "gan_scalImatf_fill_vap", GAN_ERROR_FAILURE, "" );
      return NULL;
   }

   /* read variable argument list containing diagonal element */
   A->data[0] = (float)va_arg ( *aptr, double );
