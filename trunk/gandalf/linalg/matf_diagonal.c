/**
 * File:          $RCSfile: matf_diagonal.c,v $
...
static Gan_Bool
 diagonal_matrix_set_el ( Gan_SquMatrix_f *A,
...
static float
 diagonal_matrix_get_el ( const Gan_SquMatrix_f *A, unsigned i, unsigned j )
{
   /* consistency check */
   assert ( A->type == GAN_DIAGONAL_MATRIX );
...
static Gan_Bool
 diagonal_matrix_inc_el ( Gan_SquMatrix_f *A,
                          unsigned i, unsigned j, float value )
{
   /* consistency check */
   assert ( A->type == GAN_DIAGONAL_MATRIX );
...
static Gan_Bool
 diagonal_matrix_dec_el ( Gan_SquMatrix_f *A,
                          unsigned i, unsigned j, float value )
{
   /* consistency check */
   assert ( A->type == GAN_DIAGONAL_MATRIX );
...
static Gan_Bool
 diagonal_matrix_fprint ( FILE *fp, const Gan_SquMatrix_f *A, const char *prefix,
...
static Gan_SquMatrix_f *
 diagonal_matrix_copy ( const Gan_SquMatrix_f *A, Gan_SquMatrix_f *B )
{
#ifdef HAVE_LAPACK
   long onei = 1;
#endif
   ...
   if ( B == NULL )
      B = gan_diagmatf_alloc ( A->size );
   else
      B = gan_diagmatf_set_size ( B, A->size );
...
static Gan_SquMatrix_f *
 diagonal_matrix_scale ( Gan_SquMatrix_f *A, float a, Gan_SquMatrix_f *B )
{
#ifdef HAVE_LAPACK
   long onei = 1;
#endif
...
static Gan_SquMatrix_f *
 diagonal_matrix_add ( Gan_SquMatrix_f *A, Gan_TposeFlag A_tr,
                       ...
                       Gan_SquMatrix_f *C )
{
   ...
   /* check that the type and dimensions of input matrices A & B are
      compatible */
   gan_err_test_ptr ( gan_squmatf_same_type_size(A,B),
   ...
   if ( C == A )
      /* in-place operation A += B */
      gan_saxpy ( A->size, 1.0, B->data, 1, A->data, 1 );
   else if ( C == B )
      /* in-place operation B += A */
      gan_saxpy ( A->size, 1.0, A->data, 1, B->data, 1 );
   else
   {
      /* C = A + B */
      gan_scopy ( A->size, A->data, 1, C->data, 1 );
      gan_saxpy ( A->size, 1.0, B->data, 1, C->data, 1 );
   }
#endif /* #ifdef HAVE_LAPACK */
 ...
 */
static Gan_SquMatrix_f *
 diagonal_matrix_sub ( Gan_SquMatrix_f *A, Gan_TposeFlag A_tr,
                       Gan_SquMatrix_f *B, Gan_TposeFlag B_tr,
                       Gan_SquMatrix_f *C )
{
#ifdef HAVE_LAPACK
   ...
   /* check that the type and dimensions of input matrices A & B are
      compatible */
   gan_err_test_ptr ( gan_squmatf_same_size(A,B), "diagonal_matrix_sub",
     ...
     Gan_SquMatrix_f *C )
{
   ...
   if ( A == B && A_inv == B_inv )
   {
      if ( A != C )
         /* copy A into C */
         if ( gan_squmatf_copy_q ( A, C ) == NULL )
         {
            gan_err_register ( "diagonal_matrix_rmult_squ", GAN_ERROR_FAILURE,
                               "" );
            return NULL;
      ...
      if ( A_inv )
      {
         ...
         for ( i = C->size-1; i >= 0; i-- )
         {
            ...
            C->data[i] = 1.0F/gan_sqr(C->data[i]);
         }
      }
      else
         /* compute A^T * A */
   ...
   gan_err_flush_trace();
   gan_err_register ( "diagonal_matrix_rmult_squ", GAN_ERROR_NOT_IMPLEMENTED,
                      "" );
   return NULL;
}
 ...
 */
static Gan_SquMatrix_f *
 diagonal_matrix_lrmult ( const Gan_SquMatrix_f *A,
                          ...
                          Gan_Matrix_f *C, Gan_SquMatrix_f *D )
{
   ...
   gan_matf_rmult_sym_gen ( B, B_tr, C, GAN_NOTRANSPOSE, D );
 ...
 */
static Gan_SquMatrix_f *
 diagonal_matrix_invert ( Gan_SquMatrix_f *A, Gan_SquMatrix_f *B )
{
   long i;
 ...
 */
static Gan_SquMatrix_f *
 diagonal_matrix_cholesky ( Gan_SquMatrix_f *A, Gan_SquMatrix_f *B,
                            int *error_code )
{
   long i;
 ...
 */
static float
 diagonal_matrix_sumsqr ( const Gan_SquMatrix_f *A )
{
   long i;
   float total = 0.0F;
 ...
 */
static float
 diagonal_matrix_Fnorm ( const Gan_SquMatrix_f *A )
{
   long i;
   float total = 0.0F;
...
static Gan_Matrix_f *
 diagonal_matrix_matrix ( const Gan_SquMatrix_f *A, struct Gan_Matrix_f *B )
{
   long i;
   ...
   /* fill with zeros */
   gan_matf_fill_const_q ( B, A->size, A->size, 0.0F );
 ...
 */
Gan_SquMatrix_f *
 gan_diagmatf_set_size ( Gan_SquMatrix_f *A, unsigned long size )
{
   /* set type of matrix */
   ...
   if ( A->data_size < size )
   {
      ...
      gan_err_test_ptr ( A->data_alloc, "gan_diagmatf_set_size",
                         GAN_ERROR_INCOMPATIBLE, "" );
      A->data_size = size;
      ...
      {
         gan_err_flush_trace();
         gan_err_register_with_number ( "gan_diagmatf_set_size", GAN_ERROR_MALLOC_FAILED, "", A->data_size*sizeof(float) );
         return NULL;
      }
   }
 ...
 */
Gan_SquMatrix_f *
 gan_diagmatf_fill_va ( Gan_SquMatrix_f *A, unsigned long size, ... )
{
   unsigned int i;
   va_list ap;

   if ( A == NULL )
      A = gan_diagmatf_alloc ( size );
   else
      A = gan_diagmatf_set_size ( A, size );

   if ( A == NULL )
   {
      gan_err_register ( "gan_diagmatf_fill_va", GAN_ERROR_FAILURE, "" );
      return NULL;
   }
   ...
   va_start ( ap, size );
   for ( i = 0; i < size; i++ )
      A->data[i] = (float)va_arg ( ap, double );
 ...
 */
Gan_SquMatrix_f *
 gan_diagmatf_fill_const_q ( Gan_SquMatrix_f *A, unsigned long size,
                            float value )
{
   int i;
 ...
 */
Gan_Bool
 gan_diagmatf_read_va ( const Gan_SquMatrix_f *A, unsigned long size, ... )
{
   unsigned int i;
   va_list ap;
   ...
   gan_err_test_bool ( A->type == GAN_DIAGONAL_MATRIX && size == A->size,
                       "gan_diagmatf_read_va", GAN_ERROR_INCOMPATIBLE, "" );

   /* fill variable argument list of diagonal matrix element pointers */
   va_start ( ap, size );
   for ( i = 0; i < size; i++ )
      *(va_arg ( ap, float * )) = A->data[i];
 ...
 */
Gan_SquMatrix_f *
 gan_diagmatf_diag_thres ( Gan_SquMatrix_f *A, unsigned null_space,
                          float thres, int *indexptr, float **elptr )
{
   int i, nparams;
   float *Adata;
 ...
 */
Gan_SquMatrix_f *
 gan_diagmatf_inv_diag_thres ( Gan_SquMatrix_f *A, unsigned null_space,
                                  float thres )
{
   int i, nparams;
   float *Adata;
 ...
 */
Gan_Matrix_f *
 gan_diagmatIf_lmult_inf_q ( const Gan_SquMatrix_f *A, Gan_Matrix_f *B,
                             Gan_Matrix_f *C )
{
   unsigned long i, j;
   float *Aptr = A->data, *Bptr = B->data, *Cptr = C->data;
...
/* not a user function */
Gan_SquMatrix_f *
 gan_diagmatf_form_gen ( Gan_SquMatrix_f *A, unsigned long size,
                        float *data, size_t data_size )
{
   ...
   if ( data == NULL )
   {
      ...
      else
      {
         ...
         {
            gan_err_flush_trace();
            gan_err_register_with_number ( "gan_diagmatf_form_gen", GAN_ERROR_MALLOC_FAILED, "", A->data_size*sizeof(float) );
            return NULL;
         }
      }
   ...
   else
   {
      ...
      gan_err_test_ptr ( data_size >= size, "gan_diagmatf_form_gen",
                         GAN_ERROR_INCOMPATIBLE, "" );
 ...
 */
Gan_SquMatrix_f *
 gan_diagmatf_fill_vap ( Gan_SquMatrix_f *A, unsigned long size, va_list *aptr )
{
   unsigned int i;
   ...
   if ( A == NULL )
   {
      gan_err_register ( "gan_diagmatf_fill_vap", GAN_ERROR_FAILURE, "" );
      return NULL;
   }

   /* read variable argument list of diagonal elements */
   for ( i = 0; i < size; i++ )
      A->data[i] = (float)va_arg ( *aptr, double );
