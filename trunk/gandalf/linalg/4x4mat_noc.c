/**
 * File:          $RCSfile: 4x4mat_noc.c,v $
 * Module:        4x4 matrices (double precision)
 * Part of:       Gandalf Library
 *
 * Revision:      $Revision: 1.22 $
 * Last edited:   $Date: 2004/03/18 18:26:21 $
 * Author:        $Author: pm $
 ...
 */
Gan_Bool
 gan_mat44_fprint ( FILE *fp, const Gan_Matrix44 *A,
 ...
 */
Gan_Bool
 gan_mat44_fscanf ( FILE *fp, Gan_Matrix44 *A, char *prefix, int prefix_len )
{
   int ch, result;
   ...
   {
      gan_err_flush_trace();
      gan_err_register ( "gan_mat44_fscanf", GAN_ERROR_CORRUPTED_FILE, "" );
      return GAN_FALSE;
   }
   ...
   gan_err_test_bool ( result == 16, "gan_mat44_fscanf",
                       GAN_ERROR_CORRUPTED_FILE, "" );
 ...
 */
Gan_Matrix44
 gan_mat44_fill_s ( double XX, double XY, double XZ, double XW,
                    ...
                    double WX, double WY, double WZ, double WW )
{
   ...
   (void)gan_mat44_fill_q ( &A, XX, XY, XZ, XW, YX, YY, YZ, YW,
                                ZX, ZY, ZZ, ZW, WX, WY, WZ, WW );
   return A;
}
 ...
 */
Gan_Matrix44 *
 gan_mat44_tpose_i ( Gan_Matrix44 *A )
{
   double tmp;
 ...
 */
Gan_Vector3 *
 gan_mat44_multv3h_q ( const Gan_Matrix44 *A, const Gan_Vector3 *p, double h,
                       Gan_Vector3 *q )
{
   Gan_Vector4 p4;

   p4 = gan_vec4_set_parts_s ( p, h );
   p4 = gan_mat44_multv4_s ( A, &p4 );
   gan_err_test_ptr ( p4.w != 0.0, "gan_mat44_multv3h_q",
                      GAN_ERROR_DIVISION_BY_ZERO, "" );
   gan_vec3_fill_q ( q, h*p4.x/p4.w, h*p4.y/p4.w, h*p4.z/p4.w );
   return q;
}
 ...
 */
Gan_Vector3
 gan_mat44_multv3h_s ( const Gan_Matrix44 *A, const Gan_Vector3 *p, double h )
{
   Gan_Vector3 q;

   if ( gan_mat44_multv3h_q ( A, p, h, &q ) == NULL )
      assert(0);
...
static void
 gan_mat44_LUdecomp_q ( const Gan_Matrix44 *A,
                        Gan_SquMatrix44 *L, Gan_SquMatrix44 *U )
{
#ifndef NDEBUG
   L->type = GAN_LOWER_TRI_MATRIX44;
   U->type = GAN_LOWER_TRI_MATRIX44;
#endif /* #ifndef NDEBUG */
   ...
   L->zx = A->zx;
   L->wx = A->wx;
   gan_assert ( L->xx != 0.0, "bad matrix in gan_mat44_LUdecomp_q()" );
   U->yx = A->xy/L->xx;
   U->zx = A->xz/L->xx;
   U->wx = A->xw/L->xx;
   L->yy = A->yy - L->yx*U->yx;
   gan_assert ( L->yy != 0.0, "bad matrix in gan_mat44_LUdecomp_q()" );
   U->zy = (A->yz - L->yx*U->zx)/L->yy;
   U->wy = (A->yw - L->yx*U->wx)/L->yy;
   L->zy = A->zy - L->zx*U->yx;
   L->zz = A->zz - L->zx*U->zx - L->zy*U->zy;
   gan_assert ( L->zz != 0.0, "bad matrix in gan_mat44_LUdecomp_q()" );
...
static void
 gan_ltmat44_rmultl44T_q ( const Gan_SquMatrix44 *L, Gan_SquMatrix44 *U,
                           Gan_Matrix44 *A )
{
#ifndef NDEBUG
   assert ( L->type == GAN_LOWER_TRI_MATRIX44 &&
            U->type == GAN_LOWER_TRI_MATRIX44 );
#endif /* #ifndef NDEBUG */
...
static void
 gan_ltmat44_lmultl44T_q ( const Gan_SquMatrix44 *L, Gan_SquMatrix44 *U,
                           Gan_Matrix44 *A )
{
#ifndef NDEBUG
   assert ( L->type == GAN_LOWER_TRI_MATRIX44 &&
 ...
 */
Gan_Matrix44 *
 gan_mat44_invert ( Gan_Matrix44 *A, Gan_Matrix44 *B, int *error_code )
{
   double detA = gan_mat44_det_q(A), detE, detF, detG, detH;
   Gan_Matrix22 E, F, G, H, EaT, FaT, GaT, HaT, EaTF, GaTH, tmp;

   gan_err_test_ptr ( A != B, "gan_mat44_invert_q", GAN_ERROR_ILLEGAL_ARGUMENT,
   ...
   if ( detA == 0.0 )
   {
      ...
      {
         gan_err_flush_trace();
         gan_err_register ( "gan_mat44_invert_q", GAN_ERROR_SINGULAR_MATRIX,
                            "" );
      }
      else
   ...
   /* get 2x2 blocks of A */
   gan_mat44_get_blocks_q ( A, &E, &F, &G, &H );

   /* compute determinant and adjoint transpose of each 2x2 block */
   detE = gan_mat22_det_q(&E); gan_mat22_adjointT_q ( &E, &EaT );
   detF = gan_mat22_det_q(&F); gan_mat22_adjointT_q ( &F, &FaT );
   detG = gan_mat22_det_q(&G); gan_mat22_adjointT_q ( &G, &GaT );
   detH = gan_mat22_det_q(&H); gan_mat22_adjointT_q ( &H, &HaT );

   /* compute E^*^T * F and G^*^T * H */
   (void)gan_mat22_rmultm22_q ( &EaT, &F, &EaTF );
   (void)gan_mat22_rmultm22_q ( &GaT, &H, &GaTH );

   /* compute top-left inverse block multiplied by det(A) */
   (void)gan_mat22_scale_q ( &EaT, detH, &E );
   (void)gan_mat22_rmultm22_q ( &GaTH, &FaT, &tmp );
   (void)gan_mat22_decrement ( &E, &tmp );

   /* compute top-right inverse block multiplied by det(A) */
   (void)gan_mat22_scale_q ( &GaT, detF, &F );
   (void)gan_mat22_rmultm22_q ( &EaTF, &HaT, &tmp );
   (void)gan_mat22_decrement ( &F, &tmp );

   /* compute bottom-left inverse block multiplied by det(A) */
   GaTH = gan_mat22_adjointT_s ( &GaTH );
   (void)gan_mat22_scale_q ( &FaT, detG, &G );
   (void)gan_mat22_rmultm22_q ( &GaTH, &EaT, &tmp );
   (void)gan_mat22_decrement ( &G, &tmp );

   /* compute bottom-right inverse block multiplied by det(A) */
   EaTF = gan_mat22_adjointT_s ( &EaTF );
   (void)gan_mat22_scale_q ( &HaT, detE, &H );
   (void)gan_mat22_rmultm22_q ( &EaTF, &GaT, &tmp );
   (void)gan_mat22_decrement ( &H, &tmp );

   /* build matrix and divide by det(A) to compete calculation of inverse */
   (void)gan_mat44_set_blocks_q ( B, &E, &F, &G, &H );
   (void)gan_mat44_divide_i ( B, detA );
#if 0
   {
      Gan_SquMatrix44 L, U;
      Gan_Matrix44 I;

      gan_mat44_rmultm44_q ( A, B, &I );
      gan_mat44_LUdecomp_q ( A, &L, &U );
      gan_ltmat44_invert_i ( &L );
      gan_ltmat44_invert_i ( &U );
      gan_ltmat44_lmultl44T_q ( &L, &U, B );
      gan_mat44_rmultm44_q ( A, B, &I );
   }
#endif
   return B;
 ...
 */
Gan_Matrix44 *
 gan_mat44_adjoint_q ( const Gan_Matrix44 *A, Gan_Matrix44 *B )
{
   gan_err_test_ptr ( A != B, "gan_mat44_adjoint_q",
 ...
 */
Gan_Matrix44 *
 gan_mat44_adjointT_q ( const Gan_Matrix44 *A, Gan_Matrix44 *B )
{
   gan_err_test_ptr ( A != B, "gan_mat44_adjointT_q",
                      GAN_ERROR_ILLEGAL_ARGUMENT, "can't do adjoint in place");
   gan_assert ( 0, "not implemented" );
   return B;
}
 ...
 */
void
 gan_mat44_get_rows_s ( const Gan_Matrix44 *A, Gan_Vector4 *p, Gan_Vector4 *q,
                                               Gan_Vector4 *r, Gan_Vector4 *s )
{
   gan_mat44_get_rows_q ( A, p, q, r, s );
}
 ...
 */
void
 gan_mat44_get_cols_s ( const Gan_Matrix44 *A, Gan_Vector4 *p, Gan_Vector4 *q,
                                               Gan_Vector4 *r, Gan_Vector4 *s )
{
   gan_mat44_get_cols_q ( A, p, q, r, s );
}
 ...
 */
Gan_Matrix33
 gan_mat44_get_m33tl_s ( const Gan_Matrix44 *A )
{
   Gan_Matrix33 B;

   (void)gan_mat44_get_m33tl_q ( A, &B );
   return B;
}
 ...
 */
Gan_Vector3
 gan_mat44_get_v3tr_s ( const Gan_Matrix44 *A )
{
   Gan_Vector3 p;

   (void)gan_mat44_get_v3tr_q ( A, &p );
   return p;
}
 ...
 */
void
 gan_mat44_get_parts_s ( const Gan_Matrix44 *A, Gan_Matrix33 *B,
                         Gan_Vector3 *p, Gan_Vector3 *q, double *s )
{
   gan_mat44_get_parts_q ( A, B, p, q, s );
}
...
{
   gan_mat44_get_blocks_q ( M, A, B, C, D );
}
 ...
 */
Gan_Matrix44
 gan_mat44_set_parts_s ( Gan_Matrix33 *B,
                         const Gan_Vector3 *p, const Gan_Vector3 *q, double s )
{
   Gan_Matrix44 A;
   Gan_Vector3 zerov = { 0.0, 0.0, 0.0 };

   (void)gan_mat44_set_parts_q ( &A, B, (p == NULL) ? &zerov : p,
                                 (q == NULL) ? &zerov : q, s );
   return A;
}
 ...
 */
Gan_Matrix44
 gan_mat44_set_blocks_s ( const Gan_Matrix22 *A, const Gan_Matrix22 *B,
                          const Gan_Matrix22 *C, const Gan_Matrix22 *D )
{
   Gan_Matrix44 M;
   Gan_Matrix22 zerob = { 0.0, 0.0,
                          0.0, 0.0 };

   (void)gan_mat44_set_blocks_q ( &M,
 ...
 */
Gan_Matrix44 *
 gan_mat44_from_mat_q ( const Gan_Matrix *A, Gan_Matrix44 *B )
{
   if ( !gan_mat_read_va ( A, 4, 4, &B->xx, &B->xy, &B->xz, &B->xw,
                                    &B->yx, &B->yy, &B->yz, &B->yw,
                                    &B->zx, &B->zy, &B->zz, &B->zw,
                                    &B->wx, &B->wy, &B->wz, &B->ww ) )
   {
      gan_err_register ( "gan_mat44_from_mat_q", GAN_ERROR_FAILURE, "" );
      return NULL;
   }
 ...
 */
Gan_Matrix44 *
 gan_mat44_from_squmat_q ( const Gan_SquMatrix *A, Gan_Matrix44 *B )
{
   switch ( A->type )
   {
      case GAN_SCALED_IDENT_MATRIX:
        if ( !gan_scalImat_read_va ( A, 4, &B->xx ) )
        {
           gan_err_register ( "gan_mat44_from_squmat_q", GAN_ERROR_FAILURE,
                              "" );
           return NULL;
        }

        B->xy = B->yx = B->xz = B->zx = B->yz = B->zy = B->xw = B->wx =
        B->yw = B->wy = B->zw = B->wz = 0.0;
        B->ww = B->zz = B->yy = B->xx;
        break;

      case GAN_DIAGONAL_MATRIX:
        if ( !gan_diagmat_read_va ( A, 4, &B->xx, &B->yy, &B->zz, &B->ww ) )
        {
           gan_err_register ( "gan_mat44_from_squmat_q", GAN_ERROR_FAILURE,
                              "" );
           return NULL;
      ...
      case GAN_LOWER_TRI_MATRIX:
        ...
        B->xy = B->xz =B->yz = B->xw = B->yw = B->zw = 0.0;
        break;

      case GAN_UPPER_TRI_MATRIX:
        if ( !gan_utmat_read_va ( A, 4,
        ...
        B->yx = B->zx =B->zy = B->wx = B->wy = B->wz = 0.0;
        break;
 ...
 */
Gan_SquMatrix44 *
 gan_squmat44_from_squmat_q ( const Gan_SquMatrix *A, Gan_SquMatrix44 *B )
{
   switch ( A->type )
   {
      case GAN_SCALED_IDENT_MATRIX:
        ...
        if ( !gan_scalImat_read_va ( A, 4, &B->xx ) )
        {
           gan_err_register ( "gan_squmat44_from_squmat_q", GAN_ERROR_FAILURE,
                              "" );
           return NULL;
        }
