 ...
 * Last edited:   $Date: 2007/03/19 17:29:56 $
 * Author:        $Author: wouterk $
...
/// Blind declaration for a handle to image memory
/// returned by the image memory management routines
typedef struct ImageMemoryStruct * ImageMemoryHandle;

typedef int GanStatus;

/// Status code indicating all was fine
#define kGanStatOK (0)

/// Status error code for a failed operation
#define kGanStatFailed  (1)

/// Status error code for a fatal error
#define kGanStatErrFatal (2)

/// Status error code for an operation on or request for an unknown object
#define kGanStatErrUnknown (3)

/// Status error code indicating that something failed due to memory shortage
#define kGanStatErrMemory  (8)

/// Status error code for an operation on a bad handle
#define kGanStatErrBadHandle (9)

typedef struct Gan_Image * Gan_Image_Ptr;

/// A group of function pointers for memory allocation.
struct Gan_ImageHostSuite
{
   ...
   Memory handles allocated by this should be freed by Gan_ImageHostSuite::imageMemoryFree.

   \returns
   - kGanStatOK if all went well, a valid memory handle is placed in \e memoryHandle
   - kGanStatErrBadHandle if instanceHandle is not valid, memoryHandle is set to NULL
   - kGanStatErrMemory if there was not enough memory to satisfy the call, memoryHandle is set to NULL
   */
   GanStatus (*imageMemoryAlloc)(struct Gan_ImageHostSuite * suite, size_t nBytes, Gan_Image_Ptr);

   GanStatus (*imageMemoryRealloc)(struct Gan_ImageHostSuite * suite, size_t nBytes, Gan_Image_Ptr);
   GanStatus (*imageMemoryFree)(struct Gan_ImageHostSuite * suite, Gan_Image_Ptr);

   GanStatus (*imageMemoryLock)(struct Gan_ImageHostSuite * suite, Gan_Image_Ptr);
   GanStatus (*imageMemoryUnlock)(struct Gan_ImageHostSuite * suite, Gan_Image_Ptr);
};
...
typedef struct Gan_Image
{
   ...
   union
   {
      ...
      union
      {
         ...
         Gan_YUV422Pixel_ui16 **ui16; /* needs special treatment when iterating */
      } yuv422;
   } row_data;
   ...
   /// function to free image. Default is image_free in image_common_noc.c
   void (*image_free_func) ( struct Gan_Image *img );
   ...
   struct PixData
   {
      ...
      ImageMemoryHandle image_memory_handle;
   ...
   union
   {
      ...
      union
      {
         ...
         Gan_Bool (*ui16) ( struct Gan_Image *, unsigned row, unsigned col, Gan_YUVX444Pixel_ui16 *pix );
      } yuvx444;
      ...
      union
      {
         ...
         Gan_Bool (*ui16) ( struct Gan_Image *, unsigned row, unsigned col, Gan_YUVA444Pixel_ui16 *pix );
      } yuva444;
      ...
      union
      {
         ...
         Gan_Bool (*ui16) ( struct Gan_Image *, unsigned row, unsigned col, Gan_YUV422Pixel_ui16 *pix );
      } yuv422;
   } set_pix;
   ...
   union
   {
      ...
      union
      {
         ...
         Gan_YUVX444Pixel_ui16 (*ui16) ( const struct Gan_Image *, unsigned row, unsigned col );
      } yuvx444;
      ...
      union
      {
         ...
         Gan_YUVA444Pixel_ui16 (*ui16) ( const struct Gan_Image *, unsigned row, unsigned col );
      } yuva444;
      ...
      union
      {
         ...
         Gan_YUV422Pixel_ui16 (*ui16) ( const struct Gan_Image *, unsigned row, unsigned col );
      } yuv422;
   } get_pix;
   ...
   union
   {
      ...
      union
      {
         ...
         Gan_Bool (*ui16) ( struct Gan_Image *, Gan_YUVX444Pixel_ui16 *pix );
      } yuvx444;
      ...
      union
      {
         ...
         Gan_Bool (*ui16) ( struct Gan_Image *, Gan_YUVA444Pixel_ui16 *pix );
      } yuva444;
      ...
      union
      {
         ...
         Gan_Bool (*ui16) ( struct Gan_Image *, Gan_YUV422Pixel_ui16 *pix );
      } yuv422;
   } fill_const;
} Gan_Image;
 ...
 *
 * Allocates and returns a new image with given
 * \a format, \a type, \a height, \a width and \a stride.
 *
 * If stride is zero, then uses gan_image_min_stride(\a format, \a type, \a width, 0)
 * If stride is too small for the given width, the function returns NULL.
 *
 * If \a host_suite is null then this function is equivalent to calling gan_image_alloc.
 *
 */
...
#else
#define gan_image_free(img) ( ((img) && (img)->image_free_func) ? (img)->image_free_func(img) : (void)0 )
#endif
...
GANDALF_API Gan_Image *gan_image_set_format_type_dims ( Gan_Image *img,
                                                        ...
                                                        unsigned long width );
#else
#define gan_image_set_format_type_dims(img, f, t, h, w) \
   gan_image_set_format_type_dims_gen(img, f, t, h, w, gan_image_min_stride(f, t, w, 0), GAN_TRUE)
#endif
...
GANDALF_API unsigned long gan_image_min_stride(
   Gan_ImageFormat format, Gan_Type type,
...
GANDALF_API size_t gan_pix_data_size( Gan_ImageFormat format, Gan_Type type,
                                      unsigned long height, unsigned long width,
                                      Gan_Bool * overflow );

/* compute the required size of pixel data and optionally report arithmetic overflow */
GANDALF_API size_t gan_pix_data_size_with_stride( unsigned long height, unsigned long stride,
                                                  Gan_Bool *overflow );
...
GANDALF_API void            gan_image_union_subwindows_q ( const Gan_ImageWindow *window1, const Gan_ImageWindow *window2,
                                                           Gan_ImageWindow *resultwindow );
GANDALF_API Gan_ImageWindow gan_image_union_subwindows_s ( const Gan_ImageWindow *window1, const Gan_ImageWindow *window2 );
GANDALF_API void            gan_image_union_subwindows_i ( Gan_ImageWindow *sourcedestwindow, const Gan_ImageWindow *window );
...
GANDALF_API Gan_Bool gan_image_has_rgb_channels ( const Gan_Image *img );
...
GANDALF_API Gan_Image *gan_image_set_format_type_dims_gen(
   Gan_Image *img,
   Gan_ImageFormat format, Gan_Type type,
   unsigned long height, unsigned long width, unsigned long stride,
   Gan_Bool alloc_pix_data );
