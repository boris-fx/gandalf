...
#include <gandalf/image/image_yuvx444_uint16.h>
#include <gandalf/image/image_yuva444_uint8.h>
#include <gandalf/image/image_yuva444_uint16.h>
#include <gandalf/image/image_yuv422_uint8.h>
#include <gandalf/image/image_yuv422_uint16.h>
...
GANDALF_API void gan_image_struct_init(Gan_Image * image)
{
   ...
   image->stride = 0;
   image->premult = GAN_PREMULT_UNKNOWN;

   // How to set image->row_data?

   image->row_data_ptr = NULL;
   image->row_data_size = 0;
   image->ba = NULL;
   image->ba_alloc = GAN_FALSE;
   image->offset_x = image->offset_y = 0;
   image->row_data_alloc = GAN_FALSE;
   image->struct_alloc = GAN_FALSE;
   image->data_free_func = NULL;
   image->image_free_func = NULL;
   image->pix_data.ptr = NULL;
   image->pix_data.size = 0;
   image->pix_data.alloc = GAN_FALSE;
   image->pix_data.host_suite = NULL;
   image->pix_data.image_memory_handle = NULL;
   image->copy = NULL;
}
...
/**
 * \brief Returns a new image using the given \a host_suite for memory allocation.
 *
 * Allocates and returns a new image with given \a format, \a type, \a height and \a width.
 * If \host_suite is null then this function is equivalent to calling gan_image_alloc.
 *
 */
GANDALF_API Gan_Image *gan_image_alloc_suite(
   Gan_ImageFormat format, Gan_Type type,
   unsigned long height, unsigned long width, unsigned long stride,
   struct Gan_ImageHostSuite * host_suite)
{
   Gan_Image *img;

   if (!stride)
      stride = gan_image_min_stride(format, type, width, 0);
   else if (stride < gan_image_min_stride(format, type, width, 0))
      return NULL;
   ...
   img->image_free_func = NULL;

   gan_image_form_gen( img, format, type, height, width, stride,
                       GAN_TRUE, NULL, 0, NULL, 0 );
   // HACK. gan_image_form_gen sets struct_alloc to false!
   img->struct_alloc = GAN_TRUE;
   ...
   switch ( format )
   {
      ...
      case GAN_YUVX444:
        switch ( type )
        {
           ...
           case GAN_UINT16:
             img = gan_image_form_gen_yuvx444_ui16 ( img, height, width, stride,
                                                     alloc_pix_data,
                                                     (Gan_YUVX444Pixel_ui16 *)  pix_data, pix_data_size,
                                                     (Gan_YUVX444Pixel_ui16 **) row_data, row_data_size );
             break;
      ...
      case GAN_YUVA444:
        switch ( type )
        {
           ...
           case GAN_UINT16:
             img = gan_image_form_gen_yuva444_ui16 ( img, height, width, stride,
                                                     alloc_pix_data,
                                                     (Gan_YUVA444Pixel_ui16 *)  pix_data, pix_data_size,
                                                     (Gan_YUVA444Pixel_ui16 **) row_data, row_data_size );
             break;
      ...
      case GAN_YUV422:
        switch ( type )
        {
           ...
           case GAN_UINT16:
             img = gan_image_form_gen_yuv422_ui16 ( img, height, width, stride,
                                                    alloc_pix_data,
                                                    (Gan_YUV422Pixel_ui16 *)  pix_data, pix_data_size,
                                                    (Gan_YUV422Pixel_ui16 **) row_data, row_data_size );
             break;
                                ...
                                void (*data_free_func)(void *) )
{
   ...
   gan_err_test_bool ( !img->pix_data.alloc, "gan_image_set_data_free_func",
...
Gan_Bool gan_image_test_valid ( const Gan_Image *img )
{
   return ( img
      ...
      && img->pix_data.ptr != NULL && img->pix_data.size > 0
   ...
   switch ( format )
   {
      ...
      case GAN_YUVX444:
        switch ( type )
        {
           ...
           case GAN_UINT16: return sizeof(Gan_YUVX444Pixel_ui16);
           default:
             gan_err_flush_trace();
             gan_err_register ( "gan_image_pixel_size", GAN_ERROR_ILLEGAL_IMAGE_TYPE, "" );
             break;
      ...
      case GAN_YUVA444:
        switch ( type )
        {
           ...
           case GAN_UINT16: return sizeof(Gan_YUVA444Pixel_ui16);
           default:
             gan_err_flush_trace();
             gan_err_register ( "gan_image_pixel_size", GAN_ERROR_ILLEGAL_IMAGE_TYPE, "" );
             break;
      ...
      case GAN_YUV422:
        switch ( type )
        {
           ...
           case GAN_UINT16: return sizeof(Gan_YUV422Pixel_ui16);
           default:
             gan_err_flush_trace();
             gan_err_register ( "gan_image_pixel_size", GAN_ERROR_ILLEGAL_IMAGE_TYPE, "" );
             break;
 ...
 */
unsigned long gan_image_min_stride(
   Gan_ImageFormat format, Gan_Type type,
   unsigned long width, unsigned long alignment )
{
   switch ( format )
   {
      case GAN_GREY_LEVEL_IMAGE:
      ...
      case GAN_YUVX444:
        switch ( type )
        {
           ...
           case GAN_UINT16: width *= sizeof(Gan_YUVX444Pixel_ui16); break;
      ...
      case GAN_YUVA444:
        switch ( type )
        {
           ...
           case GAN_UINT16: width *= sizeof(Gan_YUVA444Pixel_ui16); break;
      ...
      case GAN_YUV422:
        switch ( type )
        {
           ...
           case GAN_UINT16: width *= sizeof(Gan_YUV422Pixel_ui16); break;
...
size_t gan_pix_data_size( Gan_ImageFormat format, Gan_Type type,
                          unsigned long height, unsigned long width,
                          Gan_Bool * overflow )
{
   unsigned long stride = gan_image_min_stride( format, type, width, 0 /*alignment*/ );
   return gan_pix_data_size_with_stride( height, stride, overflow );
}

size_t gan_pix_data_size_with_stride(
   unsigned long height, unsigned long stride, Gan_Bool *overflow )
{
   ...
   switch ( format )
   {
      ...
      case GAN_YUVX444:
        switch ( type )
        {
           ...
           case GAN_UINT16: return gan_image_set_gen_yuvx444_ui16 ( img, height, width, stride, alloc_pix_data );
      ...
      case GAN_YUVA444:
        switch ( type )
        {
           ...
           case GAN_UINT16: return gan_image_set_gen_yuva444_ui16 ( img, height, width, stride, alloc_pix_data );
      ...
      case GAN_YUV422:
        switch ( type )
        {
           ...
           case GAN_UINT16: return gan_image_set_gen_yuv422_ui16 ( img, height, width, stride, alloc_pix_data );
   ...
   switch ( img->format )
   {
      ...
      case GAN_YUVX444:
        switch ( img->type )
        {
           ...
           case GAN_UINT16: return gan_image_fill_const_yuvx444_ui16 ( img, &pixel->data.yuvx444.ui16 );
      ...
      case GAN_YUVA444:
        switch ( img->type )
        {
           ...
           case GAN_UINT16: return gan_image_fill_const_yuva444_ui16 ( img, &pixel->data.yuva444.ui16 );
      ...
      case GAN_YUV422:
        switch ( img->type )
        {
           ...
           case GAN_UINT16: return gan_image_fill_const_yuv422_ui16 ( img, &pixel->data.yuv422.ui16 );
   ...
   switch ( img->format )
   {
      ...
      case GAN_YUVX444:
        switch ( img->type )
        {
           ...
           case GAN_UINT16:
           {
              Gan_YUVX444Pixel_ui16 pix = { 0, 0, 0 };

              return gan_image_fill_const_yuvx444_ui16 ( img, &pix );
           }
      ...
      case GAN_YUVA444:
        switch ( img->type )
        {
           ...
           case GAN_UINT16:
           {
              Gan_YUVA444Pixel_ui16 pix = { 0, 0, 0 };

              return gan_image_fill_const_yuva444_ui16 ( img, &pix );
           }
      ...
      case GAN_YUV422:
        switch ( img->type )
        {
           ...
           case GAN_UINT16:
           {
              Gan_YUV422Pixel_ui16 pix = { 0, 0 };

              return gan_image_fill_const_yuv422_ui16 ( img, &pix );
           }
   ...
   switch ( img->format )
   {
      ...
      case GAN_YUVX444:
        switch ( img->type )
        {
           ...
           case GAN_UINT16:
             pixel.data.yuvx444.ui16 = gan_image_get_pix_yuvx444_ui16 ( img, row, col );
             break;
      ...
      case GAN_YUVA444:
        switch ( img->type )
        {
           ...
           case GAN_UINT16:
             pixel.data.yuva444.ui16 = gan_image_get_pix_yuva444_ui16 ( img, row, col );
             break;
      ...
      case GAN_YUV422:
        switch ( img->type )
        {
           ...
           case GAN_UINT16:
             pixel.data.yuv422.ui16 = gan_image_get_pix_yuv422_ui16 ( img, row, col );
             break;
   ...
   switch ( img->format )
   {
      ...
      case GAN_YUVX444:
        switch ( img->type )
        {
           ...
           case GAN_UINT16:
             result = gan_image_get_pixptr_yuvx444_ui16 ( img, row, col );
             break;
      ...
      case GAN_YUVA444:
        switch ( img->type )
        {
           ...
           case GAN_UINT16:
             result = gan_image_get_pixptr_yuva444_ui16 ( img, row, col );
             break;
      ...
      case GAN_YUV422:
        switch ( img->type )
        {
           ...
           case GAN_UINT16:
             result = gan_image_get_pixptr_yuv422_ui16 ( img, row, col );
             break;
   ...
   switch ( img->format )
   {
      ...
      case GAN_YUVX444:
        switch ( img->type )
        {
           ...
           case GAN_UINT16:
           {
              Gan_YUVX444Pixel_ui16 pix = gan_image_get_pix_yuvx444_ui16 ( img, row, col );
      ...
      case GAN_YUVA444:
        switch ( img->type )
        {
           ...
           case GAN_UINT16:
           {
              Gan_YUVA444Pixel_ui16 pix = gan_image_get_pix_yuva444_ui16 ( img, row, col );
      ...
      case GAN_YUV422:
        switch ( img->type )
        {
           ...
           case GAN_UINT16:
           {
              Gan_YUV422Pixel_ui16 pix = gan_image_get_pix_yuv422_ui16 ( img, row, col );
   ...
   switch ( img->format )
   {
      ...
      case GAN_YUVX444:
        switch ( img->type )
        {
           ...
           case GAN_UINT16: return gan_image_set_pix_yuvx444_ui16 ( img, row, col, &pix->data.yuvx444.ui16);
      ...
      case GAN_YUVA444:
        switch ( img->type )
        {
           ...
           case GAN_UINT16: return gan_image_set_pix_yuva444_ui16 ( img, row, col, &pix->data.yuva444.ui16);
      ...
      case GAN_YUV422:
        switch ( img->type )
        {
           ...
           case GAN_UINT16: return gan_image_set_pix_yuv422_ui16 ( img, row, col, &pix->data.yuv422.ui16);
      ...
      case GAN_YUVX444:
        switch ( img->type )
        {
           ...
           case GAN_UINT16:
           {
              ...
              return gan_image_set_pix_yuvx444_ui16 ( img, row, col, &pix );
           }
      ...
      case GAN_YUVA444:
        switch ( img->type )
        {
           ...
           case GAN_UINT16:
           {
              ...
              return gan_image_set_pix_yuva444_ui16 ( img, row, col, &pix );
           }
      ...
      case GAN_YUV422:
        switch ( img->type )
        {
           ...
           case GAN_UINT16:
           {
              ...
              return gan_image_set_pix_yuv422_ui16 ( img, row, col, &pix );
           }
...
Gan_ImageWindow gan_image_intersect_subwindows_s ( const Gan_ImageWindow *window1, const Gan_ImageWindow *window2 )
{
   ...
   {
      iwResult.c0 = gan_max2_i(window1->c0, window2->c0);
      iwResult.r0 = gan_max2_i(window1->r0, window2->r0);
      iwResult.width  = gan_min2_i(window1->c0+window1->width,  window2->c0+window2->width)  - iwResult.c0;
      iwResult.height = gan_min2_i(window1->r0+window1->height, window2->r0+window2->height) - iwResult.r0;
   }
...
/**
 * \brief Union subwindows.
 * \param window1 First input subwindow
 * \param window2 First input subwindow
 * \param resultwindow Result window
 *
 * Union subwindows, writing the union into \a resultwindow. If the union is empty,
 * \a resultwindow is filled with zeros.
 */
void gan_image_union_subwindows_q ( const Gan_ImageWindow *window1, const Gan_ImageWindow *window2,
                                    Gan_ImageWindow *resultwindow )
{
   *resultwindow = gan_image_union_subwindows_s(window1, window2);
}
 ...
 * \param window2 First input subwindow
 *
 * Union subwindows, returning the union as a new subwindow.
 * If the union is empty, the result is filled with zeros.
 */
Gan_ImageWindow gan_image_union_subwindows_s ( const Gan_ImageWindow *window1, const Gan_ImageWindow *window2 )
{
   Gan_ImageWindow iwResult;

   iwResult.c0 = gan_min2_i(window1->c0, window2->c0);
   iwResult.r0 = gan_min2_i(window1->r0, window2->r0);
   iwResult.width  = gan_max2_i(window1->c0+window1->width,  window2->c0+window2->width)  - iwResult.c0;
   iwResult.height = gan_max2_i(window1->r0+window1->height, window2->r0+window2->height) - iwResult.r0;
   return iwResult;
}

/**
 * \brief Union subwindows in-place.
 * \param sourcedestwindow Input/output subwindow
 * \param window Second input subwindow
 *
 * Union subwindows, writing the union in-place back into \a sourcedestwindow.
 * If the union is empty, \a sourcedestwindow is filled with zeros.
 */
void gan_image_union_subwindows_i ( Gan_ImageWindow *sourcedestwindow, const Gan_ImageWindow *window )
{
   *sourcedestwindow = gan_image_union_subwindows_s(sourcedestwindow, window);
}
   ...
   switch ( image->format )
   {
      ...
      case GAN_YUVX444:
        switch ( image->type )
        {
           ...
           case GAN_UINT16:
             return gan_image_mask_window_yuvx444_ui16 ( image, r0, c0, height, width );
      ...
      case GAN_YUVA444:
        switch ( image->type )
        {
           ...
           case GAN_UINT16:
             return gan_image_mask_window_yuva444_ui16 ( image, r0, c0, height, width );
      ...
      case GAN_YUV422:
        switch ( image->type )
        {
           ...
           case GAN_UINT16:
             return gan_image_mask_window_yuv422_ui16 ( image, r0, c0, height, width );
   ...
   switch ( image->format )
   {
      ...
      case GAN_YUVX444:
        switch ( image->type )
        {
           ...
           case GAN_UINT16:
             return gan_image_clear_window_yuvx444_ui16 ( image, r0, c0, height, width );
      ...
      case GAN_YUVA444:
        switch ( image->type )
        {
           ...
           case GAN_UINT16:
             return gan_image_clear_window_yuva444_ui16 ( image, r0, c0, height, width );
      ...
      case GAN_YUV422:
        switch ( image->type )
        {
           ...
           case GAN_UINT16:
             return gan_image_clear_window_yuv422_ui16 ( image, r0, c0, height, width );
 ...
 *
 * Handles \c NULL values of \a img and its \t image_free_func correctly.
 */
...
Gan_Bool gan_image_has_alpha_channel ( const Gan_Image *img )
{
   switch ( img->format )
   {
      case GAN_GREY_LEVEL_IMAGE:       return GAN_TRUE; break; /* can be interpreted as alpha */
      ...
      default:
        gan_err_flush_trace();
        gan_err_register ( "gan_image_has_alpha_channel", GAN_ERROR_ILLEGAL_IMAGE_FORMAT, "" );
        return GAN_FALSE;
        break;
   }
}
...
/**
 * \brief Returns value indicating whether an image has RGB channels
 * \param img Input image
 * \return #GAN_TRUE if the image has an alpha channel, #GAN_FALSE if it doesn't
 */
Gan_Bool gan_image_has_rgb_channels ( const Gan_Image *img )
{
   switch ( img->format )
   {
      case GAN_RGB_COLOUR_IMAGE:
