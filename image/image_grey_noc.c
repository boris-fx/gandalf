 ...
 * Last edited:   $Date: 2007/01/29 15:17:48 $
 * Author:        $Author: pm $
 ...
 image_get_pix ( const Gan_Image *img, unsigned row, unsigned col )
{
   ...
   gan_assert ( row < img->height && col < img->width, "illegal pixel position in image_get_pix()" );

   /* get pixel */
   return (gan_bit_array_get_bit ( &img->ba[row], col ) ? GAN_TRUE : GAN_FALSE);
}
 ...
 image_fill_const ( Gan_Image *img, Gan_Bool pix )
{
   ...
   /* consistency check */
   gan_err_test_bool ( img->format == GAN_PIXFMT && img->type == GAN_PIXTYPE, "image_fill_const", GAN_ERROR_INCOMPATIBLE, "image format/type" );
                                  ...
                                  Gan_ImageWindow *subwin )
{
   ...
   {
      gan_err_flush_trace();
      gan_err_register ( "GAN_IMAGE_GET_ACTIVE_SUBWINDOW", GAN_ERROR_ILLEGAL_ARGUMENT, "" );
      return GAN_FALSE;
   }
...
#error "Only 8-bit characters supported currently"
   ...
   gan_err_test_bool ( bit != INT_MAX, "GAN_IMAGE_GET_ACTIVE_SUBWINDOW", GAN_ERROR_FAILURE, "" );
...
#error "Only 8-bit characters supported currently"
   ...
   /* make sure we found a valid bit */
   gan_err_test_bool ( bit != INT_MIN, "GAN_IMAGE_GET_ACTIVE_SUBWINDOW", GAN_ERROR_FAILURE, "" );
