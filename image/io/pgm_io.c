 ...
 * Last edited:   $Date: 2006/03/15 14:09:07 $
 * Author:        $Author: pm $
 ...
 */
Gan_Image *
 gan_read_pgm_image(const Gan_UnicodeChar *filename, Gan_Image *image, const struct Gan_ImageReadControlStruct *ictrlstr, struct Gan_ImageHeaderStruct *header,
                    Gan_Bool (*abortRequested)(void*), void* abortObj)
{
   FILE *infile;
   Gan_Image *result;
   ...
   {
      gan_err_flush_trace();
      gan_err_register_unicode( "gan_read_pgm_image", GAN_ERROR_OPENING_FILE, filename );
      return NULL;
   }

   result = gan_read_pgm_image_stream(infile, image, ictrlstr, header, abortRequested, abortObj);
   fclose(infile);
   return result;
}
 ...
 */
Gan_Bool
 gan_write_pgm_image ( const Gan_UnicodeChar *filename, const Gan_Image *image, const struct Gan_ImageWriteControlStruct *octrlstr )
{
   if(filename == NULL)
   ...
   else
   {
      ...
      {
         gan_err_flush_trace();
         gan_err_register_unicode ( "gan_write_pgm_image", GAN_ERROR_OPENING_FILE, filename );
         return GAN_FALSE;
      }
