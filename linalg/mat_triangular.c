/**
 * File:          $RCSfile: mat_triangular.c,v $
...
static Gan_Bool
 lower_tri_matrix_set_el ( Gan_SquMatrix *L,
...
static double
 lower_tri_matrix_get_el ( const Gan_SquMatrix *L, unsigned i, unsigned j )
{
   /* consistency check */
   assert ( L->type == GAN_LOWER_TRI_MATRIX );
 ...
 * value */
static Gan_Bool
 lower_tri_matrix_inc_el ( Gan_SquMatrix *L,
                           unsigned i, unsigned j, double value )
{
   /* consistency check */
   assert ( L->type == GAN_LOWER_TRI_MATRIX );
 ...
 */
static Gan_Bool
 lower_tri_matrix_dec_el ( Gan_SquMatrix *L,
                           unsigned i, unsigned j, double value )
{
   /* consistency check */
   assert ( L->type == GAN_LOWER_TRI_MATRIX );
...
static Gan_Bool
 upper_tri_matrix_set_el ( Gan_SquMatrix *U,
                           unsigned i, unsigned j, double value )
{
   /* consistency check */
...
static double
 upper_tri_matrix_get_el ( const Gan_SquMatrix *U, unsigned i, unsigned j )
{
   /* consistency check */
   assert ( U->type == GAN_UPPER_TRI_MATRIX );
 ...
 * value */
static Gan_Bool
 upper_tri_matrix_inc_el ( Gan_SquMatrix *U,
                           unsigned i, unsigned j, double value )
{
   /* consistency check */
   assert ( U->type == GAN_UPPER_TRI_MATRIX );
 ...
 * value */
static Gan_Bool
 upper_tri_matrix_dec_el ( Gan_SquMatrix *U,
                           unsigned i, unsigned j, double value )
{
   /* consistency check */
   assert ( U->type == GAN_UPPER_TRI_MATRIX );
...
static Gan_Bool
 lower_tri_matrix_fprint ( FILE *fp, const Gan_SquMatrix *L, const char *prefix,
...
static Gan_Bool
 upper_tri_matrix_fprint ( FILE *fp, const Gan_SquMatrix *U, const char *prefix,
                           unsigned indent, const char *fmt )
{
   unsigned long i, j;
   int p;
...
static Gan_SquMatrix *
 lower_tri_matrix_copy ( const Gan_SquMatrix *A, Gan_SquMatrix *B )
{
   /* consistency check */
   ...
   if ( B == NULL )
      B = gan_ltmat_alloc ( A->size );
   else
      B = gan_ltmat_set_size ( B, A->size );
...
static Gan_SquMatrix *
 upper_tri_matrix_copy ( const Gan_SquMatrix *A, Gan_SquMatrix *B )
{
   /* consistency check */
   ...
   /* allocate matrix B if necessary */
   if ( B == NULL )
      B = gan_utmat_alloc ( A->size );
   else
      B = gan_utmat_set_size ( B, A->size );
...
static Gan_SquMatrix *
 lower_tri_matrix_scale ( Gan_SquMatrix *A, double a,
                          Gan_SquMatrix *B )
{
   /* consistency check */
   gan_err_test_ptr ( A->type == GAN_LOWER_TRI_MATRIX,
   ...
   if ( (A!=B &&!gan_dcopy ( A->size*(A->size+1)/2, A->data, 1, B->data, 1 )) ||
        !gan_dscal ( A->size*(A->size+1)/2, a, B->data, 1 ) )
   {
      gan_err_flush_trace();
      gan_err_register ( "lower_tri_matrix_scale", GAN_ERROR_FAILURE, "" );
      return NULL;
...
static Gan_SquMatrix *
 upper_tri_matrix_scale ( Gan_SquMatrix *A, double a,
                          Gan_SquMatrix *B )
{
   /* consistency check */
   gan_err_test_ptr ( A->type == GAN_UPPER_TRI_MATRIX,
...
static Gan_SquMatrix *
 lower_tri_matrix_add ( Gan_SquMatrix *A, Gan_TposeFlag A_tr,
                        Gan_SquMatrix *B, Gan_TposeFlag B_tr,
                        Gan_SquMatrix *C )
{
   /* consistency check */
   gan_err_test_ptr ( A->type == GAN_LOWER_TRI_MATRIX &&
   ...
   /* check that the type and dimensions of input matrices A & B are
      compatible */
   gan_err_test_ptr ( gan_squmat_same_size(A,B), "lower_tri_matrix_add",
...
#else /* !HAVE_LAPACK */
   {
      ...
      if ( C == A )
         /* in-place operation A += B */
         result = gan_daxpy ( A->size*(A->size+1)/2, 1.0, B->data, 1,
                              A->data, 1 );
      else if ( C == B )
         /* in-place operation B += A */
         result = gan_daxpy ( A->size*(A->size+1)/2, 1.0, A->data, 1,
                              B->data, 1 );
      else
      {
         /* C = A + B */
         result = gan_dcopy ( A->size*(A->size+1)/2, A->data, 1, C->data, 1 );
         result = (Gan_Bool) (result & gan_daxpy ( A->size*(A->size+1)/2, 1.0,
                                                   B->data, 1, C->data, 1 ));
      }
...
static Gan_SquMatrix *
 upper_tri_matrix_add ( Gan_SquMatrix *A, Gan_TposeFlag A_tr,
                        Gan_SquMatrix *B, Gan_TposeFlag B_tr,
                        Gan_SquMatrix *C )
{
   /* consistency check */
   ...
   /* check that the type and dimensions of input matrices A & B are
      compatible */
   gan_err_test_ptr ( gan_squmat_same_size(A,B), "upper_tri_matrix_add",
                      GAN_ERROR_INCOMPATIBLE, "" );
 ...
 */
static Gan_SquMatrix *
 lower_tri_matrix_sub ( Gan_SquMatrix *A, Gan_TposeFlag A_tr,
                        Gan_SquMatrix *B, Gan_TposeFlag B_tr,
                        Gan_SquMatrix *C )
{
   /* consistency check */
   ...
   /* check that the type and dimensions of input matrices A & B are
      compatible */
   gan_err_test_ptr ( gan_squmat_same_size(A,B), "lower_tri_matrix_sub",
                      GAN_ERROR_INCOMPATIBLE, "" );
...
#else /* !HAVE_LAPACK */
   {
      ...
      {
         /* in-place operation B = A - B */
         result = gan_dscal ( A->size*(A->size+1)/2, -1.0, B->data, 1 );
         result = (Gan_Bool) (result & gan_daxpy ( A->size*(A->size+1)/2, 1.0,
                                                   A->data, 1, B->data, 1 ));
      }
      else
 ...
 */
static Gan_SquMatrix *
 upper_tri_matrix_sub ( Gan_SquMatrix *A, Gan_TposeFlag A_tr,
                        Gan_SquMatrix *B, Gan_TposeFlag B_tr,
                        Gan_SquMatrix *C )
{
   /* consistency check */
   ...
   /* check that the type and dimensions of input matrices A & B are
      compatible */
   gan_err_test_ptr ( gan_squmat_same_size(A,B), "upper_tri_matrix_sub",
                      GAN_ERROR_INCOMPATIBLE, "" );
...
static Gan_Vector *
 lower_tri_matrix_multv (
     const Gan_SquMatrix *L, Gan_TposeFlag A_tr, Gan_InvertFlag A_inv,
     Gan_Vector *x, Gan_Vector *y )
{
   ...
   if ( y == NULL )
      y = gan_vec_alloc ( L->size );
   else
      y = gan_vec_set_size ( y, L->size );
...
#ifdef HAVE_LAPACK
   {
      ...
      if ( x != y ) BLAS_DCOPY ( (long *)&x->rows, x->data, &onei, y->data, &onei );
      BLAS_DTPSV ( "U", "T", "N", (long *)&y->rows, L->data, y->data, &onei );
   }
#else /* !HAVE_LAPACK */
   {
      /* compute matrix/vector product y = L^-1*x */
      if ( x != y ) gan_dcopy ( x->rows, x->data, 1, y->data, 1 );
      gan_dtpsv ( GAN_MATRIXPART_UPPER, GAN_TRANSPOSE, GAN_NOUNIT, y->rows, L->data,
                  y->data, 1 );
   }
#endif /* #ifdef HAVE_LAPACK */
   else if ( !A_inv )
   ...
   else /* A_tr && A_inv */
   {
      ...
      /* compute matrix/vector product y = L^-T*x */
      if ( x != y ) BLAS_DCOPY ( (long *)&x->rows, x->data, &onei, y->data, &onei);
      BLAS_DTPSV ( "U", "N", "N", (long *)&y->rows, L->data, y->data, &onei );
#else /* !HAVE_LAPACK */
      /* compute matrix/vector product y = L^-T*x */
      if ( x != y )
         gan_dcopy ( x->rows, x->data, 1, y->data, 1 );

      gan_dtpsv ( GAN_MATRIXPART_UPPER, GAN_NOTRANSPOSE, GAN_NOUNIT, y->rows, L->data,
                  y->data, 1 );
#endif /* #ifdef HAVE_LAPACK */
   }
...
static Gan_Vector *
 upper_tri_matrix_multv (
     const Gan_SquMatrix *U, Gan_TposeFlag A_tr, Gan_InvertFlag A_inv,
     Gan_Vector *x, Gan_Vector *y )
{
   double total;
   long i, j;
   ...
   /* allocate result vector y if necessary */
   if ( y == NULL )
      y = gan_vec_alloc ( U->size );
   else
      y = gan_vec_set_size ( y, U->size );
   ...
   else if ( !A_tr )
   {
      ...
      if ( x != y ) BLAS_DCOPY ( (long *)&x->rows, x->data, &onei, y->data, &onei);
      BLAS_DTPSV ( "U", "N", "N", (long *)&y->rows, U->data, y->data, &onei );
#else /* !HAVE_LAPACK */
      /* compute matrix/vector product y = U^-1*x */
      if ( x != y ) gan_dcopy ( x->rows, x->data, 1, y->data, 1 );
      gan_dtpsv ( GAN_MATRIXPART_UPPER, GAN_NOTRANSPOSE, GAN_NOUNIT, y->rows, U->data,
                  y->data, 1 );
#endif /* #ifdef HAVE_LAPACK */
   }
   else if ( !A_inv )
   ...
   else /* A_tr && A_inv */
   {
      ...
      if ( x != y ) BLAS_DCOPY ( (long *)&x->rows, x->data, &onei, y->data, &onei);
      BLAS_DTPSV ( "U", "T", "N", (long *)&y->rows, U->data, y->data, &onei );
#else /* !HAVE_LAPACK */
      /* compute matrix/vector product y = U^-T*x */
      if ( x != y ) gan_dcopy ( x->rows, x->data, 1, y->data, 1 );
      gan_dtpsv ( GAN_MATRIXPART_UPPER, GAN_TRANSPOSE, GAN_NOUNIT, y->rows, U->data,
                  y->data, 1 );
#endif /* #ifdef HAVE_LAPACK */
   }
...
static Gan_Matrix *
 lower_tri_matrix_rmult (
     const Gan_SquMatrix *L, Gan_TposeFlag L_tr, Gan_InvertFlag L_inv,
     Gan_Matrix          *B, Gan_TposeFlag B_tr, Gan_Matrix *C )
{
   ...
   /* allocate result matrix C if necessary */
   if ( C == NULL )
      C = gan_mat_alloc ( L->size, Ccols );
   else
      C = gan_mat_set_dims ( C, L->size, Ccols );
   ...
   if ( B_tr )
   {
      ...
      else if ( !L_tr )
      {
         ...
         for ( j = 0, Cptr = C->data; (unsigned long) j < C->cols;
               j++, Cptr += C->rows )
            BLAS_DTPSV ( "U", "T", "N", (long *)&C->rows, L->data, Cptr, &onei );
#else /* !HAVE_LAPACK */
         for ( j = 0, Cptr = C->data; (unsigned long) j < C->cols;
               j++, Cptr += C->rows )
            gan_dtpsv ( GAN_MATRIXPART_UPPER, GAN_TRANSPOSE, GAN_NOUNIT, C->rows, L->data,
                        Cptr, 1 );
#endif /* #ifdef HAVE_LAPACK */
      }
      else if ( !L_inv )
      ...
      else /* L_tr && L_inv */
      {
         ...
         for ( j = 0, Cptr = C->data; (unsigned long) j < C->cols;
               j++, Cptr += C->rows )
            BLAS_DTPSV ( "U", "N", "N", (long *)&C->rows, L->data, Cptr, &onei );
#else /* !HAVE_LAPACK */
         for ( j = 0, Cptr = C->data; (unsigned long) j < C->cols;
               j++, Cptr += C->rows )
            gan_dtpsv ( GAN_MATRIXPART_UPPER, GAN_NOTRANSPOSE, GAN_NOUNIT, C->rows,
                        L->data, Cptr, 1 );
#endif /* #ifdef HAVE_LAPACK */
      }
   }
...
#ifdef HAVE_LAPACK
         if ( B != C ) BLAS_DCOPY ( &nel, B->data, &onei, C->data, &onei );
         for ( j = 0, Cptr = C->data; (unsigned long) j < C->cols;
               j++, Cptr += C->rows )
            BLAS_DTPSV ( "U", "T", "N", (long *)&C->rows, L->data, Cptr, &onei );
#else /* !HAVE_LAPACK */
         if ( B != C )
            gan_dcopy ( L->size*Ccols, B->data, 1, C->data, 1 );
...
static Gan_Matrix *
 upper_tri_matrix_rmult (
     const Gan_SquMatrix *U, Gan_TposeFlag U_tr, Gan_InvertFlag U_inv,
     Gan_Matrix          *B, Gan_TposeFlag B_tr, Gan_Matrix *C )
{
   unsigned long Ccols = B_tr ? B->rows : B->cols;
   long     i, j, k;
   ...
   /* allocate result matrix C if necessary */
   if ( C == NULL )
      C = gan_mat_alloc ( U->size, Ccols );
   else
      C = gan_mat_set_dims ( C, U->size, Ccols );
   ...
   if ( B_tr )
   {
      ...
      else if ( !U_tr )
      {
         ...
         for ( j = 0, Cptr = C->data; (unsigned long) j < C->cols;
               j++, Cptr += C->rows )
            BLAS_DTPSV ( "U", "N", "N", (long *)&C->rows, U->data, Cptr, &onei );
#else /* !HAVE_LAPACK */
         for ( j = 0, Cptr = C->data; (unsigned long) j < C->cols;
               j++, Cptr += C->rows )
            gan_dtpsv ( GAN_MATRIXPART_UPPER, GAN_NOTRANSPOSE, GAN_NOUNIT, C->rows,
      ...
      else /* U_tr && U_inv */
      {
         ...
         for ( j = 0, Cptr = C->data; (unsigned long) j < C->cols;
               j++, Cptr += C->rows )
            BLAS_DTPSV ( "U", "T", "N", (long *)&C->rows, U->data, Cptr, &onei );
#else /* !HAVE_LAPACK */
         for ( j = 0, Cptr = C->data; (unsigned long) j < C->cols;
               j++, Cptr += C->rows )
            gan_dtpsv ( GAN_MATRIXPART_UPPER, GAN_TRANSPOSE, GAN_NOUNIT, C->rows,
                        U->data, Cptr, 1 );
#endif /* #ifdef HAVE_LAPACK */
      }
   }
...
#else /* !HAVE_LAPACK */
         if ( B != C )
            gan_dcopy ( U->size*Ccols, B->data, 1, C->data, 1 );
     ...
     Gan_Matrix          *B, Gan_TposeFlag B_tr, Gan_Matrix *C )
{
   ...
   /* allocate result matrix C if necessary */
   if ( C == NULL )
      C = gan_mat_alloc ( Crows, L->size );
   else
      C = gan_mat_set_dims ( C, Crows, L->size );
   ...
   if ( B_tr )
   {
      ...
      else if ( !L_tr )
      {
         ...
         for ( i = 0, Cptr = C->data; (unsigned long) i < C->rows;
               i++, Cptr++ )
            BLAS_DTPSV ( "U", "N", "N", (long *)&C->cols, L->data, Cptr, (long *)&C->rows );
#else /* !HAVE_LAPACK */
         for ( i = 0, Cptr = C->data; (unsigned long) i < C->rows;
               i++, Cptr++ )
            gan_dtpsv ( GAN_MATRIXPART_UPPER, GAN_NOTRANSPOSE, GAN_NOUNIT, C->cols,
                        L->data, Cptr, C->rows );
#endif /* #ifdef HAVE_LAPACK */
      }
      else if ( !L_inv )
      ...
      else /* L_tr && L_inv */
      {
         ...
         for ( i = 0, Cptr = C->data; (unsigned long) i < C->rows;
               i++, Cptr++ )
            BLAS_DTPSV ( "U", "T", "N", (long *)&C->cols, L->data, Cptr, (long *)&C->rows );
#else /* !HAVE_LAPACK */
         for ( i = 0, Cptr = C->data; (unsigned long) i < C->rows;
               i++, Cptr++ )
            gan_dtpsv ( GAN_MATRIXPART_UPPER, GAN_TRANSPOSE, GAN_NOUNIT, C->cols, L->data,
                        Cptr, C->rows );
#endif /* #ifdef HAVE_LAPACK */
      }
   }
      ...
      else if ( !L_tr )
      {
         ...
         for ( i = 0, Cptr = C->data; (unsigned long) i < C->rows;
            ...
            BLAS_DTPSV ( "U", "N", "N", (long *)&C->cols, L->data, Cptr, (long *)&C->rows );
#else /* !HAVE_LAPACK */
         if ( B != C ) gan_dcopy ( Crows*L->size, B->data, 1, C->data, 1 );
         for ( i = 0, Cptr = C->data; (unsigned long) i < C->rows;
               i++, Cptr++ )
            gan_dtpsv ( GAN_MATRIXPART_UPPER, GAN_NOTRANSPOSE, GAN_NOUNIT, C->cols,
                        L->data, Cptr, C->rows );
     ...
     Gan_Matrix          *B, Gan_TposeFlag B_tr, Gan_Matrix *C )
{
   ...
   /* allocate result matrix C if necessary */
   if ( C == NULL )
      C = gan_mat_alloc ( Crows, U->size );
   else
      C = gan_mat_set_dims ( C, Crows, U->size );
   ...
   if ( B_tr )
   {
      ...
      else if ( !U_tr )
      {
         ...
         for ( i = 0, Cptr = C->data; (unsigned long) i < C->rows;
               i++, Cptr++ )
            BLAS_DTPSV ( "U", "T", "N", (long *)&C->cols, U->data, Cptr, (long *)&C->rows );
#else /* !HAVE_LAPACK */
         for ( i = 0, Cptr = C->data; (unsigned long) i < C->rows;
               i++, Cptr++ )
            gan_dtpsv ( GAN_MATRIXPART_UPPER, GAN_TRANSPOSE, GAN_NOUNIT, C->cols, U->data,
                        Cptr, C->rows );
#endif /* #ifdef HAVE_LAPACK */
      }
      else if ( !U_inv )
      ...
      else /* U_tr && U_inv */
      {
         ...
         for ( i = 0, Cptr = C->data; (unsigned long) i < C->rows;
               i++, Cptr++ )
            BLAS_DTPSV ( "U", "N", "N", (long *)&C->cols, U->data, Cptr, (long *)&C->rows );
#else /* !HAVE_LAPACK */
         for ( i = 0, Cptr = C->data; (unsigned long) i < C->rows;
               i++, Cptr++ )
            gan_dtpsv ( GAN_MATRIXPART_UPPER, GAN_NOTRANSPOSE, GAN_NOUNIT, C->cols,
...
#else /* !HAVE_LAPACK */
         if ( B != C )
            gan_dcopy ( Crows*U->size, B->data, 1, C->data, 1 );
      ...
      else /* U_tr && U_inv */
      {
         ...
         for ( i = 0, Cptr = C->data; (unsigned long) i < C->rows;
            ...
            BLAS_DTPSV ( "U", "N", "N", (long *)&C->cols, U->data, Cptr, (long *)&C->rows );
#else /* !HAVE_LAPACK */
         if ( B != C ) gan_dcopy ( Crows*U->size, B->data, 1, C->data, 1 );
         for ( i = 0, Cptr = C->data; (unsigned long) i < C->rows;
               i++, Cptr++ )
            gan_dtpsv ( GAN_MATRIXPART_UPPER, GAN_NOTRANSPOSE, GAN_NOUNIT, C->cols,
                        U->data, Cptr, C->rows );
...
static Gan_SquMatrix *
 lower_tri_matrix_rmult_squ (
     Gan_SquMatrix *L, Gan_TposeFlag L_tr, Gan_InvertFlag L_inv,
     ...
     Gan_SquMatrix *C )
{
   ...
   /* set result matrix */
   if ( C == NULL )
      C = gan_ltmat_alloc ( L->size );
   else
      C = gan_ltmat_set_size ( C, L->size );
   ...
   if ( L == B && L_tr != B_tr && L_inv == B_inv )
   {
      ...
      if ( L_tr )
      {
...
#ifdef HAVE_LAPACK
            dpptri_ ( "U", (long *)&C->size, C->data, &info );
            gan_err_test_ptr ( info == 0, "lower_tri_matrix_rmult_squ",
                               GAN_ERROR_FAILURE, "" );
#else /* !HAVE_LAPACK */
            if ( !gan_dpptri ( GAN_MATRIXPART_UPPER, C->size, C->data, &info ) )
            {
               gan_err_flush_trace();
         ...
         else
         {
            ...
            for ( i = 0; i < (long)C->size; i++ )
               for ( j = 0; j <= i; j++ )
               {
                  for ( k = i, total = 0.0; k < (long)C->size; k++ )
      ...
      if ( gan_symmat_set_size ( C, L->size ) == NULL )
      {
...
static Gan_SquMatrix *
 upper_tri_matrix_rmult_squ (
     Gan_SquMatrix *U, Gan_TposeFlag U_tr, Gan_InvertFlag U_inv,
     Gan_SquMatrix *B, Gan_TposeFlag B_tr, Gan_InvertFlag B_inv,
     Gan_SquMatrix *C )
{
   long info;
   ...
   /* set result matrix */
   if ( C == NULL )
      C = gan_utmat_alloc ( U->size );
   else
      C = gan_utmat_set_size ( C, U->size );
   ...
   if ( U == B && U_tr != B_tr && U_inv == B_inv )
   {
...
#ifdef HAVE_LAPACK
            dpptri_ ( "U", (long *)&C->size, C->data, &info );
            gan_err_test_ptr ( info == 0, "upper_tri_matrix_rmult_squ", GAN_ERROR_FAILURE, "" );
#else /* !HAVE_LAPACK */
            if ( !gan_dpptri ( GAN_MATRIXPART_UPPER, C->size, C->data, &info ) )
            {
               gan_err_flush_trace();
      ...
      /* convert result matrix to symmetric type */
      if ( gan_symmat_set_size ( C, U->size ) == NULL )
      {
         gan_err_register ( "upper_tri_matrix_rmult_squ", GAN_ERROR_FAILURE,
                            "" );
         return NULL;
...
static Gan_SquMatrix *
 triangular_matrix_lrmult (
     const Gan_SquMatrix *A, const Gan_Matrix *B, Gan_TposeFlag B_tr,
 ...
 */
static Gan_SquMatrix *
 triangular_matrix_invert ( Gan_SquMatrix *A, Gan_SquMatrix *B )
{
   long info;
   ...
   dtptri_ ( "U", "N", (long *)&B->size, B->data, &info );
   if ( info != 0 )
   {
      gan_err_flush_trace();
      if ( info < 0 )
...
#else /* !HAVE_LAPACK */
   /* copy input matrix S into L */
   gan_dcopy ( A->size*(A->size+1)/2, A->data, 1, B->data, 1 );

   /* perform matrix inversion */
   if ( !gan_dtptri ( GAN_MATRIXPART_UPPER, GAN_NOUNIT, B->size, B->data, &info ) )
   {
      gan_err_flush_trace();
      if ( info < 0 )
      {
 ...
 */
static Gan_SquMatrix *
 triangular_matrix_cholesky ( Gan_SquMatrix *A, Gan_SquMatrix *B,
                              int *error_code )
{
   gan_err_flush_trace();
 ...
 */
static double
 triangular_matrix_sumsqr ( const Gan_SquMatrix *A )
{
   long i;
   double total = 0.0;
 ...
 */
static double
 triangular_matrix_Fnorm ( const Gan_SquMatrix *A )
{
   long i;
   double total = 0.0;
...
static Gan_Matrix *
 lower_tri_matrix_matrix ( const Gan_SquMatrix *L, struct Gan_Matrix *B )
{
   long i, j;
   ...
   if ( B == NULL )
      B = gan_mat_alloc ( L->size, L->size );
   else
      B = gan_mat_set_dims ( B, L->size, L->size );
   ...
   /* fill with zeros */
   gan_mat_fill_const_q ( B, L->size, L->size, 0.0 );
...
static Gan_Matrix *
 upper_tri_matrix_matrix ( const Gan_SquMatrix *U, struct Gan_Matrix *B )
{
   unsigned long i, j;
   ...
   /* allocate/set result matrix B */
   if ( B == NULL )
      B = gan_mat_alloc ( U->size, U->size );
   else
      B = gan_mat_set_dims ( B, U->size, U->size );
   ...
   /* fill with zeros */
   gan_mat_fill_const_q ( B, U->size, U->size, 0.0 );
 ...
 */
Gan_SquMatrix *
 gan_ltmat_set_size ( Gan_SquMatrix *L, unsigned long size )
{
   /* set type of matrix */
   ...
   if ( L->data_size < size*(size+1)/2 )
   {
      ...
      gan_err_test_ptr ( L->data_alloc, "gan_ltmat_set_size",
                         GAN_ERROR_INCOMPATIBLE, "" );
      L->data_size = size*(size+1)/2;
      L->data = gan_realloc_array ( double,  L->data, L->data_size );
      if ( L->data == NULL )
      {
         gan_err_flush_trace();
         gan_err_register_with_number ( "gan_ltmat_set_size", GAN_ERROR_MALLOC_FAILED, "", L->data_size*sizeof(double) );
         return NULL;
      }
   }
 ...
 */
Gan_SquMatrix *
 gan_utmat_set_size ( Gan_SquMatrix *U, unsigned long size )
{
   /* set type of matrix */
   ...
   {
      /* re-allocate matrix data */
      gan_err_test_ptr ( U->data_alloc, "gan_utmat_set_size",
                         GAN_ERROR_INCOMPATIBLE, "" );
      U->data_size = size*(size+1)/2;
      U->data = gan_realloc_array ( double,  U->data, U->data_size );
      if ( U->data == NULL )
      {
         gan_err_flush_trace();
         gan_err_register_with_number ( "gan_utmat_set_size", GAN_ERROR_MALLOC_FAILED, "", U->data_size*sizeof(double) );
         return NULL;
      }
   }
 ...
 */
Gan_SquMatrix *
 gan_ltmat_fill_va ( Gan_SquMatrix *L, unsigned long size, ... )
{
   unsigned int i, j;
   va_list ap;
   ...
   if ( L == NULL )
      L = gan_ltmat_alloc ( size );
   else
      L = gan_ltmat_set_size ( L, size );

   /* read variable argument list of matrix elements, lower triangle only */
   va_start ( ap, size );
   for ( i = 0; i < size; i++ )
      for ( j = 0; j <= i; j++ )
         L->data[i*(i+1)/2+j] = va_arg ( ap, double );
 ...
 */
Gan_SquMatrix *
 gan_utmat_fill_va ( Gan_SquMatrix *U, unsigned long size, ... )
{
   unsigned int i, j;
   va_list ap;
   ...
   /* allocate matrix A if necessary */
   if ( U == NULL )
      U = gan_utmat_alloc ( size );
   else
      U = gan_utmat_set_size ( U, size );

   /* read variable argument list of matrix elements, upper triangle only */
   va_start ( ap, size );
   for ( i = 0; i < size; i++ )
      for ( j = i; j < size; j++ )
         U->data[j*(j+1)/2+i] = va_arg ( ap, double );
 ...
 */
Gan_SquMatrix *
 gan_ltmat_fill_const_q ( Gan_SquMatrix *L, unsigned long size, double value )
{
   int i;
   ...
   if ( L == NULL )
   {
      gan_err_register ( "gan_ltmat_fill_const_q", GAN_ERROR_FAILURE, "" );
      return NULL;
   }
 ...
 */
Gan_SquMatrix *
 gan_utmat_fill_const_q ( Gan_SquMatrix *U, unsigned long size, double value )
{
   int i;
   ...
   if ( U == NULL )
   {
      gan_err_register ( "gan_utmat_fill_const_q", GAN_ERROR_FAILURE, "" );
      return NULL;
   }
 ...
 */
Gan_Bool
 gan_ltmat_read_va ( const Gan_SquMatrix *L, unsigned long size, ... )
{
   unsigned int i, j;
   va_list ap;
   ...
   gan_err_test_bool ( L->type == GAN_LOWER_TRI_MATRIX && size == L->size,
                       "gan_ltmat_read_va", GAN_ERROR_INCOMPATIBLE, "" );

   /* fill variable argument list of lower triangular matrix element pointers*/
   va_start ( ap, size );
   for ( i = 0; i < size; i++ )
      for ( j = 0; j <= i; j++ )
         *(va_arg ( ap, double * )) = L->data[i*(i+1)/2+j];
 ...
 */
Gan_Bool
 gan_utmat_read_va ( const Gan_SquMatrix *U, unsigned long size, ... )
{
   unsigned int i, j;
   va_list ap;
   ...
   gan_err_test_bool ( U->type == GAN_UPPER_TRI_MATRIX && size == U->size,
                       "gan_utmat_read_va", GAN_ERROR_INCOMPATIBLE, "" );

   /* fill variable argument list of upper triangular matrix element pointers*/
   va_start ( ap, size );
   for ( i = 0; i < size; i++ )
      for ( j = i; j < size; j++ )
         *(va_arg ( ap, double * )) = U->data[i*(i+1)/2+j];
...
/* not a user function */
Gan_SquMatrix *
 gan_ltmat_form_gen ( Gan_SquMatrix *L, unsigned long size,
                      double *data, size_t data_size )
{
   if ( L == NULL )
   {
      /* dyamically allocate matrix */
      L = gan_malloc_object(Gan_SquMatrix);
   ...
   if ( data == NULL )
   {
      ...
      {
         L->data = gan_malloc_array ( double, L->data_size );
         if ( L->data == NULL )
         {
            gan_err_flush_trace();
            gan_err_register_with_number ( "gan_ltmat_form_gen", GAN_ERROR_MALLOC_FAILED, "", L->data_size*sizeof(double) );
            return NULL;
         }
      }
   ...
   else
   {
      ...
      gan_err_test_ptr ( data_size >= size*(size+1)/2, "gan_ltmat_form_gen",
                         GAN_ERROR_INCOMPATIBLE, "" );
...
/* not a user function */
Gan_SquMatrix *
 gan_utmat_form_gen ( Gan_SquMatrix *U, unsigned long size,
                      double *data, size_t data_size )
{
   if ( U == NULL )
   {
      /* dyamically allocate matrix */
      U = gan_malloc_object(Gan_SquMatrix);
   ...
   if ( data == NULL )
   {
      ...
      {
         U->data = gan_malloc_array ( double, U->data_size );
         if ( U->data == NULL )
         {
            gan_err_flush_trace();
            gan_err_register_with_number ( "gan_utmat_form_gen", GAN_ERROR_MALLOC_FAILED, "", U->data_size*sizeof(double) );
            return NULL;
         }
      }
   ...
   {
      /* data for this matrix is provided */
      gan_err_test_ptr ( data_size >= size*(size+1)/2, "gan_utmat_form_gen",
                         GAN_ERROR_INCOMPATIBLE, "" );
 ...
 */
Gan_SquMatrix *
 gan_ltmat_fill_vap ( Gan_SquMatrix *L, unsigned long size, va_list *aptr )
{
   unsigned int i, j;
   ...
   for ( i = 0; i < size; i++ )
      for ( j = 0; j <= i; j++ )
         L->data[i*(i+1)/2+j] = va_arg ( *aptr, double );
 ...
 */
Gan_SquMatrix *
 gan_utmat_fill_vap ( Gan_SquMatrix *U, unsigned long size, va_list *aptr )
{
   unsigned int i, j;
   ...
   for ( i = 0; i < size; i++ )
      for ( j = i; j < size; j++ )
         U->data[j*(j+1)/2+i] = va_arg ( *aptr, double );
