 ...
 */
Gan_Bool
 gan_vec3_fprint ( FILE *fp, const Gan_Vector3 *p,
 ...
 */
Gan_Bool
 gan_vec3_fscanf ( FILE *fp, Gan_Vector3 *p, char *prefix, int prefix_len )
{
   int ch, result;
   ...
   {
      gan_err_flush_trace();
      gan_err_register ( "gan_vec3_fscanf", GAN_ERROR_CORRUPTED_FILE, "" );
      return GAN_FALSE;
   }
   ...
   /* read vector data */
   result = fscanf ( fp, "%lf%lf%lf", &p->x, &p->y, &p->z );
   if ( result != 3 )
   {
      gan_err_flush_trace();
      gan_err_register ( "gan_vec3_fscanf", GAN_ERROR_CORRUPTED_FILE, "" );
 ...
 */
Gan_Vector3
 gan_vec3_fill_s ( double X, double Y, double Z )
{
   Gan_Vector3 p;

   (void)gan_vec3_fill_q ( &p, X, Y, Z );
   return p;
}
 ...
 */
Gan_Vector3
 gan_vec3_cross_s ( const Gan_Vector3 *p, const Gan_Vector3 *q )
{
   Gan_Vector3 r;

   (void)gan_vec3_cross_q ( p, q, &r );
   return r;
}
 ...
 */
Gan_Vector3
 gan_vec3_set_parts_s ( const Gan_Vector2 *q, double s )
{
   Gan_Vector3 p;

   (void)gan_vec3_set_parts_q ( &p, q, s );
   return p;
}
 ...
 */
Gan_Vector3 *
 gan_vec3_from_vec_q ( const Gan_Vector *x, Gan_Vector3 *a )
{
   if ( !gan_vec_read_va ( x, 3, &a->x, &a->y, &a->z ) )
   {
      gan_err_register ( "gan_vec3_from_vec_q", GAN_ERROR_FAILURE, "" );
      return NULL;
   }
