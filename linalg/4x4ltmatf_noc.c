/**
 * File:          $RCSfile: 4x4ltmatf_noc.c,v $
 ...
 */
Gan_Bool
 gan_ltmat44f_fprint ( FILE *fp, const Gan_SquMatrix44_f *A,
 ...
 */
Gan_Bool
 gan_ltmat44f_fscanf ( FILE *fp, Gan_SquMatrix44_f *A,
                       char *prefix, int prefix_len )
{
   ...
   {
      gan_err_flush_trace();
      gan_err_register ( "gan_ltmat44f_fscanf", GAN_ERROR_CORRUPTED_FILE, "" );
      return GAN_FALSE;
   }
   ...
   *prefix = '\0';
   ...
   gan_err_test_bool ( result == 16, "gan_ltmat44f_fscanf",
                       GAN_ERROR_CORRUPTED_FILE, "" );
   ...
   gan_err_test_bool ( dum[0] == 0.0F && dum[1] == 0.0F && dum[2] == 0.0F &&
                       dum[3] == 0.0F && dum[4] == 0.0F && dum[5] == 0.0F,
                       "gan_ltmat44f_fscanf", GAN_ERROR_CORRUPTED_FILE, "" );
 ...
 */
Gan_Bool
 gan_ltmat44Tf_fprint ( FILE *fp, const Gan_SquMatrix44_f *A,
                        const char *prefix, int indent, const char *fmt )
{
   int i;
 ...
 */
Gan_Bool
 gan_ltmat44Tf_fscanf ( FILE *fp, Gan_SquMatrix44_f *A,
                        char *prefix, int prefix_len )
{
   int ch, result;
   float dum[6];
   ...
   {
      gan_err_flush_trace();
      gan_err_register ( "gan_ltmat44Tf_fscanf", GAN_ERROR_CORRUPTED_FILE, "");
      return GAN_FALSE;
   }
   ...
   *prefix = '\0';
   ...
   result += fscanf ( fp, "%f%f%f%f",     &dum[3], &dum[4], &dum[5], &A->ww );
   gan_err_test_bool ( result == 16, "gan_ltmat44Tf_fscanf",
                       GAN_ERROR_CORRUPTED_FILE, "" );
   ...
   gan_err_test_bool ( dum[0] == 0.0F && dum[1] == 0.0F && dum[2] == 0.0F &&
                       dum[3] == 0.0F && dum[4] == 0.0F && dum[5] == 0.0F,
                       "gan_ltmat44Tf_fscanf", GAN_ERROR_CORRUPTED_FILE, "" );
                       ...
                       float WX, float WY, float WZ, float WW )
{
   ...
   (void)gan_ltmat44f_fill_q ( &A, XX, YX, YY, ZX, ZY, ZZ, WX, WY, WZ, WW );
   return A;
}
 ...
 */
void
 gan_ltmat44f_get_parts_s ( const Gan_SquMatrix44_f *A,
                            Gan_SquMatrix33_f *B, Gan_Vector3_f *p, float *s )
{
   Gan_SquMatrix33_f Bp;
   Gan_Vector3_f     pp;
   float             sp;

   gan_ltmat44f_get_parts_q ( A, (B == NULL) ? &Bp : B, (p == NULL) ? &pp : p,
                             (s == NULL) ? &sp : s );
}
 ...
 */
Gan_SquMatrix44_f
 gan_ltmat44f_set_parts_s ( const Gan_SquMatrix33_f *B, const Gan_Vector3_f *p, float s )
{
   Gan_SquMatrix44_f A;
   Gan_Vector3_f zerov = { 0.0F, 0.0F, 0.0F };

   (void)gan_ltmat44f_set_parts_q ( &A, B, (p == NULL) ? &zerov : p, s );
   return A;
}
 ...
 */
Gan_SquMatrix44_f *
 gan_ltmat44f_invert ( Gan_SquMatrix44_f *A, Gan_SquMatrix44_f *B,
                       int *error_code )
{
   gan_err_test_ptr ( A->type == GAN_LOWER_TRI_MATRIX44_F,
                      "gan_ltmat44f_invert", GAN_ERROR_ILLEGAL_ARGUMENT,
                      "" );
   if ( A->xx == 0.0F || A->yy == 0.0F || A->zz == 0.0F || A->ww == 0.0F )
   {
      if ( error_code == NULL )
      {
         gan_err_flush_trace();
         gan_err_register ( "gan_ltmat44f_invert", GAN_ERROR_SINGULAR_MATRIX,
                            "" );
      }
      else
