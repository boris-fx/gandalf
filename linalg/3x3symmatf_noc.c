/**
 * File:          $RCSfile: 3x3symmatf_noc.c,v $
 * Module:        3x3 symmetric matrices (single precision)
 * Part of:       Gandalf Library
 *
 * Revision:      $Revision: 1.13 $
 * Last edited:   $Date: 2004/03/18 18:26:18 $
 * Author:        $Author: pm $
 ...
 */
Gan_Bool
 gan_symmat33f_fprint ( FILE *fp, const Gan_SquMatrix33_f *A,
 ...
 */
Gan_Bool
 gan_symmat33f_fscanf ( FILE *fp, Gan_SquMatrix33_f *A,
                        char *prefix, int prefix_len )
{
   ...
   {
      gan_err_flush_trace();
      gan_err_register ( "gan_symmat33f_fscanf", GAN_ERROR_CORRUPTED_FILE, "");
      return GAN_FALSE;
   }
   ...
   *prefix = '\0';
   ...
   gan_err_test_bool ( result == 9, "gan_symmat33f_fscanf",
                       GAN_ERROR_CORRUPTED_FILE, "" );
   ...
   gan_err_test_bool ( dum1 == A->yx && dum2 == A->zx && dum3 == A->zy,
                       "gan_symmat33f_fscanf", GAN_ERROR_CORRUPTED_FILE, "" );
 ...
 */
Gan_SquMatrix33_f
 gan_symmat33f_fill_s ( float XX,
                        float YX, float YY,
                        float ZX, float ZY, float ZZ )
{
   Gan_SquMatrix33_f A;

   (void)gan_symmat33f_fill_q ( &A, XX, YX, YY, ZX, ZY, ZZ );
   return A;
}
 ...
 */
Gan_SquMatrix33_f *
 gan_symmat33f_cholesky ( Gan_SquMatrix33_f *A, Gan_SquMatrix33_f *B,
                          int *error_code )
{
   float tmp;

   gan_err_test_ptr ( A->type == GAN_SYMMETRIC_MATRIX33_F,
                      "gan_symmat33f_cholesky", GAN_ERROR_ILLEGAL_ARGUMENT,
                      "" );
   ...
   if ( A->xx <= 0.0F )
   {
      ...
      {
         gan_err_flush_trace();
         gan_err_register ( "gan_symmat33f_cholesky",
 ...
 */
Gan_SquMatrix33_f *
 gan_symmat33f_invert ( Gan_SquMatrix33_f *A, Gan_SquMatrix33_f *B,
                        int *error_code )
{
   float d1 = A->yy*A->zz-A->zy*A->zy, d2 = A->zy*A->zx-A->yx*A->zz,
         d3 = A->yx*A->zy-A->yy*A->zx;
   float det = A->xx*d1 + A->yx*d2 + A->zx*d3;

   gan_err_test_ptr ( A->type == GAN_SYMMETRIC_MATRIX33_F,
                      "gan_symmat33f_invert", GAN_ERROR_ILLEGAL_ARGUMENT,
                      "" );
   if ( det == 0.0F )
   {
      if ( error_code == NULL )
      {
         gan_err_flush_trace();
         gan_err_register ( "gan_symmat33f_invert", GAN_ERROR_SINGULAR_MATRIX,
                            "" );
      }
      else
 ...
 */
void
 gan_symmat33f_get_parts_s ( const Gan_SquMatrix33_f *A,
                             Gan_SquMatrix22_f *B, Gan_Vector2_f *p, float *s )
{
   ...
   gan_symmat33f_get_parts_q ( A, (B == NULL) ? &Bp : B, (p == NULL) ? &pp : p,
                              (s == NULL) ? &sp : s );
}
 ...
 */
Gan_SquMatrix33_f
 gan_symmat33f_set_parts_s ( Gan_SquMatrix22_f *B, const Gan_Vector2_f *p, float s )
{
   Gan_SquMatrix33_f A;
   Gan_Vector2_f zerov = { 0.0F, 0.0F };

   (void)gan_symmat33f_set_parts_q ( &A, B, (p == NULL) ? &zerov : p, s );
   return A;
}
 ...
 */
Gan_Matrix33_f *
 gan_symmat33f_to_mat33f_q ( const Gan_SquMatrix33_f *A, Gan_Matrix33_f *B )
{
   gan_err_test_ptr ( A->type == GAN_SYMMETRIC_MATRIX33_F,
                      "gan_symmat33f_to_mat33f_q", GAN_ERROR_ILLEGAL_TYPE,
                      "" );
   B->xx = A->xx;
 ...
 */
Gan_Matrix33_f
 gan_symmat33f_to_mat33f_s ( const Gan_SquMatrix33_f *A )
{
   Gan_Matrix33_f B;

   gan_symmat33f_to_mat33f_q ( A, &B );
   return B;
}
