 ...
 */
Gan_Bool
 gan_ltmat33_fprint ( FILE *fp, const Gan_SquMatrix33 *A,
 ...
 */
Gan_Bool
 gan_ltmat33_fscanf ( FILE *fp, Gan_SquMatrix33 *A,
                      char *prefix, int prefix_len )
{
   ...
   {
      gan_err_flush_trace();
      gan_err_register ( "gan_ltmat33_fscanf", GAN_ERROR_CORRUPTED_FILE, "" );
      return GAN_FALSE;
   }
   ...
   *prefix = '\0';
   ...
   gan_err_test_bool ( result == 9, "gan_ltmat33_fscanf",
                       GAN_ERROR_CORRUPTED_FILE, "" );
   ...
   gan_err_test_bool ( dum[0] == 0.0 && dum[1] == 0.0 && dum[2] == 0.0,
                       "gan_ltmat33_fscanf", GAN_ERROR_CORRUPTED_FILE, "" );
 ...
 */
Gan_Bool
 gan_ltmat33T_fprint ( FILE *fp, const Gan_SquMatrix33 *A,
                       const char *prefix, int indent, const char *fmt )
{
   int i;
 ...
 */
Gan_Bool
 gan_ltmat33T_fscanf ( FILE *fp, Gan_SquMatrix33 *A,
                       char *prefix, int prefix_len )
{
   int ch, result;
   double dum[3];
   ...
   {
      gan_err_flush_trace();
      gan_err_register ( "gan_ltmat33T_fscanf", GAN_ERROR_CORRUPTED_FILE, "" );
      return GAN_FALSE;
   }
   ...
   *prefix = '\0';
   ...
   result += fscanf ( fp, "\n%lf%lf%lf\n", &A->xx, &A->yx, &A->zx );
   result += fscanf ( fp, "%lf%lf%lf\n",   &dum[0], &A->yy, &A->zy );
   result += fscanf ( fp, "%lf%lf%lf",     &dum[1], &dum[2], &A->zz );
   gan_err_test_bool ( result == 9, "gan_ltmat33T_fscanf",
                       GAN_ERROR_CORRUPTED_FILE, "" );
   ...
   /* check that matrix is indeed upper triangular */
   gan_err_test_bool ( dum[0] == 0.0 && dum[1] == 0.0 && dum[2] == 0.0,
                       "gan_ltmat33T_fscanf", GAN_ERROR_CORRUPTED_FILE, "" );
 ...
 * \f[
 *     \left(\begin{array}{ccc} XX & 0 & 0 \\ YX & YY & 0 \\
 *                              ZX & ZY & ZZ \end{array}\right)
 * \f]
 */
Gan_SquMatrix33
 gan_ltmat33_fill_s ( double XX,
                      double YX, double YY,
                      double ZX, double ZY, double ZZ )
{
   Gan_SquMatrix33 A;

   (void)gan_ltmat33_fill_q ( &A, XX, YX, YY, ZX, ZY, ZZ );
   return A;
}
 ...
 */
void
 gan_ltmat33_get_parts_s ( const Gan_SquMatrix33 *A,
                           Gan_SquMatrix22 *B, Gan_Vector2 *p, double *s )
{
   ...
   gan_ltmat33_get_parts_q ( A, (B == NULL) ? &Bp : B, (p == NULL) ? &pp : p,
                             (s == NULL) ? &sp : s );
}
 ...
 */
Gan_SquMatrix33
 gan_ltmat33_set_parts_s ( const Gan_SquMatrix22 *B, const Gan_Vector2 *p, double s )
{
   Gan_SquMatrix33 A;
   Gan_Vector2 zerov = { 0.0, 0.0 };

   (void)gan_ltmat33_set_parts_q ( &A, B, (p == NULL) ? &zerov : p, s );
   return A;
}
 ...
 */
Gan_SquMatrix33 *
 gan_ltmat33_invert ( Gan_SquMatrix33 *A, Gan_SquMatrix33 *B,
                      int *error_code )
{
   gan_err_test_ptr ( A->type == GAN_LOWER_TRI_MATRIX33,
                      "gan_ltmat33_invert", GAN_ERROR_ILLEGAL_ARGUMENT, "" );
   if ( A->xx == 0.0 || A->yy == 0.0 || A->zz == 0.0 )
   {
      if ( error_code == NULL )
      {
         gan_err_flush_trace();
         gan_err_register ( "gan_ltmat33_invert", GAN_ERROR_SINGULAR_MATRIX,
                            "" );
      }
      else
   ...
   B->xx = 1.0/A->xx;
   B->yx = -A->yx*B->xx/A->yy; B->yy = 1.0/A->yy;
