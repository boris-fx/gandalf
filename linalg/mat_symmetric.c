/**
 * File:          $RCSfile: mat_symmetric.c,v $
...
static Gan_Bool
 symmetric_matrix_set_el ( Gan_SquMatrix *A,
...
static double
 symmetric_matrix_get_el ( const Gan_SquMatrix *A, unsigned i, unsigned j )
{
   /* consistency check */
   gan_err_test_double ( A->type == GAN_SYMMETRIC_MATRIX,
                         "symmetric_matrix_set_el", GAN_ERROR_ILLEGAL_ARGUMENT,
                         "" );

   /* make sure element is inside matrix */
   gan_err_test_double ( i < A->size && j < A->size, "symmetric_matrix_get_el",
                         GAN_ERROR_REF_OUTSIDE_MATRIX, "" );
...
static Gan_Bool
 symmetric_matrix_inc_el ( Gan_SquMatrix *A,
                           unsigned i, unsigned j, double value )
{
   /* consistency check */
   gan_err_test_bool ( A->type == GAN_SYMMETRIC_MATRIX,
...
static Gan_Bool
 symmetric_matrix_dec_el ( Gan_SquMatrix *A,
                           unsigned i, unsigned j, double value )
{
   /* consistency check */
   gan_err_test_bool ( A->type == GAN_SYMMETRIC_MATRIX,
...
static Gan_Bool
 symmetric_matrix_fprint ( FILE *fp, const Gan_SquMatrix *A, const char *prefix,
...
static Gan_SquMatrix *
 symmetric_matrix_copy ( const Gan_SquMatrix *A, Gan_SquMatrix *B )
{
   /* consistency check */
   ...
   if ( B == NULL )
      B = gan_symmat_alloc ( A->size );
   else
      B = gan_symmat_set_size ( B, A->size );
...
static Gan_SquMatrix *
 symmetric_matrix_scale ( Gan_SquMatrix *A, double a, Gan_SquMatrix *B )
{
   /* consistency check */
   gan_err_test_ptr ( A->type == GAN_SYMMETRIC_MATRIX,
...
static Gan_SquMatrix *
 symmetric_matrix_add ( Gan_SquMatrix *A, Gan_TposeFlag A_tr,
                        ...
                        Gan_SquMatrix *C )
{
   ...
   /* check that the type and dimensions of input matrices A & B are
      compatible */
   gan_err_test_ptr ( gan_squmat_same_size(A,B), "symmetric_matrix_add",
   ...
   switch ( Btype )
   {
...
#else
      {
         ...
         {
            /* C = A + B */
            result = gan_dcopy ( A->size*(A->size+1)/2, A->data, 1,
                                 C->data, 1 );
 ...
 */
static Gan_SquMatrix *
 symmetric_matrix_sub ( Gan_SquMatrix *A, Gan_TposeFlag A_tr,
                        Gan_SquMatrix *B, Gan_TposeFlag B_tr,
                        Gan_SquMatrix *C )
{
   /* consistency check */
   ...
   /* check that the type and dimensions of input matrices A & B are
      compatible */
   gan_err_test_ptr ( gan_squmat_same_type_size(A,B), "symmetric_matrix_sub",
                      GAN_ERROR_DIFFERENT_DIMS, "" );
...
#else
   /* compute matrix/vector product */
   if ( !gan_dspmv ( GAN_MATRIXPART_UPPER, A->size, 1.0, A->data, x->data, 1, 0.0,
                     y->data, 1 ) )
   {
      gan_err_flush_trace();
      gan_err_register ( "symmetric_matrix_multv", GAN_ERROR_FAILURE, "" );
     ...
     Gan_Matrix    *B, Gan_TposeFlag B_tr, Gan_Matrix *C )
{
   ...
   if ( B_tr )
   {
      /* multiply matrix data C = A*B^T */
      for ( j = 0; j < Ccols; j++ )
         if ( !gan_dspmv ( GAN_MATRIXPART_UPPER, A->size, 1.0, A->data, &B->data[j],
                           B->rows, 0.0, &C->data[j*C->rows], 1 ) )
         {
            gan_err_flush_trace();
   ...
   else
   {
      /* multiply matrix data C = A*B */
      for ( j = 0; j < Ccols; j++ )
         if ( !gan_dspmv ( GAN_MATRIXPART_UPPER, A->size, 1.0, A->data,
                           &B->data[j*B->rows], 1, 0.0,
                           &C->data[j*C->rows], 1 ) )
         {
            gan_err_flush_trace();
            gan_err_register ( "symmetric_matrix_rmult",
     ...
     Gan_Matrix    *B, Gan_TposeFlag B_tr, Gan_Matrix *C )
{
   ...
   if ( B_tr )
   {
      ...
      for ( i = 0; i < Crows; i++ )
         if ( !gan_dspmv ( GAN_MATRIXPART_UPPER, A->size, 1.0, A->data,
                           &B->data[i*B->rows], 1, 0.0, &C->data[i], C->rows ))
         {
            gan_err_flush_trace();
            gan_err_register ( "symmetric_matrix_lmult", GAN_ERROR_FAILURE,
                               "" );
   ...
   else
   {
      /* multiply matrix data C = B*A */
      for ( i = 0; i < Crows; i++ )
         if ( !gan_dspmv ( GAN_MATRIXPART_UPPER, A->size, 1.0, A->data, &B->data[i],
                           B->rows, 0.0, &C->data[i], C->rows ) )
         {
            gan_err_flush_trace();
            gan_err_register ( "symmetric_matrix_lmult", GAN_ERROR_FAILURE,
                               "" );
 ...
 */
static Gan_SquMatrix *
 symmetric_matrix_lrmult ( const Gan_SquMatrix *A, const Gan_Matrix *B, Gan_TposeFlag B_tr,
                           Gan_Matrix *C, Gan_SquMatrix *D )
{
   ...
   if ( gan_mat_rmult_sym_gen ( B, B_tr, C, GAN_NOTRANSPOSE, D ) == NULL )
   {
      gan_err_register ( "symmetric_matrix_lrmult", GAN_ERROR_FAILURE, "" );
      return NULL;
   }
...
Gan_SquMatrix *
 symmetric_matrix_lrmult_squ (
     ...
     Gan_Matrix *C, Gan_SquMatrix *D )
{
   long nel = A->size*(A->size+1)/2; /* number of matrix elements */
   Gan_Bool Calloc = GAN_FALSE;
   ...
   if ( C == NULL )
   {
      C = gan_mat_alloc ( A->size, A->size );
      Calloc = GAN_TRUE;
   }
   else
      C = gan_mat_set_dims ( C, A->size, A->size );

   /* allocate result matrix D if necessary */
   if ( D == NULL )
      D = gan_symmat_alloc ( A->size );
   else
      D = gan_symmat_set_size ( D, A->size );

   /* perform first multiplication step A*B(^T)(^-1) */
   gan_squmat_rmult_squ ( A, GAN_NOTRANSPOSE, GAN_NOINVERT,
                          B, B_tr, B_inv, C );
 ...
 */
static Gan_SquMatrix *
 symmetric_matrix_invert ( Gan_SquMatrix *A, Gan_SquMatrix *B )
{
   long info;
   ...
   dpptrf_ ( "U", (long *)&B->size, B->data, &info );
   if ( info != 0 )
   {
      gan_err_flush_trace();
      if ( info < 0 )
   ...
   dpptri_ ( "U", (long *)&B->size, B->data, &info );
   if ( info != 0 )
   {
      gan_err_flush_trace();
      if ( info < 0 )
   ...
   /* compute Cholesky factorisation */
   if ( !gan_dpptrf ( GAN_MATRIXPART_UPPER, B->size, B->data, &info ) )
   {
      gan_err_flush_trace();
      if ( info < 0 )
      {
   ...
   /* compute inverse from Cholesky factorisation */
   if ( !gan_dpptri ( GAN_MATRIXPART_UPPER, B->size, B->data, &info ) )
   {
      gan_err_flush_trace();
      if ( info < 0 )
      {
 ...
 */
static Gan_SquMatrix *
 symmetric_matrix_cholesky ( Gan_SquMatrix *S,
                             Gan_SquMatrix *L, int *error_code )
{
   /* consistency check */
   ...
   if ( L == NULL )
      L = gan_symmat_alloc ( S->size );
   else
      L = gan_symmat_set_size ( L, S->size );
   ...
   if ( L->size > 0 && gan_squmat_sumsqr ( L ) == 0.0 )
   {
#if 1
      if ( error_code == NULL )
      {
      ...
      if ( gan_ltmat_set ( L, L->size ) == NULL )
      {
         gan_err_register ( "symmetric_matrix_cholesky", GAN_ERROR_FAILURE,
                            "" );
         return NULL;
      ...
      /* compute Cholesky factorisation */
      if ( !gan_clapack_pptrf ( L, error_code ) )
      {
         if ( error_code == NULL )
            gan_err_register ( "symmetric_matrix_cholesky", GAN_ERROR_FAILURE,
                               "" );
 ...
 */
static double
 symmetric_matrix_sumsqr ( const Gan_SquMatrix *A )
{
   long i, j;
   double total = 0.0;
   ...
   {
      for ( j = 0; j < i; j++ )
         total += 2.0*gan_sqr(A->data[i*(i+1)/2+j]);
 ...
 */
static double
 symmetric_matrix_Fnorm ( const Gan_SquMatrix *A )
{
   long i, j;
   double total = 0.0;
...
static Gan_Matrix *
 symmetric_matrix_matrix ( const Gan_SquMatrix *A, struct Gan_Matrix *B )
{
   long i, j;
 ...
 */
Gan_SquMatrix *
 gan_symmat_set_size ( Gan_SquMatrix *A, unsigned long size )
{
   /* set type of matrix */
   ...
   if ( A->data_size < size*(size+1)/2 )
   {
      ...
      gan_err_test_ptr ( A->data_alloc, "gan_symmat_set_size",
      ...
      {
         gan_err_flush_trace();
         gan_err_register_with_number ( "gan_symmat_set_size", GAN_ERROR_MALLOC_FAILED, "", A->data_size );
         return NULL;
      }
   }
 ...
 */
Gan_SquMatrix *
 gan_symmat_fill_va ( Gan_SquMatrix *A, unsigned long size, ... )
{
   unsigned int i, j;
   va_list ap;

   if ( A == NULL )
      A = gan_symmat_alloc ( size );
   else
      A = gan_symmat_set_size ( A, size );

   if ( A == NULL )
   {
      gan_err_register ( "gan_symmat_fill_va", GAN_ERROR_FAILURE, "" );
      return NULL;
   }
   ...
   for ( i = 0; i < size; i++ )
      for ( j = 0; j <= i; j++ )
         A->data[i*(i+1)/2+j] = va_arg ( ap, double );
 ...
 */
Gan_SquMatrix *
 gan_symmat_fill_const_q ( Gan_SquMatrix *A, unsigned long size, double value )
{
   int i;
 ...
 */
Gan_Bool
 gan_symmat_read_va ( const Gan_SquMatrix *A, unsigned long size, ... )
{
   unsigned int i, j;
   va_list ap;
   ...
   gan_err_test_bool ( A->type == GAN_SYMMETRIC_MATRIX && size == A->size,
                       "gan_symmat_read_va", GAN_ERROR_DIFFERENT_DIMS, "" );
   ...
   for ( i = 0; i < size; i++ )
      for ( j = 0; j <= i; j++ )
         *(va_arg ( ap, double * )) = A->data[i*(i+1)/2+j];
 ...
 */
Gan_SquMatrix *
 gan_symmat_insert_gen ( Gan_SquMatrix *S,
                         ...
                         unsigned long rows, unsigned long cols )
{
   ...
   gan_err_test_ptr ( rS != cS, "gan_symmat_insert_gen", GAN_ERROR_FAILURE,
   ...
   gan_err_test_ptr ( cB + cols <= B->cols && rB + rows <= B->rows &&
                      ...
                      "gan_symmat_insert_gen", GAN_ERROR_ILLEGAL_ARGUMENT, "");
   ...
   if ( rS < cS )
   {
      ...
      gan_err_test_ptr ( rS+rows <= cS, "gan_symmat_insert_gen",
                         GAN_ERROR_FAILURE, "" );
   ...
   else /* rS > cS */
   {
      ...
      /* make sure region doesn't overlap diagonal */
      gan_err_test_ptr ( cS+cols <= rS, "gan_symmat_insert_gen",
                         GAN_ERROR_FAILURE, "" );
 ...
 */
Gan_SquMatrix *
 gan_symmat_insertv_gen ( Gan_SquMatrix *S,
                          unsigned long rS, unsigned long cS,
                          const Gan_Vector *x, Gan_TposeFlag x_tr,
                          unsigned long rx,
                          unsigned long size )
{
   long i;
   ...
   if ( x_tr )
   {
      ...
      gan_err_test_ptr ( rx + size <= x->rows &&
                         ...
                         "gan_symmat_insertv_gen", GAN_ERROR_INCOMPATIBLE, "");
 ...
 */
Gan_SquMatrix *
 gan_symmat_insertsym ( Gan_SquMatrix *A, unsigned long rA,
                        ...
                        unsigned long size )
{
   ...
   gan_err_test_ptr ( A->type == GAN_SYMMETRIC_MATRIX &&
                      ...
                      "gan_symmat_insertsym", GAN_ERROR_INCOMPATIBLE,
                      "matrix dimensions" );
   ...
   switch ( B->type )
   {
      ...
      default:
        gan_err_flush_trace();
        gan_err_register ( "gan_symmat_insertsym", GAN_ERROR_ILLEGAL_TYPE,
                           "" );
        return NULL;
   }
 ...
 */
Gan_SquMatrix *
 gan_symmat_insertsym_od ( Gan_SquMatrix *A,
                           ...
                           unsigned long cols )
{
   ...
   gan_err_test_ptr ( A->type == GAN_SYMMETRIC_MATRIX &&
                      B->type == GAN_SYMMETRIC_MATRIX &&
                      rA != cA && rB != cB, "gan_symmat_insertsym_od",
                      GAN_ERROR_FAILURE, "" );

   /* make sure dimensions are consistent */
   gan_err_test_ptr ( cA + cols <= A->size && rA + rows <= A->size &&
                      cB + cols <= B->size && rB + rows <= B->size,
                      "gan_symmat_insertsym_od", GAN_ERROR_ILLEGAL_ARGUMENT,
                      "" );
   ...
   if ( rA < cA )
   {
      ...
      gan_err_test_ptr ( rA+rows <= cA && rB+rows <= cB,
                         "gan_symmat_insertsym_od", GAN_ERROR_FAILURE, "" );
 ...
 * ascending order */
static Gan_Bool
 sort_eigenstuff ( Gan_SquMatrix *W, Gan_Matrix *Z )
{
   struct EigenStuff *aes;
   int i, j;
   Gan_Matrix *Znew=NULL;
   ...
   if ( aes == NULL )
   {
      gan_err_register_with_number ( "sort_eigenstuff", GAN_ERROR_MALLOC_FAILED, "", W->size*sizeof(struct EigenStuff) );
      return GAN_FALSE;
   }
   ...
   {
      aes[i].index = i;
      aes[i].eval = gan_squmat_get_el(W,i,i);
   }
   ...
   for ( i = (int)W->size-1; i >= 0; i-- )
   {
      gan_squmat_set_el ( W, i, i, aes[i].eval );
      if ( Z != NULL )
         for ( j = (int)W->size-1; j >= 0; j-- )
            gan_mat_set_el ( Z, j, i,
                             gan_mat_get_el ( Znew, j, aes[i].index ) );
   }
   
   /* success */
   if ( Znew != NULL ) gan_mat_free(Znew);
   free(aes);
   return GAN_TRUE;
}
 ...
 */
Gan_Bool
 gan_symmat_eigen ( Gan_SquMatrix *A,
                    Gan_SquMatrix *W, Gan_Matrix *Z,
                    Gan_Bool sort,
                    double *work, unsigned long work_size )
{
   gan_err_test_bool ( gan_squmat_symmetric_type(A),
                       "gan_clapack_spev", GAN_ERROR_ILLEGAL_TYPE, "" );

   /* set sizes of eigenvalue and eigenvector matrix */
   if ( gan_diagmat_set_size ( W, A->size ) == NULL ||
        (Z != NULL && gan_mat_set_dims(Z,A->size,A->size) == NULL) )
   {
...
#ifdef HAVE_LAPACK
   {
      ...
      gan_err_test_bool ( work_size >= 3*A->size,
                          "gan_symmat_eigen", GAN_ERROR_FAILURE,
      ...
      {
#if 0
         extern Gan_Bool gan_dspev(Gan_LapackEvecType jobz, Gan_LapackUpLoFlag upper,
         ...
         gan_dspev ( GAN_EVECTYPE_EVECSORIGINAL, GAN_MATRIXPART_UPPER, A->size, Acopy->data, W->data,
                     Z->data, Z->rows, work, &info );
         gan_squmat_free(Acopy);
#endif
      }
      ...
      if ( info != 0 )
      {
         ...
         if ( info < 0 )
         {
            gan_err_register ( "gan_symmat_eigen",
   ...
   if ( Z == NULL )
   {
      ...
      ccm_eigval ( A->data, W->data, A->size );
   }
   else
   {
      ...
      if ( gan_mat_from_squmat_q ( A, Z ) == NULL )
      {
         gan_err_flush_trace();
         gan_err_register ( "gan_symmat_eigen", GAN_ERROR_FAILURE, "" );
         return GAN_FALSE;
      }
   
      ccm_eigen ( Z->data, W->data, A->size );

      /* transpose eigenvector matrix in-place */
      if ( !gan_mat_tpose_i(Z) )
      {
         gan_err_register ( "gan_symmat_eigen", GAN_ERROR_FAILURE, "" );
         return GAN_FALSE;
      }
...
#else
   {
      ...
      if ( Z == NULL )
      {
         result = gan_dspev ( GAN_EVECTYPE_EVECSORIGINAL, GAN_MATRIXPART_UPPER,
                              A->size, A->data, W->data, NULL, 1, work, &info );
      }
      else
      {
         result = gan_dspev ( GAN_EVECTYPE_EVECSORIGINAL, GAN_MATRIXPART_UPPER,
                              A->size, A->data, W->data, Z->data, Z->rows, work,
                              &info );
      }
...
/* not a user function */
Gan_SquMatrix *
 gan_symmat_form_gen ( Gan_SquMatrix *A, unsigned long size,
                       double *data, size_t data_size )
{
   if ( A == NULL )
   {
      ...
      {
         gan_err_flush_trace();
         gan_err_register_with_number ( "gan_symmat_form_gen", GAN_ERROR_MALLOC_FAILED, "", sizeof(*A) );
         return NULL;
      }
   ...
   if ( data == NULL )
   {
      ...
      else
      {
         ...
         {
            gan_err_flush_trace();
            gan_err_register_with_number ( "gan_symmat_form_gen", GAN_ERROR_MALLOC_FAILED, "", A->data_size*sizeof(double) );
            return NULL;
         }
      }
   ...
   else
   {
      ...
      gan_err_test_ptr ( data_size >= size*(size+1)/2, "gan_symmat_form_gen",
                         GAN_ERROR_INCOMPATIBLE, "" );
   ...
   if ( gan_symmat_set_size ( A, size ) == NULL )
   {
      gan_err_register ( "gan_symmat_form_gen", GAN_ERROR_FAILURE, "" );
      return NULL;
   }
 ...
 */
Gan_SquMatrix *
 gan_symmat_fill_vap ( Gan_SquMatrix *A, unsigned long size, va_list *aptr )
{
   unsigned int i, j;
   ...
   if ( A == NULL )
   {
      gan_err_register ( "gan_symmat_fill_vap", GAN_ERROR_FAILURE, "" );
      return NULL;
   }
   ...
   for ( i = 0; i < size; i++ )
      for ( j = 0; j <= i; j++ )
         A->data[i*(i+1)/2+j] = va_arg ( *aptr, double );
