...
/**
 * \defgroup GeneralSizeMatrixSVD Singular Value Decomposition of a General Size Matrix
 * \{
 */
 ...
 */
Gan_Bool
 gan_mat_svd ( Gan_Matrix *A,
               Gan_Matrix *U, Gan_SquMatrix *S, Gan_Matrix *VT,
               double *work, unsigned long work_size )
{
   ...
   if ( gan_mat_set_dims ( U, A->rows, minmn ) == NULL ||
        gan_squmat_set_type_size ( S, GAN_DIAGONAL_MATRIX, minmn ) == NULL ||
        gan_mat_set_dims ( VT, minmn, A->cols ) == NULL )
   {
      gan_err_register ( "gan_mat_svd", GAN_ERROR_FAILURE,
   ...
   if ( work == NULL )
   {
      ...
      {
         gan_err_flush_trace();
         gan_err_register_with_number ( "gan_mat_svd", GAN_ERROR_MALLOC_FAILED, "", work_size );
         return GAN_FALSE;
      }
   ...
   dgesvd_ ( "S", "S", (long *)&A->rows, (long *)&A->cols,
   ...
   if ( info != 0 )
   {
      ...
      if ( info < 0 )
      {
         gan_err_register ( "gan_mat_gesvd", GAN_ERROR_CLAPACK_ILLEGAL_ARG,
                            "in dgesvd_()" );
      }
      else /* info > 0 */
         gan_err_register ( "gan_mat_gesvd", GAN_ERROR_NO_CONVERGENCE,
                            "in dgesvd_()" );
...
#else
   int ccm_svduv(double *d,double *a,double *u,int m,double *v,int n);
   long i;
   SVDOrder *index;
   ...
   {
      /* set correct sizes of U, S and VT */
      if ( gan_mat_set_dims ( U, A->rows, A->rows ) == NULL ||
           gan_squmat_set_type_size ( S, GAN_DIAGONAL_MATRIX, A->rows )
           == NULL ||
           gan_mat_set_dims ( VT, A->cols, A->cols ) == NULL )
      {
         gan_err_register ( "gan_mat_svd", GAN_ERROR_FAILURE,
                            "faied to set up output matrices");
         return GAN_FALSE;
      ...
      if ( ccm_svduv ( S->data, A->data, VT->data, A->cols, U->data, A->rows )
           != 0 )
      {
         gan_err_flush_trace();
         gan_err_register ( "gan_mat_svd", GAN_ERROR_CCMATH_FAILED, "" );
         return GAN_FALSE;
      }

      /* transpose U in-place */
      if ( gan_mat_tpose_i ( U ) == NULL )
      {
         gan_err_register ( "gan_mat_svd", GAN_ERROR_FAILURE, "" );
         return GAN_FALSE;
      }
      ...
      if ( gan_mat_copy_q ( VT, A ) == NULL )
      {
         gan_err_register ( "gan_mat_svd", GAN_ERROR_FAILURE, "" );
         return GAN_FALSE;
      }

      if ( gan_mat_set_dims ( VT, U->rows, VT->cols ) == NULL ||
           gan_mat_insert ( VT, 0, 0, A, 0, 0, VT->rows, VT->cols ) == NULL )
      {
         gan_err_register ( "gan_mat_svd", GAN_ERROR_FAILURE, "" );
         return GAN_FALSE;
      }
   ...
   else
   {
      ...
      if ( (AT = gan_mat_tpose_s(A)) == NULL ||
           gan_mat_copy_q ( AT, A ) == NULL )
      {
         gan_err_register ( "gan_mat_svd", GAN_ERROR_FAILURE, "" );
         return GAN_FALSE;
      }
      ...
      /* set correct sizes of U, S and VT */
      if ( gan_mat_set_dims ( U, A->cols, A->cols ) == NULL ||
           gan_squmat_set_type_size ( S, GAN_DIAGONAL_MATRIX, A->rows )
           == NULL ||
           gan_mat_set_dims ( VT, A->rows, A->rows ) == NULL )
      {
         gan_err_register ( "gan_mat_svd", GAN_ERROR_FAILURE,
                            "faied to set up output matrices");
         return GAN_FALSE;
      ...
      /* call CCM SVD routine */
      if ( ccm_svduv ( S->data, A->data, U->data, A->cols, VT->data, A->rows )
           != 0 )
      {
         gan_err_flush_trace();
         gan_err_register ( "gan_mat_svd", GAN_ERROR_CCMATH_FAILED, "" );
      ...
      if ( gan_mat_copy_q ( U, A ) == NULL )
      {
         gan_err_register ( "gan_mat_svd", GAN_ERROR_FAILURE, "" );
         return GAN_FALSE;
      }

      if ( gan_mat_set_dims ( U, U->rows, VT->cols ) == NULL ||
           gan_mat_insert ( U, 0, 0, A, 0, 0, U->rows, U->cols ) == NULL )
      {
         gan_err_register ( "gan_mat_svd", GAN_ERROR_FAILURE, "" );
         return GAN_FALSE;
      }
   ...
   {
      gan_err_flush_trace();
      gan_err_register_with_number ( "gan_mat_svd", GAN_ERROR_MALLOC_FAILED, "", S->size*sizeof(SVDOrder) );
      return GAN_FALSE;
   }
   ...
   {
      index[i].index = i;
      index[i].sing_val = gan_squmat_get_el(S,i,i);
   }
   ...
   if ( i >= 0 )
   {
      ...
      Up  = gan_mat_copy_s(U);
      VTp = gan_mat_copy_s(VT);
      Sp  = gan_squmat_copy_s(S);
      ...
      for ( i = (long)S->size-1; i >= 0; i-- )
         if ( (ip = index[i].index) != i )
         {
            ...
            if ( gan_mat_insert ( U, 0, i, Up, 0, ip, U->rows, 1 ) == NULL ||
                 gan_mat_insert ( VT, i, 0, VTp, ip, 0, 1, VT->cols) == NULL ||
                 !gan_squmat_set_el ( S, i, i,
                                      gan_squmat_get_el ( Sp, ip, ip ) ) )
            {
               gan_err_register ( "gan_mat_svd", GAN_ERROR_FAILURE, "" );
               free(index);
               return GAN_FALSE;
