/**
 * File:          $RCSfile: 3x3mat_noc.c,v $
 * Module:        3x3 matrices (double precision)
 * Part of:       Gandalf Library
 *
 * Revision:      $Revision: 1.21 $
 * Last edited:   $Date: 2004/03/18 18:26:17 $
 * Author:        $Author: pm $
 ...
 */
Gan_Bool
 gan_mat33_fprint ( FILE *fp, const Gan_Matrix33 *A,
 ...
 */
Gan_Bool
 gan_mat33_fscanf ( FILE *fp, Gan_Matrix33 *A, char *prefix, int prefix_len )
{
   int ch, result;
   ...
   {
      gan_err_flush_trace();
      gan_err_register ( "gan_mat33_fscanf", GAN_ERROR_CORRUPTED_FILE, "" );
      return GAN_FALSE;
   }
   ...
   gan_err_test_bool ( result == 9, "gan_mat33_fscanf",
                       GAN_ERROR_CORRUPTED_FILE, "" );
 ...
 */
Gan_Matrix33
 gan_mat33_fill_s ( double XX, double XY, double XZ,
                    double YX, double YY, double YZ,
                    double ZX, double ZY, double ZZ )
{
   Gan_Matrix33 A;

   (void)gan_mat33_fill_q ( &A, XX, XY, XZ, YX, YY, YZ, ZX, ZY, ZZ );
   return A;
}
 ...
 */
Gan_Vector2 *
 gan_mat33_multv2h_q ( const Gan_Matrix33 *A, const Gan_Vector2 *p, double h,
                       Gan_Vector2 *q )
{
   Gan_Vector3 p3;

   p3 = gan_vec3_set_parts_s ( p, h );
   p3 = gan_mat33_multv3_s ( A, &p3 );
   gan_err_test_ptr ( p3.z != 0.0, "gan_mat33_multv2h_q",
                      GAN_ERROR_DIVISION_BY_ZERO, "" );

   gan_vec2_fill_q ( q, h*p3.x/p3.z, h*p3.y/p3.z );
   return q;
}
 ...
 */
Gan_Vector2
 gan_mat33_multv2h_s ( const Gan_Matrix33 *A, const Gan_Vector2 *p, double h )
{
   Gan_Vector2 q;

   if ( gan_mat33_multv2h_q ( A, p, h, &q ) == NULL )
      assert(0);
 ...
 */
Gan_Matrix33
 gan_mat33_cross_s ( const Gan_Vector3 *p )
{
   Gan_Matrix33 A;

   (void)gan_mat33_cross_q ( p, &A );
   return A;
}
 ...
 */
Gan_Matrix33 *
 gan_mat33_tpose_i ( Gan_Matrix33 *A )
{
   double tmp;
 ...
 */
Gan_Matrix33 *
 gan_mat33_invert ( Gan_Matrix33 *A, Gan_Matrix33 *B, int *error_code )
{
   double d1 = A->yy*A->zz-A->yz*A->zy, d2 = A->yz*A->zx-A->yx*A->zz,
          d3 = A->yx*A->zy-A->yy*A->zx;
   double det = A->xx*d1 + A->xy*d2 + A->xz*d3;
   ...
   if ( det == 0.0 )
   {
      ...
      {
         gan_err_flush_trace();
         gan_err_register ( "gan_mat33_invert_q", GAN_ERROR_SINGULAR_MATRIX,
                            "" );
      }
      else
 ...
 */
Gan_Matrix33 *
 gan_mat33_adjoint_q ( const Gan_Matrix33 *A, Gan_Matrix33 *B )
{
   gan_err_test_ptr ( A != B, "gan_mat33_adjoint_q",
 ...
 */
Gan_Matrix33 *
 gan_mat33_adjointT_q ( const Gan_Matrix33 *A, Gan_Matrix33 *B )
{
   gan_err_test_ptr ( A != B, "gan_mat33_adjointT_q",
                      GAN_ERROR_ILLEGAL_ARGUMENT, "can't do adjoint in place");
 ...
 */
void
 gan_mat33_get_rows_s ( const Gan_Matrix33 *A,
                        Gan_Vector3 *p, Gan_Vector3 *q, Gan_Vector3 *r )
{
   gan_mat33_get_rows_q ( A, p, q, r );
}
 ...
 */
void
 gan_mat33_get_cols_s ( const Gan_Matrix33 *A,
                        Gan_Vector3 *p, Gan_Vector3 *q, Gan_Vector3 *r )
{
   gan_mat33_get_cols_q ( A, p, q, r );
}
 ...
 */
Gan_Matrix22
 gan_mat33_get_m22tl_s ( const Gan_Matrix33 *A )
{
   Gan_Matrix22 B;

   (void)gan_mat33_get_m22tl_q ( A, &B );
   return B;
}
 ...
 */
Gan_Vector2
 gan_mat33_get_v2tr_s ( const Gan_Matrix33 *A )
{
   Gan_Vector2 p;

   (void)gan_mat33_get_v2tr_q ( A, &p );
   return p;
}
 ...
 */
void
 gan_mat33_get_parts_s ( const Gan_Matrix33 *A, Gan_Matrix22 *B,
                         Gan_Vector2 *p, Gan_Vector2 *q, double *s )
{
   gan_mat33_get_parts_q ( A, B, p, q, s );
}
 ...
 */
Gan_Matrix33
 gan_mat33_set_parts_s ( const Gan_Matrix22 *B,
                         const Gan_Vector2 *p, const Gan_Vector2 *q, double s )
{
   Gan_Matrix33 A;
   Gan_Vector2 zerov = { 0.0, 0.0 };

   (void)gan_mat33_set_parts_q ( &A, B, (p == NULL) ? &zerov : p,
                                 (q == NULL) ? &zerov : q, s );
   return A;
}
 ...
 */
Gan_Matrix33
 gan_mat33_set_cols_s ( const Gan_Vector3 *p, const Gan_Vector3 *q, const Gan_Vector3 *r )
{
   Gan_Matrix33 A;

   (void)gan_mat33_set_cols_q ( &A, p, q, r );
   return A;
}
 ...
 */
Gan_Matrix33
 gan_mat33_set_rows_s ( const Gan_Vector3 *p, const Gan_Vector3 *q, const Gan_Vector3 *r )
{
   Gan_Matrix33 A;

   (void)gan_mat33_set_rows_q ( &A, p, q, r );
   return A;
}
 ...
 */
Gan_Matrix33 *
 gan_mat33_from_mat_q ( const Gan_Matrix *A, Gan_Matrix33 *B )
{
   if ( !gan_mat_read_va ( A, 3, 3, &B->xx, &B->xy, &B->xz,
                                    &B->yx, &B->yy, &B->yz,
                                    &B->zx, &B->zy, &B->zz ) )
   {
      gan_err_register ( "gan_mat33_from_mat_q", GAN_ERROR_FAILURE, "" );
      return NULL;
   }
 ...
 */
Gan_Matrix33 *
 gan_mat33_from_squmat_q ( const Gan_SquMatrix *A, Gan_Matrix33 *B )
{
   switch ( A->type )
   {
      case GAN_SCALED_IDENT_MATRIX:
        if ( !gan_scalImat_read_va ( A, 3, &B->xx ) )
        {
           gan_err_register ( "gan_mat33_from_squmat_q", GAN_ERROR_FAILURE,
                              "" );
           return NULL;
        }

        B->xy = B->yx = B->xz = B->zx = B->yz = B->zy = 0.0;
        B->zz = B->yy = B->xx;
        break;

      case GAN_DIAGONAL_MATRIX:
        if ( !gan_diagmat_read_va ( A, 3, &B->xx, &B->yy, &B->zz ) )
        {
           gan_err_register ( "gan_mat33_from_squmat_q", GAN_ERROR_FAILURE,
                              "" );
           return NULL;
      ...
      case GAN_SYMMETRIC_MATRIX:
        if ( !gan_symmat_read_va ( A, 3, &B->xx, &B->yx, &B->yy,
                                         &B->zx, &B->zy, &B->zz ) )
        {
           gan_err_register ( "gan_mat33_from_squmat_q", GAN_ERROR_FAILURE,
                              "" );
      ...
      case GAN_LOWER_TRI_MATRIX:
        if ( !gan_ltmat_read_va ( A, 3, &B->xx, &B->yx, &B->yy,
                                        &B->zx, &B->zy, &B->zz ) )
        {
           gan_err_register ( "gan_mat33_from_squmat_q", GAN_ERROR_FAILURE,
                              "" );
      ...
      case GAN_UPPER_TRI_MATRIX:
        if ( !gan_utmat_read_va ( A, 3, &B->xx, &B->xy, &B->yy,
                                        &B->xz, &B->yz, &B->zz ) )
        {
           gan_err_register ( "gan_mat33_from_squmat_q", GAN_ERROR_FAILURE,
                              "" );
 ...
 */
Gan_SquMatrix33 *
 gan_squmat33_from_squmat_q ( const Gan_SquMatrix *A, Gan_SquMatrix33 *B )
{
   switch ( A->type )
   {
      case GAN_SCALED_IDENT_MATRIX:
        ...
        if ( !gan_scalImat_read_va ( A, 3, &B->xx ) )
        {
           gan_err_register ( "gan_squmat33_from_squmat_q", GAN_ERROR_FAILURE,
                              "" );
           return NULL;
        }
