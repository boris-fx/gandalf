/**
 * File:          $RCSfile: cameraf_sine_law.c,v $
 ...
 * \sa gan_camera_build_sine_law().
 */
Gan_Bool
 gan_cameraf_build_sine_law ( Gan_Camera_f *camera,
                              float zh, float fx, float fy,
                              float x0, float y0 )
{
   if ( !gan_cameraf_set_common_fields ( camera, GAN_SINE_LAW_CAMERA,
                                         zh, fx, fy, x0, y0 ) )
   {
      gan_err_register ( "gan_cameraf_build_sine_law", GAN_ERROR_FAILURE,
                         "" );
      return GAN_FALSE;
   }
