...
static void build_linear_interpolation_neighbours( const Gan_Vector2 *x,
                                       int neib_x[], int neib_y[] )
{
   ...
   neib_x[0] = (int)floor(x->x - 0.5);
   neib_x[1] = neib_x[0] + 1;
   neib_y[0] = (int)floor(x->y - 0.5);
   neib_y[1] = neib_y[0] + 1;
...
*/
static void build_linear_interpolation_weights( const Gan_Vector2 *x,
                                                const int neib_x[], const int neib_y[],
                                                double a[], double b[] )
{
   a[0] = neib_x[1] - x->x + 0.5;
   a[1] = 1.0 - a[0];
   b[0] = neib_y[1] - x->y + 0.5;
   b[1] = 1.0 - b[0];
}
...
\param b - interpolation coefficients for y
*/
static void build_linear_interpolation_weightsJ( const Gan_Vector2 *x,
                                                const int neib_x[], const int neib_y[],
                                                double ax[], double bx[],
                                                double ay[], double by[] )
{
...
static Gan_Vector2 interpolate_linear( const double a[],
                                       const double b[],
                                       const Gan_Vector2 neib[][2] )
{
   Gan_Vector2 v;
   unsigned int k,l;

   // Do linear interpolation
   (void)gan_vec2_zero_q( &v );
   for ( k = 0; k < 2; ++k )
   {
      for ( l = 0; l < 2; ++l )
      {
...
static Gan_Bool
 point_distort ( const Gan_Camera *camera,
                        ...
                        int *error_code, Gan_Bool undistort )
{
   ...
   double a[2], b[2]; // Interpolation coefficients for x and y respectively
   int neib_y[2], neib_x[2]; // Integer neighbours of point
   Gan_Vector2 neib[2][2];
   Gan_Vector2 centreST; // Centre of ST map
   ...
   {
      centreST.x = 0.5 * camera->nonlinear.stmap.stmap_inv->width;
      centreST.y = 0.5 * camera->nonlinear.stmap.stmap_inv->height;
   }
   else
   {
      centreST.x = 0.5 * camera->nonlinear.stmap.stmap_dir->width;
      centreST.y = 0.5 * camera->nonlinear.stmap.stmap_dir->height;
   }
   ...
   if ( build_linear_interpolation_values(
         ...
         neib_x, neib_y, neib, 2.0 * CX0, 2.0 * CY0 ) == GAN_TRUE )
   {
                 ...
                 int *error_code )
{
   ...
   if ( HX || HC )
   {
      ...
      {
         Gan_Matrix22 H; // derivatives of distorted point with respect to coordinates of undistorted image point

         // Compute derivatives of projected point with respect to coordinates of original point
         // Use linear interpolation
         double ax[2], bx[2], ay[2], by[2]; // Interpolation coefficients for x and y respectively
         ...
         build_linear_interpolation_values(
            camera->nonlinear.stmap.stmap_dir, neib_x, neib_y, neib, 2.0 * CX0, 2.0 * CY0 );
      ...
      else
      {
         /* compute Jacobians */
         if ( HX != NULL )
            (void)gan_mat22_fill_q ( HX, CFX, 0.0,
                                         0.0, CFY );
...
Gan_Bool gan_camera_build_ST_map ( Gan_Camera *camera,
                                   double zh,
                                   double fx, double fy,
                                   double x0, double y0,
...
{
   if ( !gan_camera_set_common_fields ( camera, GAN_STMAP_CAMERA,
                                        zh, fx, fy, x0, y0 ) )
   {
