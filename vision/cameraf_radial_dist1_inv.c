/**
 * File:          $RCSfile: cameraf_radial_dist1_inv.c,v $
...
static Gan_Bool
 compute_distortion ( float K1, float thres_dR,
                      float outer_a, float outer_b, Gan_Vector2_f *X,
                      float *dp, int *error_code )
{
   float dR = (float)sqrt(gan_vec2f_sqrlen_q(X));
   Gan_Complex x[3];
   double R = DBL_MAX;
   int nos;
                 ...
                 int *error_code )
{
   ...
   if ( X->z != 1.0F )
   {
      ...
      (void)gan_vec2f_fill_q ( &X2, X->x/X->z, X->y/X->z );
   }
   else
   {
      (void)gan_vec2f_fill_q ( &X2, X->x, X->y );
   }
   ...
   (void)gan_vec3f_fill_q ( p, CX0 + CFX*X2.x/dp, CY0 + CFY*X2.y/dp, CZH );
...
static Gan_Bool
 point_backproject ( const Gan_Camera_f *camera,
                     ...
                     int *error_code )
{
   float XZp, YZp, XZp2, YZp2, Rp2, dp;
   ...
   dp = 1.0F + CK1*Rp2;
   (void)gan_vec3f_fill_q ( X, dp*XZp, dp*YZp, 1.0F );
                        ...
                        int *error_code )
{
   ...
   (void)gan_vec2f_fill_q ( &X, (CZH*pu->x - CX0*pu->z)/(CFX*pu->z),
   ...
   (void)gan_vec3f_fill_q ( p, CX0 + CFX*X.x/dp, CY0 + CFY*X.y/dp, CZH );
                           ...
                           int *error_code)
{
   ...
   /* compute distortion scale factor */
   Rp2 = gan_vec2f_sqrlen_q(&Xd);
   dp = 1.0F + CK1*Rp2;

   /* fill image point with distortion removed */
   (void)gan_vec3f_fill_q ( pu, CX0 + CFX*dp*Xd.x, CY0 + CFY*dp*Xd.y, CZH );
 ...
 */
Gan_Bool
 gan_cameraf_build_radial_distortion_1_inv ( Gan_Camera_f *camera,
                                             float zh,
                                             float fx, float fy,
                                             float x0, float y0,
                                             float K1 )
