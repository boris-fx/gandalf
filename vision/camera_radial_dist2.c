/**
 * File:          $RCSfile: camera_radial_dist2.c,v $
 * Module:        Functions for camera with 2 radial distortion parameters
 * Part of:       Gandalf Library
 *
 * Revision:      $Revision: 1.12 $
 * Last edited:   $Date: 2005/06/10 11:02:39 $
 * Author:        $Author: pm $
                 ...
                 int *error_code )
{
   ...
   if ( R2 > CTR2 )
   {
      ...
      if ( HC != NULL )
      {
         ...
         HC[0].nonlinear.radial2.K1 = 0.0;
         HC[0].nonlinear.radial2.K2 = 0.0;
         HC[1].zh = 0.0;
         HC[1].fx = 0.0;
         HC[1].fy = 0.0;
         HC[1].x0 = 0.0;
         ...
         HC[1].nonlinear.radial2.K1 = 0.0;
         HC[1].nonlinear.radial2.K2 = 0.0;
      }
...
#if 0
   2.0*CFX*YZ*(CK1 + 2.0*CK2*XZ*R2),
   2.0*CFY*XZ*(CK1 + 2.0*CK2*YZ*R2),
#endif
   /* compute Jacobians */
   if ( HX != NULL )
      (void)gan_mat22_fill_q ( HX,
                    CFX*(1.0 + CK1*(3.0*XZ2 + YZ2) + CK2*R2*(5.0*XZ2 + YZ2)),
                    2.0*CFX*XZ*YZ*(CK1 + R2*2.0*CK2),
                    2.0*CFY*XZ*YZ*(CK1 + R2*2.0*CK2),
                    CFY*(1.0 + CK1*(3.0*YZ2 + XZ2) + CK2*R2*(5.0*YZ2 + XZ2)) );
...
static Gan_Bool
 compute_distortion ( double K1, double K2,
                      double thres_R2, double thres_dR,
                      double outer_a, double outer_b,
                      Gan_Vector2 *Xd,
                      double *dp, int *error_code )
{
   double R, R2, dR = sqrt(gan_vec2_sqrlen_q(Xd)), deriv, Rdiff;
   int it;
   ...
   for ( it = 1; ; it++ )
   {
      ...
      Rdiff = (R*(1.0 + R2*(K1 + R2*K2)) - dR)/deriv;
      R -= Rdiff;
 ...
 * \sa gan_cameraf_build_radial_distortion_2().
 */
Gan_Bool
 gan_camera_build_radial_distortion_2 ( Gan_Camera *camera,
                                        double zh, double fx, double fy,
                                        double x0, double y0,
                                        double K1, double K2 )
{
   if ( !gan_camera_set_common_fields ( camera, GAN_RADIAL_DISTORTION_2,
                                        zh, fx, fy, x0, y0 ) )
   {
      gan_err_register ( "gan_camera_build_radial_distortion_2",
                         GAN_ERROR_FAILURE, "" );
      return GAN_FALSE;
   }

   /* copy non-linear camera parameters */
   CK1 = K1; /* radial distortion cubic term */
   CK2 = K2; /* radial distortion quintic term */
   ...
   if ( CTR2 != DBL_MAX )
   {
      ...
      CTDR = (1.0 + CTR2*(K1 + CTR2*K2))*R;

      /* compute parameters of outer linear model */
      COA = 1.0 + CTR2*(3.0*K1 + CTR2*5.0*K2);
      COB = CTDR - COA*R;
   }
