/**
 * File:          $RCSfile: cameraf_radial_dist1.c,v $
                 ...
                 int *error_code )
{
   ...
   if ( R2 > CTR2 )
   {
      ...
      if ( HC != NULL )
      {
         ...
         HC[0].nonlinear.radial1.K1 = 0.0F;
         HC[1].zh = 0.0F;
         HC[1].fx = 0.0F;
         HC[1].fy = 0.0F;
         HC[1].x0 = 0.0F;
         ...
         HC[1].nonlinear.radial1.K1 = 0.0F;
      }
   ...
   if ( HX != NULL )
      (void)gan_mat22f_fill_q ( HX,
                          CFX*(1.0F + CK1*(3.0F*XZ2 + YZ2)), 2.0F*CFX*CK1*XZ*YZ,
                          2.0F*CFY*CK1*XZ*YZ, CFY*(1.0F + CK1*(XZ2 + 3.0F*YZ2)) );
...
{
   float dR = (float)sqrt(gan_vec2f_sqrlen_q(Xd));
   Gan_Complex x[3];
   double R = DBL_MAX;
   int nos;
 ...
 * \sa gan_camera_build_radial_distortion_1().
 */
Gan_Bool
 gan_cameraf_build_radial_distortion_1 ( Gan_Camera_f *camera,
                                         float zh, float fx, float fy,
                                         float x0, float y0, float K1 )
{
   if ( !gan_cameraf_set_common_fields ( camera, GAN_RADIAL_DISTORTION_1,
                                         zh, fx, fy, x0, y0 ) )
   {
      gan_err_register ( "gan_cameraf_build_radial_distortion_1",
                         GAN_ERROR_FAILURE, "" );
      return GAN_FALSE;
   }
