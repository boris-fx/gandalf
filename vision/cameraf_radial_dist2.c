/**
 * File:          $RCSfile: cameraf_radial_dist2.c,v $
                 ...
                 int *error_code )
{
   ...
   /* consistency check */
   gan_err_test_bool ( camera->type == GAN_RADIAL_DISTORTION_2,
   ...
   if ( R2 > CTR2 )
   {
      ...
      if ( HC != NULL )
      {
         ...
         HC[0].x0 = 0.0F;
         HC[0].y0 = 0.0F;
         HC[0].type = GAN_RADIAL_DISTORTION_2;
         HC[0].nonlinear.radial2.K1 = 0.0F;
         HC[0].nonlinear.radial2.K2 = 0.0F;
         HC[1].zh = 0.0F;
         HC[1].fx = 0.0F;
         HC[1].fy = 0.0F;
         HC[1].x0 = 0.0F;
         HC[1].y0 = 0.0F;
         HC[1].type = GAN_RADIAL_DISTORTION_2;
         HC[1].nonlinear.radial2.K1 = 0.0F;
         HC[1].nonlinear.radial2.K2 = 0.0F;
      }
   ...
   d = 1.0F + (CK1 + CK2*R2)*R2;
   x.x = CX0 + d*CFX*XZ;
   x.y = CY0 + d*CFY*YZ;

#if 0
   2.0F*CFX*YZ*(CK1 + 2.0F*CK2*XZ*R2),
   2.0F*CFY*XZ*(CK1 + 2.0F*CK2*YZ*R2),
#endif
   /* compute Jacobians */
   if ( HX != NULL )
      (void)gan_mat22f_fill_q ( HX,
                 CFX*(1.0F + CK1*(3.0F*XZ2 + YZ2) + CK2*R2*(5.0F*XZ2 + YZ2)),
                 2.0F*CFX*XZ*YZ*(CK1 + R2*2.0F*CK2),
                 2.0F*CFY*XZ*YZ*(CK1 + R2*2.0F*CK2),
                 CFY*(1.0F + CK1*(3.0F*YZ2 + XZ2) + CK2*R2*(5.0F*YZ2 + XZ2)) );
   ...
   if ( HC != NULL )
   {
      ...
      HC[0].y0 = 0.0F;
      HC[0].type = GAN_RADIAL_DISTORTION_2;
      HC[0].nonlinear.radial2.K1 = CFX*XZ*R2;
      HC[0].nonlinear.radial2.K2 = CFX*XZ*R2*R2;
      HC[1].zh = 0.0F;
      HC[1].fx = 0.0F;
      HC[1].fy = d*YZ;
      HC[1].x0 = 0.0F;
      HC[1].y0 = 1.0F;
      HC[1].type = GAN_RADIAL_DISTORTION_2;
      HC[1].nonlinear.radial2.K1 = CFY*YZ*R2;
      HC[1].nonlinear.radial2.K2 = CFY*YZ*R2*R2;
   }
...
/* compute distortion value d from distorted image point Xd */
static Gan_Bool
 compute_distortion ( float K1, float K2,
                      ...
                      float *dp, int *error_code )
{
   ...
   if ( (K1 == 0.0F && K2 == 0.0F) || dR < SMALL_DR_THRES )
   {
      *dp = 1.0F;
      return GAN_TRUE;
   }
   ...
   {
      R2 = R*R;
      deriv = 1.0F + R2*(3.0F*K1 + 5.0F*R2*K2);
      if ( deriv == 0.0F )
      {
         if ( error_code == NULL )
         {
      ...
      Rdiff = (R*(1.0F + R2*(K1 + R2*K2)) - dR)/deriv;
      R -= Rdiff;
                     ...
                     int *error_code )
{
   ...
   if ( !compute_distortion ( CK1, CK2, CTR2, CTDR, COA, COB, &Xd,
                              &d, error_code ) )
   {
      if ( error_code == NULL )
 ...
 * model with two parameters of radial distortion.
 * This is the single precision version.
 *
 * \return #GAN_TRUE on successfully building the camera structure,
 *         #GAN_FALSE on failure.
 * \sa gan_camera_build_radial_distortion_2().
 */
Gan_Bool
 gan_cameraf_build_radial_distortion_2 ( Gan_Camera_f *camera,
                                         float zh, float fx, float fy,
                                         float x0, float y0,
                                         float K1, float K2 )
{
   if ( !gan_cameraf_set_common_fields ( camera, GAN_RADIAL_DISTORTION_2,
                                         zh, fx, fy, x0, y0 ) )
   {
      gan_err_register ( "gan_cameraf_build_radial_distortion_2",
                         GAN_ERROR_FAILURE, "" );
      return GAN_FALSE;
   }

   /* copy non-linear camera parameters */
   camera->nonlinear.radial2.K1 = K1; /* radial distortion cubic term */
   camera->nonlinear.radial2.K2 = K2; /* radial distortion quintic term */
   ...
   {
      if ( K1 < 0.0F )
         CTR2 = -MODEL_FRACTION*MODEL_FRACTION*GAN_ONE_THIRD_F/K1;
      else
         CTR2 = CTDR = COA = COB = FLT_MAX;
   }
   else
   {
      float disc = 0.09F*K1*K1 - 0.2F*K2;
   ...
   if ( CTR2 != FLT_MAX )
   {
      float R = (float)sqrt(CTR2);

      /* compute limited on distorted radial distance */
      CTDR = (1.0F + CTR2*(K1 + CTR2*K2))*R;

      /* compute parameters of outer linear model */
      COA = 1.0F + CTR2*(3.0F*K1 + CTR2*5.0F*K2);
      COB = CTDR - COA*R;
   }
