/**
 * File:          $RCSfile: camera_linear.c,v $
                 ...
                 int *error_code )
{
   ...
   /* consistency check */
   gan_err_test_bool ( camera->type == GAN_LINEAR_CAMERA, "point_project",
                     ...
                     int *error_code )
{
   ...
   /* consistency check */
   gan_err_test_bool ( camera->type == GAN_LINEAR_CAMERA, "point_backproject", GAN_ERROR_INCOMPATIBLE, "" );
                        ...
                        int *error_code )
{
   ...
   /* consistency check */
   gan_err_test_bool ( camera->type == GAN_LINEAR_CAMERA,
...
/**
 * \brief Builds a structure representing a linear camera.
 ...
 *
 * Constructs a structure representing a camera, under the linear pinhole
 ...
 * \sa gan_cameraf_build_linear().
 */
Gan_Bool
 gan_camera_build_linear ( Gan_Camera *camera,
                           double zh, double fx, double fy,
                           double x0, double y0 )
{
   if ( !gan_camera_set_common_fields ( camera, GAN_LINEAR_CAMERA,
                                        zh, fx, fy, x0, y0 ) )
   {
      gan_err_register ( "gan_camera_build_linear", GAN_ERROR_FAILURE, "" );
      return GAN_FALSE;
   }
