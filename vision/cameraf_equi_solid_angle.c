/**
 * File:          $RCSfile: cameraf_equi_solid_angle.c,v $
                 ...
                 int *error_code )
{
   ...
   else
   {
      ...
      float d = 2.0F*(float)sin(0.5*atan(R))/R;
 ...
 * \sa gan_camera_build_equi_solid_angle().
 */
Gan_Bool
 gan_cameraf_build_equi_solid_angle ( Gan_Camera_f *camera,
                                      float zh, float fx, float fy,
                                      float x0, float y0 )
{
   if ( !gan_cameraf_set_common_fields ( camera, GAN_EQUI_SOLID_ANGLE_CAMERA,
                                         zh, fx, fy, x0, y0 ) )
   {
      gan_err_register ( "gan_cameraf_build_equi_solid_angle",
                         GAN_ERROR_FAILURE, "" );
      return GAN_FALSE;
   }
